...
-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║ ЧАСТИНА 1b · ШАВРОВА ЛІЛІЯ, 30.07 — ⚠️ РІШЕННЯ ТРЕБА ПЕРЕГЛЯНУТИ            ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
--
-- 🔴 Я НАЗВАВ ЦЕЙ ЗАПИС ПОРОЖНІМ — І ПОМИЛИВСЯ. Дивився на `answers`, `enps_score`,
--    `overall` і не подивився на `notes`. Заміряно 24.08.2026 на бойовій базі:
--    анкети справді немає, eNPS немає — але Є НОТАТКА HR:
--      notes = {"development": "Цікавить обмін досвіду з лідами, був контекст співбесід "}
--    Тобто це не тестовий рядок на кшталт Ворошилової, а зустріч, після якої HR
--    записала одну змістовну думку. Видалення знищить саме її.
--
--    Для порівняння, другий «порожній» запис того ж дня — Борівець Олеся, 30.07:
--    `answers = {}`, `notes = {}`, лише `enps_score = 7`. Ось там справді нема чого
--    втрачати (і людина вже не працює).
--
-- 📐 ЩО ОСИРОТІЄ: НІЧОГО. Задач з 1×1 у Шаврової немає жодної; планів на 30.07 немає.
--    Тобто технічно видалення безпечне — питання лише в тому, чи хочемо ми втратити
--    нотатку.
--
-- ⚠️ ТОМУ БЛОК НИЖЧЕ НЕ ЗАПУСКАТИ, ПОКИ ВЛАСНИК НЕ ПІДТВЕРДИТЬ ПОВТОРНО, знаючи про
--    нотатку. Він готовий і має той самий запобіжник; бракує саме рішення.
-- ───────────────────────────────────────────────────────────────────────────
DO $$
DECLARE
  v_subject_id  int  := 81;                 -- Шаврова Лілія (заміряно 24.08)
  v_type        text := 'V';                -- HR → Всі
  v_date        date := DATE '2026-07-30';  -- зустріч без анкети, але з нотаткою HR
  n int;
BEGIN
  DELETE FROM one_on_ones
   WHERE subject_manager_id = v_subject_id
...
-- Контроль ПІСЛЯ (для Шаврової): мусить лишитись 0 зустрічей — інших у неї немає.
SELECT count(*) AS meetings_left
  FROM one_on_ones o JOIN managers m ON m.id = o.subject_manager_id
 WHERE m.name ILIKE '%аврова%';
