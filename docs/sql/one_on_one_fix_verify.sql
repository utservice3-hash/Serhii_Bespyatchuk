...
-- ───────────────────────────────────────────────────────────────────────────
-- 📐 ЗНАЧЕННЯ ЗАМІРЯНІ НА БОЙОВІЙ БАЗІ 24.08.2026 (read-only, `kk_ro`) і вже підставлені.
--    A1.1 дав по Ворошиловій ТРИ зустрічі: 20.08 (V, enps 9) · 03.08 (A, overall 10) ·
--    01.07 (V, overall 8, enps 8, збережена 29.07 о 14:50 обліковкою HR). Видаляємо
--    ОСТАННЮ з переліку — рядок на 01.07 рівно один, і він єдиний на цю дату.
--    A1.2: єдина задача з 1×1 у цієї людини (id 1296) висить на зустрічі 03.08, НЕ на
--    01.07 — тобто видалення не лишає жодної задачі без своєї зустрічі.
--    ⚠️ Сама Ворошилова — ЖИВИЙ активний менеджер: чіпаємо тільки цей один запис 1×1.
DO $$
DECLARE
  v_subject_id  int  := 205;                -- Ворошилова Вікторія Петрівна (заміряно 24.08)
  v_type        text := 'V';                -- HR → Всі
  v_date        date := DATE '2026-07-01';  -- тестова зустріч
  n int;
BEGIN
...
--
-- 🔴 ДЛЯ БОРІВЕЦЬ ЦЕЙ БЛОК ЗАПУСКАТИ НЕ ТРЕБА — заміряно 24.08.2026:
--    рядків ДВА (дублі Kommo), і ОБИДВА вже `is_active = false` — синк відпрацював сам
--    (останній успіх за 4 хв до заміру, тобто стан свіжий, а не застряглий).
--    id 200 (kommo 15354672): 2 813 дзвінків Ringostat за 26.03-04.08;
--    id 201 (kommo 15354684): 5 задач, 2 плани, 2 зустрічі 1×1, логін bom1@uts.ua.
--    Видалення будь-якого — це знесена історія, і блок нижче відмовить сам
--    (обидва мають `kommo_user_id`). Робити нічого не треба.
-- ───────────────────────────────────────────────────────────────────────────
DO $$
DECLARE
