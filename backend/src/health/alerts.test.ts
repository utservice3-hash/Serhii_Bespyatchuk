...
/**
 * #135e — ВІКНО ВИКАТУ ВИДНО В `/api/health`.
 *
 * 🔴 Механізм, що ГЛУШИТЬ сигналізацію, зобовʼязаний показувати себе сам. Поки
 * намір чинний, тривога про рестарт не зʼявиться — і якщо про це не видно
 * ззовні, ми отримуємо «гудок відʼєднаний» у новій формі: на сигналізацію
 * розраховують, а вона мовчить із поважної, але невидимої причини.
 *
 * ⚠️ Поле мусить бути ПРИСУТНІМ завжди (`null`, коли наміру немає): поле, яке
 * зʼявляється лише в одному зі станів, неможливо відрізнити від зниклого.
 *
 * 🧨 САБОТАЖ (виконано): прибрати `deployIntent` із відповіді → червоніє.
 */
test("#135e /api/health показує чинне вікно викату", needsApi(), async () => {
  const r = await fetch(`${API_BASE}/api/health`);
  assert.equal(r.status, 200, `🔴 /api/health віддав ${r.status}`);
  const body = await r.json() as Record<string, unknown>;
  assert.ok("deployIntent" in body,
    "🔴 у health немає поля deployIntent — механізм, що вимикає тривогу, себе не показує");
  const di = body.deployIntent;
  if (di !== null) {
    assert.ok(typeof di === "object" && di !== null && "expiresAt" in di,
      "🔴 намір є, але без стелі часу — саме стеля відрізняє намір від застряглого прапорця");
  }
});
