...
/**
 * ⏱ ТІ САМІ ХЕЛПЕРИ, АЛЕ ВІД ЗАДАНОЇ МИТІ — для гейтів, які порівнюють фікстуру з
 * власним `now`.
 *
 * 🔴 ЧОМУ ЦЕ НЕ ПРИДИРКА, А ВИПРАВЛЕННЯ КРИХКОСТІ. `at()` читає годинник САМ, тож у
 * тесті, який спершу взяв `const now = new Date()` (мить T0), а потім побудував
 * фікстуру (мить T1 ≥ T0), вік рахується як `floor((T0 − (T1 − N хв)) / 60000)` — і
 * дає N−1, щойно між двома рядками мине бодай ОДНА мілісекунда. Помилка однобічна:
 * буває N або N−1, ніколи N+1.
 * 📐 Заміряно 14.09.2026 на 200 000 прогонів тієї самої арифметики: 8 промахів
 * (0.004 %). Рідко — але «рідко» на спільному наборі означає червоне в чужому
 * прогоні під навантаженням, і саме так `#317` одного разу впав у повному прогоні,
 * лишаючись зеленим у власному файлі.
 *
 * ⚠️ `at`/`claim` НЕ переписуємо: їх вживають `#227`, `#227b`, `#227c`, `#227e`, і
 * там вони правильні — фікстура й перевірка беруть годинник в одну мить. Міняти
 * спільний хелпер заради одного гейта означало б потягти чужі фікстури без потреби.
 */
const atFrom = (base: Date, min: number) => new Date(base.getTime() - min * 60_000).toISOString();
const claimFrom = (base: Date, who: string, min: number, reason = "викат"): Claim =>
  ({ who, at: atFrom(base, min), reason });
...
test("#317 прострочення рахується від ОСТАННЬОГО ДОТИКУ, а не від взяття", () => {
  ...
  const live = viewLock({ ...claimFrom(now, "Основний", 100), touchedAt: atFrom(now, 1) }, now);
  ...
  const dead = viewLock({ ...claimFrom(now, "Основний", 5), touchedAt: atFrom(now, TTL_MIN + 5) }, now);
  ...
  assert.equal(viewLock(claimFrom(now, "Основний", TTL_MIN + 1), now).expired, true);
  assert.equal(viewLock(claimFrom(now, "Основний", 1), now).expired, false);
});
...
test("#318b 🪞 ДЗЕРКАЛО: у run немає lockTake, і саме тому ініціалізація — єдиний шлях", async () => {
  ...
  // 🔴 І друга половина: після lockTake ланцюг МУСИТЬ вважати замок своїм, інакше кроки
  // до нього (їх у `check` лише toolsCheck) і після нього поводились би однаково.
  //
  // ⚠️ УТОЧНЕНО 07.09.2026 — І ЦЕ ПРО ПРОКСІ, А НЕ ПРО ТВЕРДЖЕННЯ. Тут стояла регулярка
  // по ДЖЕРЕЛУ `deploy.ts` (`step.id === "lockTake" && r.ok) lockOurs = true`). Рішення
  // переїхало в чисту функцію `holdsLockAfter` — і гейт почервонів на рефакторингу, який
  // саме це рішення ПОЛІПШИВ (правило 10: проксі падає від рефакторингу й мовчить від
  // дефекту). Тепер питаємо ПОВЕДІНКУ; сама функція має власне дзеркало на обидва боки (#352).
  const { holdsLockAfter } = await import("./deployPlan.js");
  assert.equal(holdsLockAfter("lockTake", false, true), true,
    "🔴 після lockTake ланцюг не вважає замок своїм — на вільному замку дотику не буде ніколи");
});
