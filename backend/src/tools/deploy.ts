 ...
 */
export function judgeAccept(out: string, log: string): { ok: boolean; detail: string } {
  ...
  if (doneN !== needN) return { ok: false,
  ...
  return { ok: true, detail: `${doneN} із ${needN}, падінь 0` };
}
...
export const handlers: Record<string, (ctx: Ctx) => Promise<StepResult> | StepResult> = {
  ...
  accept: async (c) => {
    ...
    return { id: "accept", ...judgeAccept(out, log) };
  },
  /**
