 ...
 *
 * 🔴 ЗАМІРЯНА ДІРА КРИТЕРІЮ: ВКЛАДЕНІ ПІДТЕСТИ ЙОМУ НЕВИДИМІ (27.08.2026).
 * `node --test` виводить у TAP верхнього рівня лише КОРЕНЕВИЙ тест; підтести
 * `await t.test(...)` — рядками з відступом усередині його блоку. Ми ж читаємо
 * ліві `ok`/`not ok`, тож усе, що всередині, для нас не існує.
 *
 ...
 * рівно «+1 падіння, перестало виконуватись 0» — тобто число ② було НУЛЬ саме
 * тому, що воно цього класу не бачить. Полагодив я їх лише тому, що запустив файл
 * очима, а не тому, що інструмент сказав.
 *
 * ⚠️ ПРАКТИЧНИЙ НАСЛІДОК, ПОКИ ЦЕ НЕ ЗАКРИТО: «+1 падіння» у файлі з вкладеними
 * підтестами читати як «+1 і НЕВІДОМО СКІЛЬКИ ЩЕ» — і прогнати цей файл окремо.
 * Закривати треба розбором відступів у TAP (підтести мають власні `ok` з відступом),
 * а це зміна самого критерію викату — окремий прохід зі своїм прийманням.
 */
