...
test("#354 запит публікації новини розбирається й планується живою БД", needsDb(), async () => {
  ...
  try {
    ...
    // 🎯 Четвертий параметр — адресат (09.09.2026). NULL = «бачать усі»; окремо перевіряє,
    //    що приведення `$4::text[]` живою базою планується (типова помилка масивів).
    await cl.query(PUBLISH_SQL, ["проба гейта", "тіло проби", "гейт-354-неіснуючий-sha", null]);
  } catch (e) {
  ...
  assert.ok(code === "" || code === "42501",
    ...
    + "08P01 — що змінили кількість параметрів у PUBLISH_SQL і забули цей гейт.");
});
