...
/**
 * 🔗 #646 — ОДИН платіж у ДВОХ формах видачі мусить дати ОДИН ключ.
 ...
 */
test("#646 ТОЙ САМИЙ ПЛАТІЖ У ФОРМІ «_online» — той самий ключ, двійник не заводиться", () => {
...
/**
 * 🪞 #646b — запис БЕЗ `NUM_DOC` не лишається без ключа й не клеїться з чужими.
 ...
 */
test("#646b 🪞 ДЗЕРКАЛО: без NUM_DOC драбина ID → REF жива, privat:undefined не буває", () => {
...
/**
 * 🔗 #646c ЖИВИЙ — після `tools/dedupePrivat.ts` у базі (1) немає двох рядків Привату з
 ...
 */
test("#646c ЖИВИЙ: двійників (рахунок, REF, NUM_DOC) — нуль, і ключ кожного рядка == normalizePrivat(raw_json)",
  { ...needsBackendEnv() }, async () => {
  const { pool } = await import("../db/pool.js");
