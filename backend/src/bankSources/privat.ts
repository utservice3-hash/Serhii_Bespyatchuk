...
export interface PrivatItem {
  ID?: string; REF?: string; NUM_DOC?: string; TECHNICAL_TRANSACTION_ID?: string;
  TRANTYPE?: "C" | "D"; CCY?: string;
...
/**
 * 🔑 КЛЮЧ ЗАПИСУ — `REF` + `NUM_DOC`. Це ТРЕТЯ редакція ключа, і кожна з двох попередніх
 * коштувала викривлених грошей на екрані. Обидві обирали ключ за тим, що бачили в даних на
 * той момент, не спитавши, чи він СТАБІЛЬНИЙ між двома видачами того самого платежу.
 *
 * ① До 08.09.2026 ключем був `REF`. Платіж і комісія за нього мають ОДИН `REF` (різняться
 *    лише `ID` = `REF + літера + дата`), тож комісія затирала платіж: за 30 днів 400 пар,
 *    370 платежів зникли з виписки (ЮТС 305, Автомув 93). Привід — Юля, ГУТА-ТРАНС
 *    41 666,67 ₴ «немає на виписці».
 * ② З 08.09 ключем став `ID`. Він розрізняє платіж і комісію — але Приват віддає той самий
 *    платіж ПОВТОРНО в іншій формі: числовий `ID` і `TECHNICAL_TRANSACTION_ID` з суфіксом
 *    `_online`, при тих самих `REF` і `NUM_DOC`. 13–14.09 так удруге приїхали 109 платежів
 *    за 09–14.09 (ЮТС 50, Автомув 43, ФОП 10, EUR 5, USD 1): вихідних на ≈2,08 млн ₴ і
 *    вхідних на ≈1,70 млн ₴ стало на екрані ВДВІЧІ більше, і CSV для 1С поїхав із дублями.
 *
 * 📐 Чому саме ця пара, заміряно на ВСІХ 7 445 рядках Привату (22.09.2026):
 *   · платіж і його комісія: `REF` спільний, `NUM_DOC` різний у 1 517 із 1 517 пар
 *     (у комісії `NUM_DOC` похідний від `REF`, напр. `8SOEM47D6Y`);
 *   · дві форми одного платежу: `REF` і `NUM_DOC` однакові в 109 із 109 пар;
 *   · груп `(рахунок, REF, NUM_DOC)` з різними сумами — 0, тобто пара нічого чужого не клеїть;
 *   · `NUM_DOC` є в 7 445 із 7 445 (у Моно цих полів немає взагалі, це інший адаптер).
 *
 * Драбина запасних шляхів для запису БЕЗ `NUM_DOC` (у базі 0, контракт API не обіцяє):
 * `ID` → `REF` → дата-сума-рахунок. Старі рядки під новий ключ переводить
 * `tools/dedupePrivat.ts` — він же згортає вже заведені двійники.
 */
export function privatTxKey(it: PrivatItem, abs: number): string {
  if (it.REF && it.NUM_DOC) return `${it.REF}#${it.NUM_DOC}`;
