...
/**
 * 📊 #145–#145d — eNPS: ОДНА ШКАЛА, ОДНА ФОРМУЛА, ДОВІЛЬНИЙ ПЕРІОД.
 *
 ...
 * мертву копію видалено — лишились ДВІ: ядро і фронтовий пікер. Другу звіряє `#145c`,
...
/**
 * #145 — СМУГИ ПОКРИВАЮТЬ УВЕСЬ ДІАПАЗОН, БЕЗ ДІРОК І БЕЗ ПЕРЕТИНІВ.
 *
 ...
 */
test("#145 смуги eNPS покривають −100..100 рівно один раз", () => {
 ...
 */
test("#145b eNPS = %промоутерів − %критиків, і рахується з сирих лічильників", () => {
...
/**
 * #145c — ФРОНТ І ЯДРО КЛАСИФІКУЮТЬ ОДНАКОВО.
 *
 ...
 */
test("#145c шкала на фронті збігається з ядром — бал у бал", async () => {
...
/**
 * #145d — ПЕРІОД ДОВІЛЬНИЙ, ОБИДВА КІНЦІ ВКЛЮЧНО.
 *
 ...
 */
test("#145d період eNPS: довільний, обидва кінці включно, грануляція за довжиною", () => {
