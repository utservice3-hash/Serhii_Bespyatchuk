...
-- 💰 МІСЯЧНИЙ ПЛАН ВИТРАТ НА РЕКЛАМУ — ручний, і це не тимчасово (рішення власника 09.09.2026).
--
-- 🔴 ЧОМУ РУЧНИЙ, ЯКЩО В НАС УМОВА «ВСЕ АВТОМАТИЧНО». Умова стосується того, що МОЖНА
-- дістати з джерела. Факт витрат саме так і автоматизовано: GA4 тягне його з Google Ads
-- щодня (звірка за 3 місяці: 69 днів із 70 збіглися з ручною таблицею до гривні). А план —
-- це домовленість людей про майбутнє («узгоджуємо з рекламщиком», Сергій, 09.09.2026);
-- його немає звідки взяти жодним API. Питання було не «руками чи ні», а ДЕ саме руками:
-- у Google-таблиці, якої не видно з дашборду, чи тут, де видно хто і коли поставив.
--
-- 🔴 ЧОМУ МІСЯЦЬ, А НЕ ДЕНЬ. Заміряно на бойових даних: у `ad_budget_daily.budget_plan`
-- у кожному місяці РІВНО ОДНЕ значення на всі дні (липень 4 839 × 30, серпень 4 333 × 31,
-- вересень 5 167 × 30). Тобто денні числа плану — механічне ділення місячної суми, і
-- жодної інформації в них немає. Зберігати їх означало б плодити 30 рядків заради одного.
--
-- ⚠️ ПЛАН БЕЗ ПДВ (Сергій, 09.09.2026). Факт із GA4 і факт із таблиці збігаються один в
-- один, тобто вони в ОДНІЙ базі ПДВ — слідів коефіцієнта 1.2 у 70 днях немає жодного.
-- Отже приведення не потрібне; якщо колись виявиться, що джерела різні, це буде видно
-- саме як розбіжність, а не як тихо неправильний ROMI.
CREATE TABLE IF NOT EXISTS ad_budget_manual (
  month   DATE PRIMARY KEY,          -- перше число місяця, за Києвом
...
-- 📵 ЖУРНАЛ СИГНАЛІВ ПО ПРОПУЩЕНИХ (ТЗ-1, прохід 3, 16.09.2026). Одна задача на
-- менеджера + номер + київський день — рішення власника. Логіка — `core/missedCallSignal.ts`.
-- 🔴 АНТИ-ДУБЛЬ ТРИМАЄ PRIMARY KEY, А НЕ ПОШУК ЗА ЗАГОЛОВКОМ. Сусідні джоби шукають
-- `title LIKE 'префікс%'`, але заголовок людина може перейменувати — і наступний тік
-- створив би другу задачу. Тут ключ живе окремо від того, що редагується на екрані.
-- `ON DELETE SET NULL`: видалена руками задача не видаляє памʼять про сигнал; новий
-- пропущений того ж дня створить нову задачу, а без нового — нічого не відродиться.
-- `last_signal_at` — момент ОСТАННЬОГО пропущеного, по якому був сигнал: вихідний
-- ПІСЛЯ нього закриває задачу, новий пропущений ПІСЛЯ нього — перевідкриває.
CREATE TABLE IF NOT EXISTS missed_call_tasks (
  manager_id     INTEGER NOT NULL REFERENCES managers(id),
...
CREATE INDEX IF NOT EXISTS idx_mct_open ON missed_call_tasks(kday) WHERE closed_at IS NULL;
...
-- ══════════════════════════════════════════════════════════════════════════
-- 🔴 ХТО ДОДАЄ ПРАВО АДМІНУ — ДОДАЄ ЙОГО ЩЕ ТРЬОМ РОЛЯМ.
-- Три рядки нижче копіюють `permissions` адміна ЦІЛКОМ у ceo/opdir, kvp і
-- financier. Отже будь-яка видача права адміну ВИЩЕ за них розтечеться на ці
-- ролі — не сьогодні, а на НАСТУПНОМУ прогоні `npm run migrate`, тобто на
-- чужому викаті. Рішення власника «це право лише адміну» так скасовується тихо,
-- і той, хто його ухвалював, про це не дізнається.
--
-- 📐 Заміряно 27.08.2026 поведінкою, не міркуванням: гейт `#231f` накочує схему
-- ДВІЧІ на scratch-базі; варто прибрати рядок зняття біля видачі 1×1 — і
-- фінансист дістає `view_all_1x1` на другому прогоні.
--
-- ⚙️ ПОКИ КЛАС НЕ ЗАКРИТО, ПРАВИЛО ТАКЕ: видавати право адміну — НИЖЧЕ за ці три
-- рядки, і поруч ставити явне зняття для тих ролей, кому воно не призначене
-- (зразок — блок 1×1 і `merge_receivables`). Справжній фікс — щоб синки не
-- копіювали набір цілком; він у боргу окремим проходом.
-- ══════════════════════════════════════════════════════════════════════════
UPDATE roles SET
...
UPDATE roles SET permissions = permissions || '{"merge_clients": true}'::jsonb
 WHERE key IN ('kvp', 'opdir', 'admin', 'ceo');
UPDATE roles SET permissions = permissions - 'merge_clients'
 WHERE key IN ('financier');

-- 🧾 `manage_credit_limits` — хто править узгоджену відстрочку (Е4, рішення
-- власника 24.08.2026): СЕО · опердир · адмін · КВП · ФІНАНСИСТ.
--
-- 🔴 СТОЇТЬ ТУТ, А НЕ ВИЩЕ, І ЦЕ НЕ КОСМЕТИКА. Рівно на цьому ми спіймались із
-- `merge_clients` для СЕО тиждень тому: грант поставили серед інших роль-апдейтів,
-- а блок зняття вище забирав право НАЗАД тим самим прогоном — `migrate` друкував
-- «Migration applied.», а права не було. Тому будь-який новий грант — після
-- ВСІХ трьох синків ролей.
--
-- ⚠️ ФІНАНСИСТ ТУТ Є, хоч `merge_clients` йому свідомо не дали. Це не непослідовність:
-- ліміт відстрочки — фінансове рішення, а склейка клієнтів — ні. Забирати право
-- колись → додати ключ у список ЗНЯТТЯ нижче, а не лише прибрати звідси: інакше
...
-- клієнтів у дебіторці й НЕ МІГ роз'єднати їх ніде. Односторонні двері, які прямо
-- суперечать правилу «дія, доступна в інтерфейсі, мусить бути скасовною тим самим
-- інтерфейсом».
--
-- 🔴 ПЕРША РЕДАКЦІЯ СТОЯЛА ВИЩЕ ПО ФАЙЛУ — І НЕ ПРАЦЮВАЛА, хоча `migrate`
-- бадьоро друкував «Migration applied.». Грант відпрацьовував, а цей самий блок
-- зняття нижче забирав право назад тим самим прогоном. Спіймано окремим запитом
-- ПІСЛЯ міграції (`ceo.merge_clients` = null), як і вимагає правило «міграція
-- пройшла ≠ зміна застосувалась» — рівно те, на чому колись три доби горів
-- `hr.data_scope`.
--
-- ⚠️ `financier` лишається БЕЗ права свідомо: рішення 31.07 підняло його до рівня
-- адміна, але склейку клієнтів власник йому не давав, і зняття тут — єдине, що
-- утримує право від просочування через копіювання `permissions` адміна вище.
-- Забирати право в СЕО колись → вернути 'ceo' у список зняття, а не лише
-- прибрати з гранту: інакше копіювання від адміна поверне його мовчки.
...
--
-- 🔴 ЦЕ НЕ ОДНОРАЗОВА МІГРАЦІЯ, А ЩОДЕННИЙ ПИСАР БЕЗ ЗВОРОТНОГО ХОДУ.
-- `schema.sql` виконується НА КОЖНОМУ ВИКАТІ, тож цей UPDATE проходить щоразу. Він
-- лише СТАВИТЬ `below_min = true` і ніколи не знімає: рядок, позначений колись, лишиться
-- позначеним, навіть якщо поріг відтоді знизили або зняли. Читати його як «бекфіл для
-- старих рядків» — помилка, якої ми вже раз припустились.
--
-- 🔴 ПОРІГ БЕРЕТЬСЯ ТІЛЬКИ З НАЛАШТУВАНЬ, БЕЗ ЗАШИТОГО ФОЛБЕКА (02.09.2026).
-- Було `COALESCE(…, 30000)`: зникне рядок `app_settings` — і межа мовчки повернулась би
-- на наступному ж викаті, без чийогось рішення. Це небезпечніше за саму межу, бо
-- відновлює її НЕВИДИМО. Немає налаштування → немає порога → UPDATE не чіпає нічого,
-- і це чесніше за тихе «30000 звідкись».
--
-- 📐 Заміряно 02.09.2026: позначених рядків 4, усі за серпень. Рішення власника —
-- ЛИШИТИ їх як історію: лишити не потребує запису в прод, а почистити потребує.
UPDATE plan_formation pf
 ...
 WHERE m.id = pf.manager_id
   ...
   AND pf.proposed_value < (SELECT (data->>'planMinPerManager')::numeric
...
-- ═════════════════════════════════════════════════════════════════════════════
-- 🧊 ЗАМОРОЖЕНИЙ ПЛАН ТИЖНЯ (рішення власника 06.08.2026)
--
-- 🔴 НАВІЩО ЗНІМОК, А НЕ ЖИВИЙ ПЕРЕРАХУНОК. Динамічна ціль рахується від залишку
-- місяця, тож поки тиждень триває, вона повзе: людина відстала у вівторок — у
-- середу план виріс — наздогнати стало неможливо в принципі. Ціль, яку не можна
-- виконати, перестає бути ціллю. Тому план тижня фіксується РАЗ у понеділок
-- 00:00 за Києвом і всередині тижня не змінюється.
--
-- 🔴 І ДРУГЕ, ВАЖЛИВІШЕ: без знімка МИНУЛІ тижні переписувались би заднім числом.
-- Звіт за минулий тиждень мусить показувати той план, що діяв ТОДІ, — інакше
-- історія змінюється щоразу, коли надходять нові гроші, і жодну обіцянку не можна
-- перевірити. Це той самий клас, що знімки виручки, які «мутували минулі місяці».
--
-- `source`: 'live' — зафіксовано в момент початку тижня; 'backfill' — відновлено
-- ретроспективно (за тижні, що минули ДО впровадження механізму). Розрізняти
-- обовʼязково: відновлений знімок — це реконструкція, і вона мусить бути видимою,
-- а не вдавати із себе збережений у момент.
CREATE TABLE IF NOT EXISTS weekly_plan_snapshots (
  manager_id INTEGER NOT NULL REFERENCES managers(id),
  week_start DATE NOT NULL,
  month_start DATE NOT NULL,
  plan NUMERIC NOT NULL,
  -- складові, збережені РАЗОМ із результатом: без них знімок неможливо перевірити
  -- через півроку («звідки взялось це число»), а перерахунок дасть уже інше.
  month_plan NUMERIC NOT NULL,
  fact_before NUMERIC NOT NULL,
  working_days_week INTEGER NOT NULL,
  working_days_rest INTEGER NOT NULL,
  source TEXT NOT NULL CHECK (source IN ('live', 'backfill')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (manager_id, week_start)
);
...
-- 🧾 УЗГОДЖЕНА ВІДСТРОЧКА ПЛАТЕЖУ — НАШЕ ДЖЕРЕЛО, НЕ ГУГЛ-ТАБЛИЦЯ (Е4, 24.08.2026)
--
-- ЩО ЗАМІНЮЄ. До Е4 `limit_days` приїжджав із аркуша «Лист20» Google-таблиці,
-- що дісталась у спадок: власник не знає, хто її веде, і орієнтується на дані
-- контрагента. Заміряно перед переходом — аркуш покривав **29 із 74** боржників,
-- решта 61% (1 683 550 ₴) не мали ліміту взагалі.
--
-- 🔴 NULL І 0 — РІЗНІ ШЛЯХИ ДО ОДНОГО БІЗНЕС-СЕНСУ, І ПЛУТАТИ ЇХ НЕ МОЖНА.
-- Рядка немає  = ліміт клієнту НІКОЛИ не ставили (успадковано з аркуша, де його
--                просто не було).
-- `limit_days=0` = ліміт розглянули і СВІДОМО не дали.
-- Обидва означають «не узгоджено», тобто перевізника ми не сплачуємо (рішення
-- власника 24.08.2026) — але «не дивились» і «подивились і відмовили» це різні
-- відповіді на питання «чому», і на екрані вони підписані окремо. Тримає #187.
--
-- ⚠️ `limit_days` тут — НАЛАШТУВАННЯ. Вік боргу (`receivables.overdue_days`) з Е4
-- рахується з дат рахунків і в цій таблиці не живе: щоденний факт не редагують.
CREATE TABLE IF NOT EXISTS client_credit_limits (
  ...
  set_by     INTEGER REFERENCES users(id),
  set_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- Примітку стереже БД, а не роут: роут обходить будь-який скрипт. Той самий
