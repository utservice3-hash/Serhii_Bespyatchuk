...
/**
 * 📊 #367…#367d — РОЗБІР ВІДПОВІДІ GA4 (08.09.2026).
 *
 ...
 * витрати замість сесій, і #367 почервоніє саме на цьому.
 */
...
test("#367 РОЗБІР GA4: рядки день×кампанія з правильними cost/clicks (метрики не за позицією)", () => {
...
test("#367b 🪞 ОРГАНІКА: cost 0 і НЕ роздуває суму витрат", () => {
  const rows = parseGa4Report(FIXTURE);
...
test("#367c Σ ВИТРАТ ПО ДНЯХ == Σ ПО КАМПАНІЯХ (інваріант групування)", () => {
  const rows = parseGa4Report(FIXTURE);
...
test("#367d НЕНАЛАШТОВАНА ІНТЕГРАЦІЯ — вид помилки `config`, а не `data`/`unknown`", () => {
