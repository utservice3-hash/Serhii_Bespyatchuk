...
test("#68c УСПІХ НЕ ЗАТИРАЄ last_error", async (t) => {
  await withScratch(t, async (c) => {
    ...
    // ⚠️ `>=`, А НЕ `>` — І ЦЕ ВИПРАВЛЕННЯ ЛАТЕНТНОЇ ФЛУКТУАЦІЇ, а не послаблення.
    // `now()` — це transaction_timestamp (мікросекунди). Проти мережевого Neon два
    // послідовні runJob завжди різняться, але на локальному scratch-кластері (unix-сокет,
    // без мережі) успіх і помилка встигають в ОДНУ мікросекунду, і строге `>` дає false.
    // Заміряно 08.09.2026: 3 падіння з 5 локально; проявилось, щойно фікс локалі вперше
    // змусив кластер РЕАЛЬНО бігати на dev-машині (доти #68c скіпався скрізь).
    // 🔴 `>=` — правильна семантика самої свіжості: у розбіжності дат успіх ВИГРАЄ нічию
    // («одужав»), інакше джоба, що впала й одразу піднялась, читалась би як «в аварії».
    // Захист від справжньої вади (успіх записав СТАРШУ дату) лишається: тоді `>=` теж впаде.
