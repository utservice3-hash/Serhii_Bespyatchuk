 ...
 *
 * 🔴 3 → 8 (11.08.2026). Заміряно: гонка в `pg-copy-streams` валить **60%** спроб
 * (30 прогонів на `stats_series`: 12 успіхів, 18 падінь). При такій частоті три
 * спроби — не запобіжник, а лотерея: шанс втратити таблицю 0.6³ = **21.6%**, тобто
 * ≈17 таблиць із 78 за прогін. Вісім спроб дають 0.6⁸ = **1.7%** (≈1.3 таблиці).
 *
 * ⚠️ Це ПІДСТРАХОВКА, а не лікування. Причину прибирає `db/copyStreamPatch.ts`;
 * тримаємо ретраї високими, доки не переконаємось у патчі на нічних прогонах.
 * Ознака, що патч працює, — `tables retried: 0` у маніфесті.
 */
export const COPY_ATTEMPTS = 8;
