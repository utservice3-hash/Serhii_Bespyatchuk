...
export async function linkCalls(): Promise<{ byPhone: number; byFio: number }> {
  ...
  //
  // ⚠️ ФІКС НЕ ЧІПАЄ ПОЛІТИКУ `is_active`. Дзвінки людей, деактивованих у CRM
  //    (32 371, 7.8%), лишаються «менеджер невідомий» — рівно як було. Чи варто
  //    приписувати історичні дзвінки тому, хто вже пішов, — ВІДКРИТЕ ПИТАННЯ до
  //    власника, а не побічний ефект баг-фіксу.
  //
  ...
  // рядками (активний + деактивований). Якби колись зʼявились двоє РІЗНИХ
         ...
         SELECT DISTINCT ON (k) id, k FROM (
           SELECT id, is_active,
                  regexp_replace(lower(translate(
                    ...
                    'ʼ’\`', '')), '\\s+', '', 'g') AS k
             FROM managers WHERE is_active     -- політика НЕ змінюється цим фіксом
