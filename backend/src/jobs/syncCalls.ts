...
export async function linkCalls(): Promise<{ byPhone: number; byFio: number }> {
  ...
  // Геннадіївна»), а `managers.name` — коротке («Крицька Діана»). Точний збіг дав
  // би нуль мапінгів і тихо перетворив би ВСІ дзвінки на «менеджер невідомий».
  // Правило те саме, що вже працює в агрегаті «Статистик» (`resolveLead`):
  // прізвище+імʼя без пробілів, з нормалізацією апострофа.
  const byFio = await pool.query(
    `UPDATE ringostat_calls rc SET manager_id = mm.id
       FROM (
         SELECT id, regexp_replace(lower(translate(name, 'ʼ’\`', '')), '\\s+', '', 'g') AS k
           FROM managers WHERE is_active
       ) mm
