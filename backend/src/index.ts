...
// 🛡 Спам-заявки з форми сайту без телефону — відхиляти кожні 3 хв (слово Романа 23.09.2026).
// Пише в Kommo, тож підкоряється паузі кола так само, як синк. Хвилини 1,4,7,… — жодна не
// збігається з :00/:30, де сходяться 13 розкладів і синк (правило зони jobs-sync). Один тік —
// 1–2 читання + N відхилень під тротлінгом 800 мс, тобто ~20 запитів/год у тиші.
...
// що живить банер, і відправляє НОВЕ в телеграм.
//
// 🔴 БУЛО ЩОХВИЛИНИ — ЗНИЖЕНО 21.08.2026 (рішення власника). Тихий тік коштує НЕ
// «один SELECT», як тут було написано: `collectAlerts()` — це 8 перевірок, і замір
// на проді дав 1 497-1 670 мс роботи в БД НА КОЖЕН тік. Щохвилини це 1 440 прогонів
// і ~36 хв компуту Neon на добу заради події, якої зазвичай немає.
//
// ⚠️ ЦІНА, НАЗВАНА ВГОЛОС: тривога тепер приходить у межах 5 хв, а не 1. Проти
// 14 год 52 хв простою 10.08.2026, заради якого поштаря й заводили, це не втрата.
// `build:stale` (вікно між збіркою й рестартом) має власний коротший інтервал
// ПОВТОРУ в `alertRules`, але ПЕРШЕ повідомлення тепер теж чекає до 5 хв — це
// свідомо: у процедуру викату входить рестарт, а не очікування алерту про нього.
//
// 🔗 `everyMin` у `monitoredJobs.ts` МУСИТЬ дорівнювати цьому крону — інакше сторож
