...
/**
 * 👁 ВІДДІЛИ, ЯКІ НЕ ПОКАЗУЄМО НА ЕКРАНІ — СПИСОК ПОРОЖНІЙ (рішення власника 08.09.2026).
 *
 * 🔴 ТУТ СТОЯЛО `["finance", "hr"]`, І ЦЕ БУЛО ВИКОНАННЯМ НЕ ТІЄЇ ВКАЗІВКИ. Слова
 * власника 07.09 — «оці ручні вкладки всі поскривай» — прочитали як «відділи, де всі
 * метрики ручні», і сховали `finance`/`hr` у розділі **«Статистики (відділи)»**. А
 * власник показував на вкладку **«Ручні»** (✍️), і живе вона в ІНШОМУ розділі —
 * «Статистики» (`StatisticsChartsSection`, `CATS.key === "manual"`).
 *
 * 📐 Ціна помилки, заміряна 08.09.2026:
 *   ① названу ціль не зачепили — вкладка «Ручні» лишалась робочою;
 *   ② сховали те, про що не просили — **26 метрик** двох відділів;
 *   ③ із них лише **8** досяжні графіками сусіднього розділу, тобто **18 не було видно
 *      НІДЕ**, хоч новина й обіцяла «дані нікуди не зникли»;
 *   ④ новина 197 стверджувала «з розділу „Статистики“ прибрано вкладки „Фінанси“ і „HR“»
 *      — не з того розділу, і в названому вони лишались видимими. Двічі неправда, і
 *      спростувати її можна було двома кліками.
 *
 * 🔴 УРОК, ЯКИЙ ВАРТИЙ БІЛЬШЕ ЗА САМ ВІДКАТ: два розділи мають майже однакову назву
 * («Статистики» і «Статистики (відділи)» — див. CLAUDE.md, вони вже позначені як
 * кандидати на злиття), і вказівка власника була розвʼязана проти НЕ ТОГО. Перш ніж
 * ховати щось за словами «оця вкладка», спитай, у якому вона розділі — назви тут не
 * розрізняють.
 *
 * ⚠️ КОНСТАНТА ЛИШАЄТЬСЯ, А НЕ ВИДАЛЯЄТЬСЯ. Механізм приховування робочий і колись
 * знадобиться; видалити його означало б наступного разу писати наново й наново
 * помилятися. Порожній список каже «сьогодні не ховаємо нікого» — і це перевіряється
 * (`#363`), тоді як відсутність константи не перевіряється ніяк.
 */
export const HIDDEN_DEPARTMENTS: readonly string[] = [];

export function visibleDepartments(all: DepartmentDef[] = CATALOG): DepartmentDef[] {
  return all.filter((d) => !HIDDEN_DEPARTMENTS.includes(d.key));
}
