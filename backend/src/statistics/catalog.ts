...
/**
 * 🙈 ВІДДІЛИ, ЯКІ НЕ ПОКАЗУЄМО НА ЕКРАНІ (рішення власника 07.09.2026).
 *
 * Дослівно: «оці ручні вкладки всі поскривай — наша ціль, щоб у ручному нічого не
 * заповнювали, це має бути автоматизовано». Заміряно того ж дня: у `finance` і `hr`
 * УСІ метрики мають `source: "manual"`, тобто екран існує лише для ручного вводу —
 * рівно того, від чого власник відмовляється.
 *
 * 🔴 ХОВАЄМО ПОДАЧУ, А НЕ ЗМІСТ. Каталог і всі записи лишаються: `getMetric` для
 * схованого відділу мусить працювати далі, інакше зламається ручний запис і CSV-імпорт
 * (`csvIndexMonth`/`csvIndexWeek` живуть саме в цих метриках). Прибрати їх із довідника
 * означало б знищити історію заради того, щоб її не показувати.
 *
 * 🔴 ФІЛЬТР НА СЕРВЕРІ, А НЕ У ФРОНТІ. Роут віддає каталог цілком, і фільтр у фронті
 * лишив би сховані відділи у ВІДПОВІДІ — той самий клас, що фільтр замість клампа в
 * архіві: екран ховає, а один `curl` віддає.
 *
 * ⚠️ `marketing.lg_budget` — ОДНА ручна метрика серед автоматичних, і вона тут НЕ
 * ховається: рішення про неї власник не давав, а сховати цілий відділ через одну
 * клітинку означало б прибрати з очей і чотири автоматичні.
 */
export const HIDDEN_DEPARTMENTS: readonly string[] = ["finance", "hr"];
