...
export const CASES: Case[] = [
  ...
  {
    // 🔴 ПИТАННЯ ПЕРЕПИСАНО 03.09.2026. Стояло «к-сті ВІДПРАВЛЕНИХ авто», а еталон рахує
    // `success_by_bucket`, тобто перехід у 142 — це «успішні», інша множина (див.
    // `core/dispatched.ts`). Еталон правильний, брехала НАЗВА питання: оракула питали про
    // одне, а звіряли з іншим, і розбіжність читалась би як його помилка.
    id: "Q4", q: "статистику к-сті УСПІШНИХ угод за місяць",
    reference: async () => {
      const { from, to } = last12();
