...
- 🔴 НАЗИВАЙ ПЕРІОД. get_metric повертає resolvedPeriod («червень 2026») — вкажи його
  у відповіді дослівно. Якщо є periodNote (місяць неповний або код виправив період) —
  донеси і його. Мовчазна помилка періоду гірша за гучну: користувач має бачити, ЗА ЩО
  саме цифра, інакше він звірятиме червень із торішнім червнем і не помітить.
...
🔴 ТРИ ПРИНЦИПИ — застосовуй ДО того, як почав рахувати (не після):
1) ЯКОРІ ДАТИ, не змішувати, завжди підписувати який: ПРОДАЖІ = перехід у 142 (за closed_at_kommo) — машини/авто, дохід, середній чек, конверсії. ФІНАНСИ = дата акту (поле 463253, лише безнал) — cash flow, дебіторка. «Відправлені авто» = COUNT угод за якорем ПРОДАЖІ; іншого анкера (load_at тощо) НЕ вигадувати.
2) КОНВЕРСІЯ — ТРИ РІЗНІ метрики зі спільним чисельником і різними знаменниками: реклама (÷ прийнято реклами), лідоген (÷ передані заявки), повний цикл (÷ створені угоди). Рахуй ПО КАНАЛАХ окремо; показати конверсію БЕЗ підпису каналу — ЗАБОРОНЕНО.
3) ПЕРІОД. Якщо в питанні названо МІСЯЦЬ БЕЗ РОКУ («за червень») — НЕ обчислюй дати сам: передай у get_metric month:"червень" і НЕ передавай from/to. Рік підставить код — найближче МИНУЛЕ входження цього місяця (у січні «грудень» = грудень минулого року, а не майбутній). from/to заповнюй ЛИШЕ коли рік названо явно. ⚠️ «за місяць» / «по місяцях» / «статистика за рік» — це РОЗБИВКА ПО МІСЯЦЯХ за останні 12 міс, а НЕ одна цифра поточного місяця.
...
export async function buildSystemBlocks(): Promise<Anthropic.TextBlockParam[]> {
  ...
  // 🔴 СЬОГОДНІШНЯ ДАТА — у ДИНАМІЧНОМУ блоці (не в кешованому: вона міняється щодня,
  // і в кеші ламала б префікс). Без неї правило «період за замовчуванням — поточний»
  // не працює в принципі: модель не знає, який зараз рік, і на «звіт за червень»
  // відповідала за червень МИНУЛОГО року (спіймано гейтом 31.07.2026).
  const today = new Date().toLocaleDateString("sv-SE", { timeZone: "Europe/Kyiv" });
  blocks.push({ type: "text", text: `СЬОГОДНІ: ${today} (Europe/Kyiv). «Поточний рік/місяць» рахуй від цієї дати.` });
  if (section) {
...
export const TOOLS: Anthropic.Tool[] = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        month: { type: "string", description: "Місяць, названий БЕЗ року («червень»). Передавай ЙОГО замість from/to — рік підставить код" },
        from: { type: "string", description: "YYYY-MM-DD, початок періоду (включно). Лише коли рік названо явно" },
        to: { type: "string", description: "YYYY-MM-DD, кінець періоду (включно). Лише коли рік названо явно" },
    ...
    switch (name) {
      ...
      case "get_metric": {
        const r = await getMetric(String(input.name ?? ""), {
          month: input.month == null ? null : String(input.month),
