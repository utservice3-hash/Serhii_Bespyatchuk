...
test("#400l ПРИКРІПЛЕННЯ ФАЙЛА: зі СПИСКУ і з КАРТКИ, інпут не display:none, вкладення вище стрічки", () => {
  ...
  assert.ok(posFiles > 0, "🔴 блок вкладень зник із картки (або перейменований) — гейт втратив предмет");
  assert.ok(posFeed > 0, "🔴 блок стрічки зник із картки — гейт втратив предмет");
