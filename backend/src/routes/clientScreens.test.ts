...
/**
 * 🧮 #721b (було #408, знято 24.09.2026 за ТЗ 22.09 п.2.2) — ЖОРСТКИЙ ПОДІЛ, ЗІ ЗНАМЕННИКОМ ЗА ТИМ САМИМ ПРАВИЛОМ, ЩО Й ЕКРАН.
 *
 ...
 */
test("#721b ЖОРСТКИЙ ПОДІЛ: активні + сплячі + втрачені = кваліфікована база, разових серед 2+ НУЛЬ", needsApi(), async () => {
  ...
  // 🔴 ЗМІНЕНО ЗА ТЗ ЮЛІ 22.09.2026, п.2.2: «постійний = 2+ успішні угоди, одне правило для всіх».
  // Доти тут стояло «разових > 0» (гейт #408, знятий): при двошляховому правилі разові серед 2+
  // були завжди. Тепер їх НУЛЬ за побудовою, і саме нуль доводить, що нове правило застосовано.
  assert.equal(t.oneOff, 0,
    `🔴 разових ${t.oneOff} серед клієнтів із 2+ угодами — кваліфікація досі двошляхова`);
