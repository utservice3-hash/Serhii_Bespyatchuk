...
test("#91 СЕЛЕКТОР «ОБСЯГ» НАЗИВАЄ РОЗМІР, а не лише назву", () => {
  const s = readFileSync(TABLE_TSX, "utf8");
  const sel = s.split('<span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)" }}>Обсяг:')[1]
    ?.split("</select>")[0] ?? "";
  assert.ok(sel.length > 0, "🔴 селектор «Обсяг» зник — розбір зламався");
  ...
  assert.ok(/n != null \? ` · \$\{n\}` : ""/.test(sel),
    "🔴 команді без даних домальовується число — це твердження, якого ми не робили");
  // Групи: без них список із 30+ рядків зливається в суцільну стрічку.
  for (const g of ["Команди", "Менеджери"])
    assert.ok(new RegExp(`<optgroup label="${g}">`).test(sel), `🔴 немає групи «${g}» у списку`);
