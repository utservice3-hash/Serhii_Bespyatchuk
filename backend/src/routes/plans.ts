...
plansRouter.post("/formation/submit", async (req, res) => {
   ...
   *
   * 🔴 ВОНА НЕ «СТРИМУВАЛА ВІД ЗАНИЖЕННЯ» — ВОНА НЕ ДАВАЛА ПОДАТИ ПЛАН УЗАГАЛІ.
   * Заміряно на проді 02.09.2026: з 12 активних менеджерів без вересневого подання
   * кнопка «Подати» була заблокована у 12 із 12, і в 9 із них рекомендація = 0 не
   * тому, що людина погано працює, а тому що виграних угод немає ЗА ПОБУДОВОЮ —
   * лідоген і фінанси. Лемчук Вікторія (новенька, рекомендація 3 388 ₴) не могла
   * подати нічого, і команда стояла на 8/9 подано.
   *
   * ⚠️ ПРИБРАНО РІВНО ЗАБОРОНУ. Попередження на екрані і прапорець `below_min`
   * нижче ЛИШАЮТЬСЯ: власник просив зняти обмеження, а не підказку. Без прапорця
   * ми тихо втратили б видимість винятків — і саме це стереже `#275c`.
   */
  const { planMinPerManager } = await getSettings();
  /**
   * Прапорець ставить СЕРВЕР за фактичним порогом на момент подачі — фронт на нього
   * не впливає.
   *
   * 🔴 `proposedValue > 0` — НЕ причепка. План 0 ми ставимо СВІДОМО (так прибирали
   * звільненого з перерозподілу); це стан «виключений із плану», а не «нижче
   * мінімуму». Без цієї умови писач мітив би нулі, бо `0 < 30 000` — і звіт кричав
   * би про навмисне рішення. Гейт `#4.3` цю вимогу перевіряв ЗАВЖДИ, а писач її не
   * знав: два різні правила на одне поле, які збігались лише поки нулів не подавали.
   * Заміряно на проді 21.08.2026: рядки з планом 0 є (id 6, 9), обидва НЕ помічені —
   * тобто дірка латентна, через цей шлях нулі ще не подавали. Закриваємо до того, як
   * подадуть.
   *
   * ⚠️ `is_active` сюди СВІДОМО не входить, хоч правило власника від 31.07.2026
   * згадує «лише для активних». Причина названа заміром: саме через те, що
   * активність — властивість, ЩО ЗМІНЮЄТЬСЯ В ЧАСІ, вона й не має лежати в
   * замороженому записі. Рибенко подала 20 000 (нижче мінімуму) 31.07, план
   * затвердили 03.08, а деактивували її ПІСЛЯ — і гейт, що перераховував прапорець
   * за СЬОГОДНІШНІМ `is_active`, почав звинувачувати правильний запис. Тут поле
   * означає рівно «значення було нижче мінімуму», і це твердження не протухає.
   * Умову «лише активним» застосовує ЧИТАЧ, на живих даних (див. `#4.3`).
   */
  const belowMin = planMinPerManager > 0 && proposedValue > 0 && proposedValue < planMinPerManager;
  await pool.query(
