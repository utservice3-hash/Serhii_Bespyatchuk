...
/**
 * 🔗 #402–#402c — ПРЯМЕ ПОСИЛАННЯ НА ЗАДАЧУ (`/tasks?id=3367`).
 *
...
/**
 * #402 — `?id=` ЧИТАЄТЬСЯ ЛИШЕ ЯК ДОДАТНЕ ЦІЛЕ.
 *
 ...
 */
test("#402 ?id= читається лише як додатне ціле, решта — null", async () => {
...
/**
 * #402b — СЕКЦІЯ СПРАВДІ СІДАЄ СТАН ІЗ URL І ПИШЕ ЙОГО НАЗАД, В ОБИДВА БОКИ.
 *
 ...
 */
test("#402b секція сідає з URL і синхронізує параметр в обидва боки", () => {
...
/**
 * #402c — ПОРОЖНЕЧА НАЗИВАЄ СЕБЕ, І НЕ БЛИМАЄ НА ВАЛІДНОМУ ПОСИЛАННІ.
 *
 ...
 * → червоніє друге.
 */
test("#402c недоступна задача називає себе, а валідна не блимає", async () => {
