...
test("#608 ДИМ: ручні слайди — межа керівництва, порядок, видалення скасовне", async (t) => {
  ...
  try {
    ...
    assert.equal((await call("POST", "/manual-slides", { body: { weekFrom: W, kind: "news", title: "" } })).status, 400);
    const a = await call("POST", "/manual-slides", { body: { weekFrom: W, kind: "birthday", title: "З Днем народження!", person: "Тест Іван", position: 1 } });
    assert.equal(a.status, 201, JSON.stringify(a.body));
    const b = await call("POST", "/manual-slides", { body: { weekFrom: W, kind: "newcomer", title: "Вітаємо в команді!", position: 0 } });
    ...
    const e = await call("PATCH", "/manual-slides/:id", { params: { id: String(id) }, body: { weekFrom: W, kind: "birthday", title: "З ДН!", person: "Тест Іван", position: 1 } });
    assert.equal(e.body.slides[1].title, "З ДН!");
