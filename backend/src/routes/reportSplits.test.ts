...
/**
 * #105 — КОЖНА ПОВЕРХНЯ НАЗИВАЄ СВІЙ ЗНАМЕННИК (рішення власника 21.08.2026).
 *
 * 🔴 ПЕРЕПИСАНО 24.08.2026, БО ФАКТ, ЯКИЙ ГЕЙТ СТЕРІГ, ЗМІНИВСЯ. Він вимагав, щоб
 * «Плани» звались «за передачами»: тоді там справді стояв РЕЄСТР переданих заявок,
 * а у Звіті — когорта створених угод за `lead_channel`, і за серпень це давало
 * 3.8% проти 24.8%. Коміт `564bee0` перевів знаменник «Планів» на канал, і обидві
 * поверхні почали рахувати ОДНУ множину (тримають `#141b` і `#170b`). Відтоді
 * вимога «назви це передачами» стала вимогою БРЕХАТИ.
 *
 * 🔴 І ЦЕ НЕ «ПОФАРБУВАТИ ГЕЙТ У ЗЕЛЕНЕ». Гейт червонів ПРАВИЛЬНО — він спіймав,
 * що екран називає джерелом те, чого функція вже не читає (я цього не помітив на
 * викаті `564bee0`, гейт помітив). Змінюється не суворість, а ТВЕРДЖЕННЯ: замість
 * «дві метрики мусять мати різні назви» тепер «обидві мусять називати спільну
 * основу, і жоден підпис не має права обіцяти неіснуючу другу метрику».
 *
 * ⚠️ Що НЕ зійшлось і тому лишається підписаним: `won`-предикати різні. «Плани»
 * рахують лише угоди із ЗАПИСАНОЮ подією входу в грошову зону, Звіт — ще й ті, що
 * стоять там ЗАРАЗ за статусом. Заміряно на ТІЙ САМІЙ популяції: 87 проти 59.
 * Це різні фінішні лінії, а не різні метрики — і саме так має читатись підказка.
 */
test("#105 КОЖНА ПОВЕРХНЯ НАЗИВАЄ СВІЙ ЗНАМЕННИК У ПІДПИСІ", () => {
  ...
  assert.ok(/Конверсія \(за угодами лідгену\)/.test(plans),
    "🔴 у «Планах» конверсія лідогену не названа своєю основою — знаменник там КАНАЛЬНИЙ з 24.08.2026");
  assert.equal(/Конверсія \(за передачами\)/.test(plans), false,
    "🔴 повернувся підпис «за передачами» — він називає джерелом реєстр бота, якого функція вже не читає");
  ...
  assert.ok(/угод лідгену/.test(plans) && /прийнятих реклами/.test(plans),
    "🔴 підпис під числом («N/M») не каже, ЧОГО саме — знаменник знову безіменний");

  /**
   * 🔴 ПІДКАЗКА ЗВІТУ БІЛЬШЕ НЕ МАЄ ПРАВА ОБІЦЯТИ ДРУГУ МЕТРИКУ.
   * Доки знаменники розходились, вона чесно попереджала «там 3.8%, тут 24.8%».
   * Тепер вони збіглись, і те саме речення стало брехнею — причому НЕБЕЗПЕЧНОЮ:
   * читач шукав би розбіжність, якої немає, і пояснював би нею справжню, іншу
   * (різні `won`-предикати). Тому: слово про розрив знаменників — геть, слово про
   * різні фінішні лінії — обовʼязкове.
   */
  assert.equal(/там знаменник — реєстр переданих заявок/.test(cols), false,
    "🔴 підказка Звіту досі каже, що в словниковій метриці інший знаменник — з 24.08.2026 він той самий");
  assert.ok(/фінішн|ЗАРАЗ за статусом|87 проти 59/.test(cols),
    "🔴 підказка мовчить про ЄДИНУ розбіжність, що лишилась (різні `won`-предикати, 87 проти 59) — "
    + "читач пояснюватиме її неіснуючою різницею знаменників");
});
