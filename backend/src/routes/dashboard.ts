 ...
 *
 * 🟢 УСІ КАНАЛИ, А НЕ ЛИШЕ ПОШУК — РІШЕННЯ ВЛАСНИКА 09.09.2026, І ВОНО ЗАКРИВАЄ
 * ПИТАННЯ. Екран показує кожну кампанію з витратами, включно з `Cross-network`
 * (це Performance Max). Саме через це наші суми БІЛЬШІ за очікувані в ТЗ:
 * заміряно 25.08-02.09 — `Paid Search` 49 133 ₴ проти `Cross-network` 4 269 ₴,
 * розбіжність із ТЗ по тому ж вікну 3 481 ₴. На 02.09 ТЗ дає 1 015 ₴ — це рівно
 * вартість ОДНІЄЇ кампанії з чотирьох, що працювали того дня.
 *
 * 🔴 ТОМУ НЕ «ЛАГОДИТИ» РОЗБІЖНІСТЬ ФІЛЬТРОМ ПО КАНАЛУ. Наступний, хто звірятиме
 * екран із ТЗ або з аркушем Сергія, побачить, що в нас більше, і спокуса додати
 * `WHERE channel_group = 'Paid Search'` буде природною. Це зробило б Performance
 * Max невидимим, а витрати — неповними; звузити коло можна лише новим рішенням
 * власника, і тоді разом із ним має зʼявитись підпис на екрані, що саме сховано.
 */
...
dashboardRouter.post("/client-merge", async (req, res) => {
   ...
   * лише в реєстрі лишив би другий відкритим. Стереже `#422c`.
   */
  const client = await pool.connect();
  let applied = 0;
  try {
  ...
  await pool.query(
    `INSERT INTO repeat_client_plans
     ...
     ON CONFLICT (client_key, month) DO UPDATE SET
       ...
       -- ⚠️ Зворотні лапки тут ЗАБОРОНЕНІ: коментар живе всередині шаблонного рядка JS,
       -- і перша ж лапка обриває літерал. Спіймано чистою збіркою, не оком.
