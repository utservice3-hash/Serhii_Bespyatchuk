...
/**
 * Найсвіжіший місяць, у якому Є план на ДЖЕНЕРИК-ключі, разом із переліком ключів.
 *
 * 🔴 ОКРЕМИЙ ХЕЛПЕР, А НЕ `monthWithPlans()` — І ЦЕ НЕ ДРІБНИЦЯ. Той бере останній
 * місяць із планами ВЗАГАЛІ; заміряно на проді 14.09.2026: у вересні 126 планів і
 * НУЛЬ дженериків, а єдиний за всю історію дженерик-план лежить у липні. Гейт, що
 * питає вересень, зеленів би ПОРОЖНЕЧЕЮ — найнебезпечніший стан у цьому проєкті.
 * Тому предмет шукається за ознакою предмета, а не за датою.
 */
async function monthWithGenericPlan(): Promise<{ month: string; keys: string[] } | null> {
  const { pool } = await import("../db/pool.js");
  const metrics = await import("../core/metrics.js");
  const r = await pool.query<{ m: string; keys: string[] }>(
    `SELECT to_char(month,'YYYY-MM') AS m, array_agg(DISTINCT client_key) AS keys
       FROM repeat_client_plans WHERE client_key = ANY($1)
      GROUP BY month ORDER BY month DESC LIMIT 1`, [metrics.GENERIC_CLIENT_KEYS]);
  const row = r.rows[0];
  return row ? { month: row.m, keys: row.keys } : null;
}
...
/**
 * #721 (було #109, знято 24.09.2026 за ТЗ 22.09 п.2.2) — ЛІЧИЛЬНИКИ АКТИВНИХ НЕ ЗРУШИЛИСЬ, ХОЧА РОСТЕР ВИРІС.
 *
 ...
 */
test("#721 активні лічильники рахують АКТИВНИХ, а не ростер; разових серед 2+ немає", needsApi(), async () => {
  const m = await monthWithPlans();
  assert.ok(m, "🔴 планів немає взагалі");
  const token = await adminToken();
  const b = await (await get(`/api/dashboard/client-plans?month=${m.month}`, token)).json() as Resp;
  ...
  assert.ok(t.inReactivation > 0, "🔴 місток обнулився — лічильники перерахували від іншого добору");
  // 🔴 ЗМІНЕНО ЗА ТЗ ЮЛІ 22.09.2026, п.2.2 («постійний = 2+ успішні угоди»): доти тут стояло
  // «разових > 0» (гейт #109, знятий). Тепер кожен клієнт із 2+ оплатами — постійний, тож разових
  // у цій базі НУЛЬ за побудовою. Червоніє, якщо повернеться двошляхове правило.
  assert.equal(t.oneOff, 0, `🔴 разових ${t.oneOff} серед клієнтів із 2+ угодами — повернулось старе правило кваліфікації`);
});
 ...
 */
test("#343 стан підписаний у КОЖНОМУ рядку — і в API, і в рендері", needsApi(), async () => {
  const m = await monthWithPlans();
  assert.ok(m, "🔴 планів немає взагалі");
  const token = await adminToken();
  const b = await (await get(`/api/dashboard/client-plans?month=${m.month}`, token)).json() as Resp;
 ...
 *
 * ─────────────────── ПЕРЕПИСАНО 14.09.2026, І ОСЬ ЧОМУ ───────────────────
 *
 * 🔴 СТАРА ФОРМА СТВЕРДЖУВАЛА БІЛЬШЕ, НІЖ ЗНАЛА: `u.count === COUNT(дженериків у БД)`.
 * Це рівність, а отже заява «інших причин не мати рядка НЕ БУВАЄ». Їх щонайменше
 * ЧОТИРИ, і всі в `core/clientPlansList.ts`: дженерик-ключ, менше двох оплачених
 * угод (`HAVING COUNT(*) >= 2`), деактивований основний менеджер (INNER JOIN по
 * `mm.is_active`) і АРХІВ (`WHERE NOT archivedSql(...)`).
 * Блок «не привʼязано» означений як `splitUnattached` — «плани, під якими немає рядка
 * екрана»; дженерики там ПРИВІД, а не означення.
 *
 * 📐 Гейт перестверджував УЖЕ В МОМЕНТ НАПИСАННЯ (21.08.2026) — просто тоді жоден
 * архівований клієнт не мав плану, і збіг читався як правило. Червоним його зробили
 * ДАНІ: архівація «театрорг» 10.09.2026 при останній оплаті 28.08.
 *
 * 🔴 І ДРУГА ВАДА, ГІРША ЗА ПЕРШУ: гілка `if (u.count === 0)` робила гейт нездатним
 * почервоніти на САБОТАЖІ, описаному в цій же шапці. Якщо дженерик-план єдиний
 * неприв'язаний (а в липні саме так — 1 план, 40 000 ₴), то відфільтрувавши його,
 * отримуємо `u.count === 0` → перша гілка → `u.sum === 0` → ЗЕЛЕНО. Тобто «саботаж
 * виконано» доводило падіння `#107`, а не цього гейта (правило 7).
 *
 * ✅ ТЕПЕР: твердження ВКЛЮЧЕННЯ (кожен дженерик-план місяця мусить бути у блоці), і
 * перевірка виконується БЕЗУМОВНО, на місяці, де предмет справді Є.
 *
 * 🧨 САБОТАЖ: у `core/clientPlansList.ts` зняти `AND NOT (d.client_key = ANY($1))` —
 * дженерик отримує рядок, зникає з блоку, і гейт називає його ключ поіменно.
 */
test("#111b план на дженерик-ключі видно рядком «не привʼязано», не зникає", needsApi(), async () => {
  // 🔴 ПРЕДМЕТ ШУКАЄТЬСЯ ЗА ОЗНАКОЮ ПРЕДМЕТА, А НЕ ЗА ДАТОЮ. Останній місяць із
  // планами взагалі може не мати ЖОДНОГО дженерика (замір 14.09.2026: вересень —
  // 126 планів і 0 дженериків), і тоді гейт зеленів би порожнечею.
  const g = await monthWithGenericPlan();
  assert.ok(g, "🔴 у `repeat_client_plans` немає ЖОДНОГО плану на дженерик-ключі за всю "
    + "історію — предмета цього гейта не існує ніде. Це не «зелено»: або заведи фікстуру, "
    + "або зніми живу половину в RETIRED_GATES на користь чистої `#111b splitUnattached`");
  const token = await adminToken();
  const b = await (await get(`/api/dashboard/client-plans?month=${g!.month}`, token)).json() as Resp;
  assertContract(b);
  const u = b.totals.unattached;

  assert.equal(u.canSee, true, "🔴 адмін не бачить рядка «не привʼязано» — право віддано не тій ролі");

  // Структурна цілість блоку — окремою перевіркою, а не гілкою, що вимикає головну.
  ...
  if (u.count === 0) assert.equal(u.sum, 0, "🔴 «не привʼязано» порожнє, а сума ненульова");

  // ГОЛОВНЕ — ВКЛЮЧЕННЯ, А НЕ РІВНІСТЬ. Рівність стверджувала б, що інших причин не
  // мати рядка не буває; їх щонайменше чотири (див. шапку).
  const shown = new Set(u.rows.map((r) => r.clientKey));
  const missing = g!.keys.filter((k) => !shown.has(k));
  assert.deepEqual(missing, [],
    `🔴 дженерик-план за ${g!.month} НЕ показано рядком «не привʼязано»: ${missing.join(", ")}. `
    + "Саме так гроші й зникають з екрана мовчки — план у базі є, на екрані його немає, "
    + "і жодне число не сперечається");
  console.log(`   ℹ ${g!.month}: дженериків ${g!.keys.length}, у блоці всього ${u.count} на ${Math.round(u.sum)} ₴`);
});
