 ...
 *
 * 🔴 ПОПРАВКА 14.09.2026: ГЕЙТ ЗАКРІПИВ ЗБІГ ЯК ПРАВИЛО, І ЗБІГ СКІНЧИВСЯ.
 * Він вимагав `count == скільки дженерик-планів у БД` — тобто «всі неприв'язані плани
 * є дженериками». Це було правдою рівно доти, доки дженерик-ключ лишався ЄДИНОЮ
 * причиною не мати рядка. Причина `splitUnattached` ширша й написана в його ж
 * доккоментарі: «плани, під якими немає рядка екрана»; дженерики там ПРИВІД, не означення.
 *
 * 📐 Заміряно на проді 14.09: у блоці 1 рядок — `театрорг`, а дженерик-планів у БД НУЛЬ.
 * Клієнт має 3 оплати (отже постійний), `hidden = false`, але `archived_at = 10.09 08:51`
 * при останній оплаті 28.08 — тобто `archivedSql` істинний, і екран прибрав його з ростера
 * СВІДОМО (`7685ef7`, той самий перехід на архів, що зламав `#30n`). Його план лишився й
 * чесно поїхав у «не привʼязано» — рівно так, як задумано.
 *
 * ✅ ЩО ЗМІНИЛОСЬ ТУТ. Гейт лишає СВОЄ твердження — те, що написане в його назві:
 * дженерик-план ВИДНО в блоці, він не зникає. Це підмножина, і вона істинна незалежно
 * від того, скільки ще є причин не мати рядка. Точну рівність («у блоці рівно ті плани,
 * під якими немає рядка») переніс у `#398` — окремим номером, бо це ІНШЕ твердження,
 * а назву гейта не уточнюють (правило 13).
 */
test("#111b план на дженерик-ключі видно рядком «не привʼязано», не зникає", needsApi(), async () => {
  ...
  if (u.count === 0) {
    ...
    const inDb = await pool.query<{ k: string }>(
      `SELECT client_key AS k FROM repeat_client_plans
        WHERE month = $1 AND client_key = ANY($2)`, [`${m.month}-01`, metrics.GENERIC_CLIENT_KEYS]);
    // 🪞 ПІДМНОЖИНА, А НЕ РІВНІСТЬ: кожен дженерик-план МУСИТЬ бути в блоці. Рівність
    // стверджувала б ще й «інших причин не буває» — а вони є (архів), і саме на цьому
    // гейт червонів чотири доби, не спіймавши жодного дефекту.
    const inBlock = new Set(u.rows.map((r) => r.clientKey));
    const missing = inDb.rows.map((r) => r.k).filter((k) => !inBlock.has(k));
    assert.deepEqual(missing, [],
      "🔴 дженерик-план є в БД, але його НЕМАЄ в блоці «не привʼязано» — гроші зникли з екрана, "
      + "і жодне число про це не сперечається (саме та поломка, заради якої блок і заведено)");
    console.log(`   ℹ не привʼязано: ${u.count} на ${Math.round(u.sum)} ₴ · із них дженериків ${inDb.rows.length}`);
  }
});

/**
 * 🕳 #398 — У БЛОЦІ «НЕ ПРИВʼЯЗАНО» РІВНО ТІ ПЛАНИ, ПІД ЯКИМИ НЕМАЄ РЯДКА.
 *
 * 🔴 ЧОМУ ОКРЕМИМ НОМЕРОМ, А НЕ ВСЕРЕДИНІ `#111b`. Той стереже СВОЄ: «дженерик-план
 * видно, він не зникає». Це підмножина. Точна рівність — інше твердження, і вона
 * сильніша: ловить втрату в ОБИДВА боки (план без рядка, який не показали; і рядок у
 * блоці, під яким насправді є клієнт). Уточнити назву старого гейта було не можна —
 * реєстр звіряє імʼя, і «те саме імʼя про інше» це тихо підмінений гейт (правило 13).
 *
 * 🔴 І ЧОМУ ЦЕ НЕ ТАВТОЛОГІЯ. Ліворуч — відповідь роуту (`clients` + `unattached.rows`),
 * праворуч — плани ПРЯМО З БД за той самий місяць. Роут не бере участі в обчисленні
 * правої частини, тож рівність доводить саме те, що жоден план не загубився між базою
 * й екраном — байдуже, з якої причини клієнт випав із ростера (дженерик, архів, або
 * наступна, якої ще немає).
 */
test("#398 у блоці «не привʼязано» — РІВНО ті плани місяця, під якими немає рядка", needsApi(), async () => {
  const m = await monthWithPlans();
  assert.ok(m, "🔴 планів немає взагалі");
  const token = await adminToken();
  const b = await (await get(`/api/dashboard/client-plans?month=${m.month}`, token)).json() as Resp;
  assertContract(b);
  assert.equal(b.totals.unattached.canSee, true, "🔴 адмін не бачить блока — порівнювати нема з чим");

  const { pool } = await import("../db/pool.js");
  const all = await pool.query<{ k: string }>(
    `SELECT client_key AS k FROM repeat_client_plans WHERE month = $1`, [`${m.month}-01`]);
  assert.ok(all.rows.length > 0, `🔴 планів за ${m.month} у БД немає — перевірка була б порожньою`);

  const roster = new Set(b.clients.map((c) => c.clientKey));
  const expected = all.rows.map((r) => r.k).filter((k) => !roster.has(k)).sort();
  const actual = b.totals.unattached.rows.map((r) => r.clientKey).sort();

  assert.deepEqual(actual, expected,
    `🔴 блок «не привʼязано» розійшовся з базою: у блоці ${actual.length}, а планів без рядка ${expected.length}. `
    + "Зайве тут означає рядок, під яким клієнт насправді Є; брак — план, що зник з екрана мовчки");
  console.log(`   ℹ планів за ${m.month}: ${all.rows.length} · без рядка: ${expected.length}`);
});
