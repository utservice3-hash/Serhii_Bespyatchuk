...
import { needsApi, API_BASE, emptyPeriodSkip, smallSampleSkip } from "../testMode.js";
 ...
 */
test("#58b живі дані: чек == гроші÷угоди і ≠ гроші÷авто", needsApi(), async (t) => {
  ...
  /** 🈳 Ні в кого немає грошей ② — початок місяця, законна відсутність. */
  const noMoney = emptyPeriodSkip("менеджерів із грішми ②", checked, `${from}..${to}`);
  if (noMoney) return t.skip(noMoney);
  /**
   * 📉 ВИБІРКА НЕ РОЗРІЗНЯЄ ДВА ЗНАМЕННИКИ — і це стан ДАНИХ, а не дефект.
   *
   * Цей гейт — ЖИВЕ ДЗЕРКАЛО до `#58`: він доводить, що розрізнення «угоди ② проти
   * авто» не академічне, бо в реальних даних вони таки різні. Коли в усіх перевірених
   * угоди == авто, підміна знаменника невидима, і твердити нічого.
   *
   * 📐 ПОРІГ = 1, І ЦЕ ЗАМІРЯНО, а не вгадано (01.09.2026, ті самі 6 менеджерів):
   *   повний серпень — із грішми 5, розбіжних **4**;
   *   вересень на 2-й день — із грішми 1, розбіжних **0**.
   * Тобто одного розбіжного досить, щоб підміна стала видимою, а повний місяць дає
   * їх чотири. Просити більше — вигадувати вимогу до бізнесу; просити нуль —
   * лишити перевірку порожньою.
   *
   * ⚠️ І межа родини: сам ВИРОК про формулу тримає `#58`, який читає джерело екрана
   * і не скіпається НІКОЛИ. Тобто пропуск тут не лишає правило без охорони.
   */
  const unfit = smallSampleSkip("менеджерів, у яких угоди ② ≠ авто", divergent, 1,
    `${from}..${to}, перевірено ${checked}`);
  if (unfit) return t.skip(unfit);
});
