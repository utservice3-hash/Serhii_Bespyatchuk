...
test("#196 вік боргу рахується ПІСЛЯ готівки — інакше готівкові лишаються без віку", async () => {
  ...
  //
  // ⚠️ ВИРАЗ ПЕРЕЇХАВ У ЯДРО 26.08.2026, І ЦЕ НЕ ПОСЛАБЛЕННЯ ГЕЙТА. Раніше
  // `MAX((CURRENT_DATE - invoice_date))` стояв просто тут, у синку; тепер він
  // у `core/receivablesAge.CLIENT_DEBT_AGE_SQL` — бо виробників віку виявилось
  // ДВА (другий у шапці розкриття), і вони розходились: 1128 днів у рядку
  // проти 22 у шапці під ним. Тверджeння те саме — «вік із дат рахунків», —
  // але адреса нова, тож перевіряємо ОБИДВІ ланки: синк бере вираз із ядра,
  // ядро виробляє його з `invoice_date`.
  assert.match(src, /\$\{CLIENT_DEBT_AGE_SQL\}/,
    "🔴 синк більше не бере вік із ядра — найімовірніше завів власний вираз, і він розійдеться з екраном");
  const ageCore = readFileSync(SRC("core/receivablesAge.ts"), "utf8");
  assert.match(ageCore, /CURRENT_DATE - ri\.invoice_date/,
...
test("#180 ліміт у синку береться З ТАБЛИЦІ, а гугл-аркуш більше не джерело", () => {
  ...
  // ⚠️ З 26.08.2026 вираз живе в `core/receivablesAge` — див. пояснення в `#196`.
  assert.match(src, /\$\{CLIENT_DEBT_AGE_SQL\}/,
    "🔴 вік боргу більше не береться з ядра — він знову звідкись приходить");
});
