...
test("#24i чужий ЄДРПОУ не відмічає рахунок, навіть коли номер збігся", () => {
  ...
  /**
   * 🔴 `none`, А НЕ `ambiguous`, І ЦЕ НЕ ДРІБНИЦЯ ПРО СЛОВО. Спіймано
   * ПРИЙМАННЯМ на проді 28.08.2026: рядок SOKOLOV SPÓŁKA (польська фірма,
   * у полі ЄДРПОУ 9-значний NIP) отримав підпис «платіж називає кілька
   * рахунків», хоч платіж називав РІВНО ОДИН. Обидва стани означають «не
   * відмічаємо», але `ambiguous` СТВЕРДЖУЄ причину — і стверджував неправду.
   */
  assert.equal(matchPayment(i, [stranger], openMap(i), "2026-08-28").kind, "none",
    "🔴 гроші чужої фірми або зарахувались, або дістали ЧУЖУ причину");
  ...
  assert.equal(matchPayment(inv("6200", 1000, null), [pay({ purpose: "зг. рах. № 6200" })],
    openMap(inv("6200", 1000, null)), "2026-08-28").kind, "none");
  assert.equal(matchPayment(i, [pay({ purpose: "зг. рах. № 6200", payerEdrpou: null })],
    openMap(i), "2026-08-28").kind, "none");
...
test("#24l порожнеча НАЗИВАЄ СЕБЕ, і три стани не зливаються в один підпис", async () => {
  ...
  // 🔴 `ambiguous` МУСИТЬ ОЗНАЧАТИ РІВНО ОДНЕ. Поки під ним жили ще й
  // «чужий ЄДРПОУ» та «невідомий платник», екран стверджував причину, якої не
  // було (спіймано прийманням: SOKOLOV SPÓŁKA, платіж називав ОДИН рахунок).
  const onlyCause = matchPayment(inv("9001"), [pay({ purpose: "зг. рах. № 9001, № 9002" })],
    new Map([["9001", inv("9001")], ["9002", inv("9002")]]), "2026-08-28");
  assert.equal(onlyCause.kind, "ambiguous", "🔴 справжня неоднозначність перестала бути ambiguous");
