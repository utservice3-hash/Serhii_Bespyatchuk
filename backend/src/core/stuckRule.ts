 ...
 *
 * Тому 14 було СТАРТОВИМ значенням для живого заміру, а не рішенням.
 *
 * 🟢 ЗАМІР ЗРОБЛЕНО, ПОРІГ ОБРАНО: **21** (рішення власника 17.08.2026). Знято проти
 * ЖИВОГО списку — на 21 дні він дає **140 угод**. Тобто число прийшло з даних, а не
 * з попереднього заміру іншої моделі, і саме тому воно тут стоїть.
 *
 * ⚠️ ПАРАМЕТРОМ ВОНО ЛИШАЄТЬСЯ. `?minDays=` живий, обидва роути беруть дефолт звідси,
 * підпис на екрані бере число з відповіді сервера. Перезамір під інший поріг — і далі
 * три запити, без збірки й без правки коду; змінити рішення = змінити цей рядок.
 */
export const STUCK_MIN_DAYS = 21;
