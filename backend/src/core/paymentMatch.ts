...
export function matchPayment(
  ...
  if (named.length) {
     ...
     * рахунком.
     *
     * 🔴 І ВІДПОВІДЬ ТУТ — `none`, А НЕ `ambiguous`, ХОЧ ОБИДВА ОЗНАЧАЮТЬ «НЕ
     * ВІДМІЧАЄМО». Спіймано ПРИЙМАННЯМ на живому проді 28.08.2026: рядок
     * SOKOLOV SPÓŁKA (польська фірма, у полі ЄДРПОУ — 9-значний NIP) дістав
     * підпис «платіж називає кілька рахунків», хоч платіж називав РІВНО ОДИН.
     * Підпис був правдивим реченням, поставленим не до тієї величини — рівно
     * той клас, що «архів ≠ давно втрачений» і два «очікуємо».
     *
     * `ambiguous` тепер означає РІВНО ОДНЕ: платіж посилається на кілька живих
     * рахунків. Усе інше, чого ми не підтвердили, — це «не зіставлено», і воно
     * теж називає себе, просто не вигадує причини, якої не було.
     */
