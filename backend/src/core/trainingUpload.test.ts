...
/**
 * 📎 ЗАВАНТАЖЕННЯ В НАВЧАННЯ (23.09.2026) — гейти `#714`–`#716`.
...
/**
 * #714 — БІЛИЙ СПИСОК І МЕЖА, ПО ОБИДВА БОКИ. Фото й відео проходять (те, заради чого прохід),
 ...
 */
test("#714 ЗАВАНТАЖЕННЯ: фото й відео можна, виконуване й svg — ні, межа розміру тримає", () => {
...
/**
 * #715 — КРОК ДОДАЄТЬСЯ В КУРСІ, а не лише в «Бібліотеці», і йде ТИМ САМИМ роутом. Доти шлях
 ...
 */
test("#715 КУРС: крок додається прямо в темі, наявним роутом, із прев'ю до збереження", () => {
...
/**
 * #716 — МЕЖА РОЗМІРУ ІСНУЄ В ОДНІЙ КОПІЇ. Було три: роут, форма бібліотеки і (з цим проходом)
 ...
 */
test("#716 МЕЖА ЗАВАНТАЖЕННЯ: одне джерело — ядро; фронт бере число з відповіді, а не своє", () => {
