...
test("#32 UPSERT оверрайду: передані поля — пишуться, НЕпередані — лишаються", async (t) => {
  ...
  try {
    ...
    // (2) І ТЕПЕР — саме те, що робить нова кнопка в картці клієнта.
    row = await run({ clientKey: "к", hidden: true });
    assert.equal(row.hidden, true, "🔴 «прибрати з постійних» не спрацювало");
    assert.equal(row.pinned_manager_id, 10,
      "🔴 РЕГРЕС: «прибрати з постійних» ЗНЯЛО закріпленого менеджера — саме та тиха втрата, "
      + "заради якої правило й існує");
    assert.equal(row.note, "передано КВП", "🔴 коментар затерто запитом, який його не передавав");

    // (3) ДЗЕРКАЛО №1: повернути назад — теж лише одним полем.
    row = await run({ clientKey: "к", hidden: false });
    assert.equal(row.hidden, false, "🔴 повернення до постійних не спрацювало — дія не зворотна");
    assert.equal(row.pinned_manager_id, 10, "🔴 повернення зняло закріплення");
