...
async function seed(c: import("pg").Client): Promise<void> {
  ...
  for (const [key, name, mgr, ago, price] of clients) {
    for (const [i, when] of [ago + 400, ago + 200, ago].entries()) {
      /**
       * 🕛 ДАТА ОПЛАТИ СІЄТЬСЯ ВІД ТОГО САМОГО «СЬОГОДНІ», ЩО Й РАХУЄ SQL, а не від
       * `Date.now()`. Інакше межі 59/60/179/180 — нічна бомба: `days_since` рахується
       * як `CURRENT_DATE − (closed_at ${"AT TIME ZONE 'Europe/Kyiv'"})::date`, тобто
       * ДВА РІЗНІ «сьогодні» (сесія БД у GMT, кінець — київський). З 00:00 до 03:00 за
       * Києвом вони розходяться на добу, і `межа60` тихо стає активною.
       * 📐 Спіймано о 00:04 за Києвом 28.08.2026: файл окремо давав 8, повний прогін — 7.
       * ⚠️ Розходження ЖИВЕ Й У ПРОДІ (`current_setting('TimeZone') = 'GMT'`, дрейф
       * 5815 із 5815 клієнтів у цю мить) — але це ІНША задача: `clientSegments.days_since`
       * стоїть під станами обох екранів, і його правка змінює склад списків.
       */
      await c.query(
        `INSERT INTO deals (kommo_id,name,manager_id,pipeline_id,status_id,price,client_key,client_key_raw,client_name,closed_at_kommo)
         VALUES ($1,'d',$2,8921932,142,$3,$4,$4,$5,(CURRENT_DATE - $6::int) + interval '12 hours')`,
        [id++, mgr, i === 2 ? price : 1, key, name, when]);
    }
  }
...
test("#25 clientStates ВИКОНУЄТЬСЯ проти БД і дає стани з ДАТ", async (t) => {
  ...
  try {
    ...
    /**
     * 🔴 КОНТРАКТ ЗМІНИВСЯ 27.08.2026, І ЦЕ РІШЕННЯ, А НЕ РЕГРЕС. `clientStates` —
     * джерело ВКЛАДКИ «Реактивація», а не «стани всіх клієнтів»: воно й раніше різало
     * разових і телефонні дженерики. Бракувало ТРЕТЬОЇ умови — стану. До цього дня
     * 134 клієнти, які замовляють прямо зараз, висіли у вкладці (21% складу): «вихід»
     * не був зламаний, його не існувало.
     * Із десяти засіяних активні ДВОЄ — `активний` (10 днів) і `межа59` (59 днів).
     */
    assert.equal(rows.length, 8,
      `8 = 10 засіяних мінус двоє активних (активний, межа59); прийшли: ${[...by.keys()].join(", ")}`);
    assert.equal(by.has("активний"), false,
      "🔴 клієнт, який замовив 10 днів тому, знову у вкладці «Реактивація» — виходу немає");
    assert.equal(by.get("сплячий")?.state, "sleeping");
    assert.equal(by.get("втрачений")?.state, "lost");
    /**
     * Межа 59/60 — на ЖИВОМУ запиті, а не лише в чистій функції: між ними стоїть
     * обчислення `days_since` у SQL по-київськи, і воно теж може поїхати. Читається
     * вона тепер ЧЕРЕЗ виключність: 59 — ще активний, отже його тут НЕМАЄ; 60 — уже
     * сплячий, отже він тут Є. Це та сама межа, просто з іншого боку.
     */
    assert.equal(by.has("межа59"), false, "59 днів — ще активний, у реактивації йому не місце");
    ...
    /**
     * 🔴 ДЗЕРКАЛО ВІДСУТНОСТІ. Без нього «немає у списку» доводило б лише те, що їх
     * немає — тобто зеленіло б і при зламаному запиті, і при фільтрі, що ріже всіх.
     * Тому окремо: обидва зниклі КВАЛІФІКОВАНІ (тобто випали саме за станом), і
     * стан у джерелі сегментів каже `active`.
     */
    const { loadClientSegments, factsFor } = await import("./clientSegments.js");
    const seg = await loadClientSegments();
    for (const k of ["активний", "межа59"]) {
      const f = factsFor(seg, k);
      assert.equal(f.qualified, true, `🔴 ${k} випав як РАЗОВИЙ, а не за станом — це інша поломка`);
      assert.equal(f.state, "active", `🔴 ${k} не активний, отже вкладка втратила його з іншої причини`);
    }

    assert.equal(by.get("сезонний")?.seasonal, true);
    assert.equal(by.get("сезонний")?.seasonalNote, "зерно");
    assert.equal(by.get("сплячий")?.seasonal, false, "позначка не має протікати на інших");
    ...
    assert.equal(by.get("сплячий")!.managerName, "Менеджер А");
    ...
    await t.test("#25b СКОУП: менеджер бачить своїх, команда — свою", async () => {
      ...
      assert.equal(mine.length, 7, "у Менеджера А — семеро (девʼять мінус двоє активних)");
      ...
      assert.equal((await R.clientStates({})).length, 8, "🔴 без скоупу мають бути ВСІ, кого вкладка бере");
    ...
    await t.test("#25c ЗАДАЧІ ОБОХ ТИПІВ і «повернено» — на живому запиті", async () => {
      ...
      // Ознака «повернувся» лишається ЖИВОЮ в рядку: задача 100 дн. тому, оплата 70 —
      // тобто оплата ПІСЛЯ задачі, а клієнт усе ще сплячий, отже його видно.
      assert.equal(by.get("сплячий")?.returned, true, "оплата після задачі → повернений");
      assert.equal(by.get("сплячий")?.returnedRevenue, 9000);
      assert.equal(by.get("втрачений")?.taskId, null, "клієнт без задачі не має її підхопити");

      /**
       * 🔴 ПОВЕРНЕНИЙ ВИХОДИТЬ ІЗ ВКЛАДКИ — ВИДИМА ЗМІНА ПРОДУКТУ (27.08.2026), а не
       * побічний ефект. «пачковий» оплатив 5 днів тому, отже він знову АКТИВНИЙ, а
       * вкладка від цього дня показує лише сплячих і втрачених (жорсткий поділ
       * екранів, рішення власника 05.08.2026). Його успіх НЕ зникає — він нижче, у
       * плитці «Повернено за 30 дн.», і саме тому тут дві половини, а не одна.
       * 📐 Ціна на живому проді названа поіменно: дві перервані задачі — «оптаріон»
       * (1575, `in_progress`) і ТОВ «ТРІНІТІ РОБОТІКС» (1370, `ready_for_approval`).
       */
      assert.equal(by.has("пачковий"), false,
        "🔴 клієнт, який щойно оплатив, лишився у вкладці реактивації — виходу немає");
    ...
    await t.test("#25d КОНТАКТ = РОЗМОВА, спроби окремо; стан лишається від ОПЛАТИ", async () => {
      ...
      // 🕛 Дати дзвінків — від «сьогодні» БАЗИ, з тієї самої причини, що й оплати в
      // `seed()`: `last_talk_days` теж рахується через `CURRENT_DATE`, тож `Date.now()`
      // давав би −1 добу з 00:00 до 03:00 за Києвом.
      const D = (n: string) => `(CURRENT_DATE - ${n}::int) + interval '12 hours'`;
      await c.query(
        `INSERT INTO ringostat_calls (uniqueid, calldate, call_type, disposition, billsec, duration, client_key)
         VALUES ('c1',${D("$1")},'out','NO ANSWER',0,12,'сплячий'),
                ('c2',${D("$2")},'in','ANSWERED',95,120,'сплячий'),
                ('c3',${D("$3")},'out','NO ANSWER',0,8,'втрачений'),
                ('c4',${D("$4")},'out','NO ANSWER',0,10,'сплячий'),
                ('c5',${D("$5")},'out','ANSWERED',31,40,'межа60'),
                ...
                ('c6',${D("$6")},'out','NO ANSWER',0,7,'межа60')`,
        [3, 40, 2, 5, 200, 210]);
      ...
      const noCalls = by.get("межа179")!;
      assert.equal(noCalls.lastTalk, null, "розмов немає → null, а не вигадана дата");
      assert.equal(noCalls.attempts, 0);
      // 7 = 10 засіяних − 2 активні (активний, межа59) − «пачковий», який став
      // активним після оплати в #25c. Число виведене, а не підігнане.
      assert.equal(rows.length, 7, "🔴 хтось зник зі списку через LEFT JOIN дзвінків");
    ...
    await t.test("#25f СЕГМЕНТИ: поріг залежить від частоти, <3 оплат — НЕ вгадуємо", async () => {
      ...
      /**
       * 🔴 АКТИВНИХ ЧИТАЄМО З ДЖЕРЕЛА СЕГМЕНТІВ, А НЕ ЗІ СПИСКУ ВКЛАДКИ (27.08.2026).
       * `clientStates` — вітрина реактивації, і активних вона більше не показує;
       * питання «який у нього сегмент і поріг» живе в `loadClientSegments`. Читати
       * його через вкладку означало б знову плутати сегмент зі станом — рівно те,
       * від чого застерігає правило «кваліфікація ≠ сегмент ≠ стан».
       */
      const { loadClientSegments: lcs, factsFor: ff } = await import("./clientSegments.js");
      const segAll = await lcs();
      assert.equal(ff(segAll, "регулярний").segment, "regular");
      assert.equal(ff(segAll, "регулярний").state, "active",
        "🔴 регулярний на 20-й день не має бути сплячим (його поріг 30)");
      assert.equal(ff(segAll, "епізодичний").segment, "episodic");
      assert.equal(ff(segAll, "епізодичний").state, "active",
        "🔴 епізодичний на 40-й день не має бути сплячим (його поріг 60)");
      // 🪞 І дзеркало до самої виключності: обидва кваліфіковані, отже з вкладки
      // вони випали САМЕ за станом, а не тому, що правило їх узагалі не бачить.
      for (const k of ["регулярний", "епізодичний"]) {
        assert.equal(ff(segAll, k).qualified, true, `🔴 ${k} випав як разовий, а не за станом`);
        assert.equal(by.has(k), false, `🔴 активний ${k} лишився у вкладці реактивації`);
      }
      ...
      // Оплата 5 днів тому → активний, тож у вкладці його немає; питання ж тут про
      // СЕГМЕНТ, і воно читається з джерела сегментів.
      const two = ff(await lcs(), "дваблизько");
      assert.equal(two.qualified, true, "🔴 клієнт із 2 оплатами за 3 дні мав кваліфікуватись");
    ...
    await t.test("#718 ПОСТІЙНИЙ = 2+ УСПІШНІ УГОДИ: готівка й без ритму — постійні, одна оплата — разова; сегмент окремо", async () => {
      // 🔴 ПРАВИЛО ЗМІНЕНО ЗА ТЗ ЮЛІ 22.09.2026 (п.2.2): «постійний = 2+ успішні угоди, одне
      // правило для всіх». Доти тут стояв #25g, що вимагав протилежного для готівки й для двох
      // оплат без ритму; він знятий (RETIRED_GATES), бо змінилось ТВЕРДЖЕННЯ (правило 13).
      //  (1) 2 оплати безналом — постійний, сегмент `unknown` (медіана по одному інтервалу).
      //  (2) 6 оплат раз на 2 місяці — Епізодичний, поріг 60.
      //  (3) 2 оплати ГОТІВКОЮ і 2 оплати через 200 днів — тепер ТЕЖ постійні.
      // 🪞 Дзеркало: ОДНА оплата — разова. Без нього гейт зеленів би й тоді, коли постійними
      // стали геть усі.
      const R2 = await import("./reactivationRules.js");
      ...
      await mk(650, "однаоплата", [], 5, "Безнал с НДС");                  // 1 оплата — дзеркало

      /**
       * 🔴 ЧИТАЄМО КВАЛІФІКАЦІЮ ТАМ, ДЕ ВОНА ЖИВЕ (переписано 27.08.2026). Раніше
       * питання «кваліфікувався?» ставилось СПИСКУ ВКЛАДКИ, і дзеркало звучало як
       * «хто не проходить — того на екрані немає». Відколи вкладка не показує
       * активних, ця форма стала БРЕХЛИВОЮ: усі шестеро тут оплатили 5-10 днів
       * тому, тобто відсутність у списку більше нічого не каже про кваліфікацію.
       * Той самий клас, що чипи «новий/постійний»: перевірка збігалась із сусідньою
       * копією, а не з правилом.
       */
      const { loadClientSegments: lcs, factsFor: ff } = await import("./clientSegments.js");
      const seg = await lcs();

      const two = ff(seg, "двабезнал");
      assert.equal(two.qualified, true,
        "🔴 безнал із 2 оплатами за 20 днів МАЄ кваліфікуватись — правило власника");
      assert.equal(two.segment, "unknown",
      ...
      const six = ff(seg, "разнадва");
      assert.equal(six.qualified, true, "🔴 6 оплат раз на 2 місяці — це точно НЕ разовий");
      ...
      // Одне правило для всіх: форма оплати й ритм більше не вирішують.
      assert.equal(ff(seg, "дваготівка").qualified, true,
        "🔴 готівка з 2 оплатами лишилась разовою — повернулось старе правило «готівці 3+»");
      assert.equal(ff(seg, "двабезналдалеко").qualified, true,
        "🔴 2 оплати через 200 днів лишились разовими — повернулась вимога ритму 30 днів");
      assert.equal(ff(seg, "змішаний").qualified, true, "🔴 1 готівка + 1 безнал — це 2 угоди, постійний");
      // 🪞 ДЗЕРКАЛО: одна оплата — разова, і це видно прямо.
      assert.equal(ff(seg, "однаоплата").qualified, false,
        "🔴 клієнт з ОДНІЄЮ оплатою став постійним — правило пускає всіх підряд");

      // І ОКРЕМО — межа вкладки: жоден із них у ній не зʼявляється, бо всі активні.
      // Дві різні відповіді на два різні питання, і вони більше не підмінюють одна одну.
      const inTab = new Set((await R.clientStates({})).map((r) => r.clientKey));
      for (const k of ["двабезнал", "разнадва", "змішаний"]) {
        assert.equal(inTab.has(k), false, `🔴 кваліфікований, але АКТИВНИЙ ${k} — у реактивації`);
      }
    });
    ...
    await t.test("#40 ⭐ ВРУЧНУ ПОСТІЙНИЙ: правило пускає, скидання повертає, БД вимагає примітку", async () => {
      ...
      assert.equal(qualifiesAsRepeat({ payments: 1, forcedRegular: true }), true,
    ...
    await t.test("#38 АРХІВ: одна дія з ПРИЧИНОЮ прибирає, нова оплата повертає САМА", async () => {
      ...
      await c.query(
        `INSERT INTO loyalty_overrides (client_key, archived_at, archive_reason)
         VALUES ('сплячий', now() - interval '65 days', 'closed_down')
         ON CONFLICT (client_key) DO UPDATE SET archived_at = now() - interval '65 days',
                                                archive_reason = 'closed_down'`);
      ...
      /**
       * 🔴 ДАТИ ТУТ НЕ ДОВІЛЬНІ (переписано 27.08.2026). Архів — 65 днів тому, тобто
       * ПІСЛЯ останньої оплати клієнта (70 днів), інакше він і не був би архівним.
       * Нова оплата — 62 дні тому: вона ПІСЛЯ архівації (отже повертає) і водночас
       * лишає клієнта СПЛЯЧИМ (поріг епізодичного 60), отже повернення видно у
       * вкладці. Оплата «щойно» зробила б його активним, і автоповернення
       * підтвердити було б нічим — не тому, що воно зламане, а тому, що вкладка
       * активних не показує.
       */
      await c.query(
        `INSERT INTO deals (kommo_id,name,manager_id,pipeline_id,status_id,price,client_key,client_key_raw,client_name,closed_at_kommo)
         VALUES (990,'d',10,8921932,142,4242,'сплячий','сплячий','ТОВ Сплячий', now() - interval '62 days')`);
...
/**
 * #334 — ПОЗНАЧКА АВТОЗАКРИТТЯ НЕ Є ЛЮДСЬКОЮ ПРИЧИНОЮ.
 *
 * Дзеркальна пара, і обидві половини потрібні: ключ мусить бути НЕПОРОЖНІМ (інакше
 * `CHECK tasks_reactivation_close_reason` не дасть закрити задачу, і автозакриття
 * тихо не спрацює) І мусить бути ПОЗА довідником (інакше менеджер зможе поставити
...
test("#334 позначка автозакриття непорожня і поза довідником причин", async () => {
  ...
  assert.ok(!CLOSE_REASON_KEYS.includes(RETURNED_CLOSE_REASON),
    "🔴 позначка потрапила в довідник — менеджер зможе поставити «повернувся» руками");
  assert.ok(!CLOSE_REASON_KEYS.some((k) => RETURNED_CLOSE_REASON.startsWith(`${k}:`)),
    "🔴 позначка починається з людського ключа — у звітах вона зіллється з ручним закриттям");
});

/**
 * #347 — ПАЧОК СТАРОГО ФОРМАТУ В БАЗІ НЕ ІСНУЄ.
 *
 * 🔴 ПАРНЕ ТВЕРДЖЕННЯ ДО ПРИБРАНОЇ ГІЛКИ (правило 8). `reactivationTasksByClient`
 * читала клієнтів ще й із `checklist_json` старих пачок. Гілку прибрано 05.09.2026,
 * коли всі 37 пачок (347 клієнтів) перенесли в рядки-діти. Але «прибрали читання»
 * саме по собі — це перевірка на відсутність, яка ВИНАГОРОДЖУЄ зникнення: якщо
 * старий формат колись повернеться, його клієнти зникнуть із карток МОВЧКИ,
 * і жодне число на екрані про це не скаже.
 *
 * ✅ Тому стежимо за причиною, а не за наслідком: пачка з чеклістом і БЕЗ рядків-дітей
 * не має існувати. Зʼявилась — значить хтось пише старим форматом, і читання його
 * не бачить. 📐 Заміряно в момент введення гейта: таких пачок НУЛЬ із 37.
 *
 * ⚠️ Гейт стереже саме НЕПЕРЕНЕСЕНІ. Пачки з дітьми лишаються в базі разом зі своїми
 * чеклістами — це слід того, з чого зроблено перенесення, і його ми свідомо не стирали.
 */
test("#347 пачок реактивації без рядків-дітей у базі немає", needsDb(), async () => {
  const { pool } = await import("../db/pool.js");
  const r = await pool.query<{ n: string; total: string }>(
    `SELECT count(*) FILTER (WHERE NOT EXISTS (SELECT 1 FROM tasks c WHERE c.parent_id = t.id)) AS n,
            count(*) AS total
       FROM tasks t
      WHERE t.task_type = 'reactivation' AND jsonb_typeof(t.checklist_json) = 'array'`);
  const { n, total } = r.rows[0];
  assert.ok(Number(total) > 0,
    "🔴 пачок немає ЗОВСІМ — гейту нема що стерегти, і зелене тут нічого не означає");
  assert.equal(Number(n), 0,
    `🔴 ${n} із ${total} пачок не мають рядків-дітей: їхні клієнти НЕ видно ніде — `
    + "читання по чеклісту прибрано 05.09.2026, і повернення старого формату зникає мовчки");
});
