...
test("#274c ЗНАМЕННИК ДВОБІЧНИЙ: «завершує» поза ним, активний — у ньому", () => {
  ...
  /**
   * 🔴 ДВІ БЕЗЗУБІ ФОРМИ, ЧЕРЕЗ ЯКІ ЦЕЙ ГЕЙТ ПРОЙШОВ, ПЕРШ НІЖ СТАТИ ЦИМ.
   *
   * ① «вираз містить слово finishing» — спіймано власним саботажем: `WHEN false THEN
   *    'finishing'` лишає слово на місці, а стан читати перестає.
   * ② «гілок, що читають `mws.state`, рівно 2» — прибито до ЧИСЛА, яке може законно
   *    зрости (напр. зʼявиться стан «відпустка»), тобто гейт червонів би на правильному
   *    коді. Це той самий клас, що крихкий зріз за довжиною.
   *
   * Форма нижче — твердження ПРО СТАНИ ПОІМЕННО: у кожного з них мусить бути власна
   * гілка, УМОВА якої читає таблицю стану. Третя законна гілка його не зачіпає, а
   * зникнення будь-якого стану — червонить.
   */
  const branches = [...stateSql("m").matchAll(/WHEN\s+([\s\S]*?)\s+THEN\s+'(\w+)'/g)]
    .map((m) => ({ cond: m[1], state: m[2] }));
  for (const st of ["dismissed", "finishing"] as const) {
    const own = branches.filter((b) => b.state === st && /mws\.state/.test(b.cond));
    assert.equal(own.length, 1,
      `🔴 у стану «${st}» немає власної гілки, УМОВА якої читає таблицю стану ` +
      `(знайдено ${own.length}). Стан, якого не читають, не існує — хоч би що стояло в THEN`);
  }
  assert.ok(branches.some((b) => b.state === "dismissed" && !/mws\.state/.test(b.cond)),
    "🔴 зникла гілка «звільнений за ознакою CRM» — людина, прибрана з Kommo, лишиться активною");
