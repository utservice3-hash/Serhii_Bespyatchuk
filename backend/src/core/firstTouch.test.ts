...
/**
 * 🎯 #475–#478 — ТЗ-3 «ЦІНУ НАЗВАНО В ПЕРШИЙ ДОТИК» у живому Звіті (17.09.2026).
 * Рішення власника: дотик — тому, хто ДЗВОНИВ; чесні стани («не вимірюється», «без запису» поза
 * відсотком); доступ як у решти Звіту.
 */

test("#475 ПЕРШИЙ ДОТИК · ПРАВИЛА: ключ імені, три стани, відсоток лише з оцінених", () => {
...
/**
 * #476 — ЖИВИЙ SQL: звʼязка з тим, хто ДЗВОНИВ, а не з угодою; «немає запису» поза знаменником;
 * межі періоду з обох боків; покриття команд; ключ імені в SQL == ключ у JS.
 */
test("#476 ПЕРШИЙ ДОТИК · ЖИВИЙ SQL: тому, хто дзвонив; без запису окремо; межі; покриття", async (t) => {
...
/**
 * #478 — ЖИВА БД: Σ по менеджерах + незвʼязані == усі рядки періоду (останні 90 днів), і маршрут
 * справді кладе клітинку й суму команди у відповідь Звіту.
 */
test("#478 ПЕРШИЙ ДОТИК · ЖИВА БД: Σ менеджерів + незвʼязані == рядки періоду", needsBackendEnv(), async (t) => {
...
test("#478b ПРОВОДКА: Звіт кладе клітинку менеджера, суму команди й метадані джерела", () => {
