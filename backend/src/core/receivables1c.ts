...
/**
 * 🕰 ЩО СИНК МІГ ЗАБРАТИ — рішення, ЯКІ рядки стрічки зобовʼязані бути в базі.
 *
 * 🔴 ПИТАННЯ ЗМІНЕНО, А НЕ ПОРІГ. Раніше `#124` звіряв два зрізи, зняті в РІЗНІ
 * моменти: живу стрічку 1С і знімок у базі. На таке питання правильної відповіді не
 * існує — рахунок, виписаний у 15-хвилинному вікні між синками, робив гейт червоним
 * не з нашої вини. Постійно червоний гейт перестає бути гейтом: наступний спише його
 * на «та він завжди такий» і пропустить справжню течу.
 *
 * Тепер питання інше: **чи все, що синк МІГ забрати на момент свого останнього
 * успішного проходу, він забрав.** Дрейф перестає бути предметом, теча лишається.
 *
 * 🔴 НЕВІДОМІ ЛИШАЮТЬСЯ В ПЕРЕВІРЦІ, І ЦЕ СВІДОМО. 1С позначає «часу не знаємо»
 * сентинелом `00:00:00` (заміряно: 121 рядок із 292; у годині «00» немає жодного
 * іншого значення). Виключити їх було б тихішим у бік ПРОПУСКУ ТЕЧІ — а гейт існує
 * саме проти неї. Тому вони вимагаються в базі, як і всі, але при червоному їх
 * називає ОКРЕМА категорія: інакше «не знаємо» мовчки перетворилось би на
 * «виписано до синку», тобто найранішу можливу мітку доби. Це та сама родина, що
 * «`NULL` у `CHECK` проходить».
 * 📐 Ціна залишення заміряна: серед 22 рядків за 26-27.08 сентинел мав ОДИН, тобто
 * ~одне хибне червоне на добу-дві, і воно називає себе.
 *
 * ⚠️ ЗОНА НАКЛАДАЄТЬСЯ ТУТ, І ЛИШЕ ТУТ. `at` — наївний локальний час без зони
 * (контракт `invoiceRefOf`), `last_success_at` — `timestamptz` в UTC. Зводити їх у
 * двох місцях означало б завести дві зони, які одного дня розійдуться.
 */
export interface ExpectedSplit {
  /** Рядки, які синк МІГ забрати — вимагаються в базі. */
  expected: Receivable1cRow[];
  /** Виписані ПІСЛЯ останнього синку — законно ще не в базі. */
  afterSync: Receivable1cRow[];
  /** Часу немає (сентинел 1С). Входять у `expected`, але названі окремо. */
  unknownTime: Receivable1cRow[];
}

/**
 * Наївний КИЇВСЬКИЙ `at` → UTC-мілісекунди.
 *
 * 🔴 Зсув НЕ ЗАШИТИЙ: беремо його в `Intl` на ТУ САМУ дату. Заміряно —
 * `2026-08-27 → GMT+3`, `2026-01-15 → GMT+2`; константа «+3» була б правильною
 * пів року й неправильною другу пів, і помилка виглядала б як дрейф на годину.
 *
 * ⚠️ МЕЖА, НАЗВАНА ВГОЛОС: у годинному вікні переведення стрілок зсув, обчислений
 * за UTC-наближенням, може відрізнятись від справжнього локального. Двічі на рік,
 * одна година. Ціна помилки тут — рахунок на межі вікна класифікується не в той бік
 * і дає одне хибне червоне; ціна «правильного» рішення — власний розбір переходів,
 * тобто третя копія роботи, яку вже робить `Intl`. Свідомо лишаємо межу.
 */
function kyivNaiveToUtcMs(at: string): number {
  const asUtc = Date.parse(`${at.replace(" ", "T")}Z`);
  if (Number.isNaN(asUtc)) return NaN;
  const name = new Intl.DateTimeFormat("en-US", { timeZone: "Europe/Kyiv", timeZoneName: "shortOffset" })
    .formatToParts(new Date(asUtc)).find((p) => p.type === "timeZoneName")?.value ?? "GMT+0";
  const hours = Number(name.replace(/^GMT/, "")) || 0;
  return asUtc - hours * 3_600_000;
}

export function splitByLastSync(rows: readonly Receivable1cRow[], lastSyncAt: Date | null): ExpectedSplit {
  const expected: Receivable1cRow[] = [], afterSync: Receivable1cRow[] = [], unknownTime: Receivable1cRow[] = [];
  const cut = lastSyncAt ? lastSyncAt.getTime() : null;
  for (const r of rows) {
    if (!r.invoiceAt) { unknownTime.push(r); expected.push(r); continue; }
    if (cut != null && kyivNaiveToUtcMs(r.invoiceAt) > cut) { afterSync.push(r); continue; }
    expected.push(r);
  }
  return { expected, afterSync, unknownTime };
}
