...
/**
 * 🔎 #165 — РОЗКРИТТЯ ПОЯСНЮЄ ЧИСЛО ДЖЕРЕЛА, А НЕ СПЕРЕЧАЄТЬСЯ З НИМ (Е2, 24.08.2026).
 *
...
/**
 * #165 — ПОРЯДКОВА РІВНІСТЬ ПО КОЖНОМУ ДНЮ: склад розкриття == числу колонки.
 *
 ...
 */
test("#165 склад розкриття за ДЖЕРЕЛОМ == числу дня, по кожному дню", needsDb(), async () => {
...
/**
 * #165b — ПРАВИЛО ДЖЕРЕЛА ЖИВЕ В ОДНОМУ ВИРАЗІ, І РОЗКРИТТЯ РАХУЄ НИМ САМИМ.
 *
 * 🔴 ЧОМУ НЕ ДОСИТЬ #165. Рівність чисел зійдеться й тоді, коли розкриття рахує
 ...
 */
test("#165b джерело в розкритті — з ядра, а не власною копією правила", () => {
...
/**
 * #165c — ЧОТИРИ СТАНИ ДОЇЖДЖАЮТЬ ДО ЕКРАНА, і «не знаємо» не читається як «немає».
 *
 ...
 */
test("#165c чип малює всі чотири джерела, а «не знаємо» підписане окремо", () => {
  const api = FE("api.ts");
...
/**
 * #165d — ГІЛКА «КАНАЛ НЕ ВКАЗАНО» ПЕРЕВІРЯЄТЬСЯ ЧИСТОЮ ФУНКЦІЄЮ, А НЕ ДАНИМИ.
 *
 * 🔴 ЧОМУ ОКРЕМИЙ ГЕЙТ, А НЕ ЧАСТИНА #165. Заміряно: на проді НУЛЬ угод із
 * `lead_channel IS NULL`. Тому саботаж «повернути `'undef' → null`» — тобто рівно
 * той стан, що був до Е2, — лишає #165 ЗЕЛЕНИМ: усі числа ті самі. Перевірка,
 ...
 * у предикаті.
 *
 * ⚠️ І це не тимчасовий милиця: щойн `#164e` задзвонить, дані зʼявляться, і #165
 * почне покривати гілку сам. Цей гейт лишається як страховка на порожнечу.
 *
 * 🧨 САБОТАЖ (виконано): `v === "undef" ? null` → червоніє.
 */
test("#165d sourceOf розводить усі чотири стани, і жоден не зводиться в null", async () => {
