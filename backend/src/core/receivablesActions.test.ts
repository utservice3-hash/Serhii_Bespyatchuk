...
test("#161 примітка обовʼязкова ДО відправки, і пробіли не рахуються", () => {
  ...
  const ed = readFileSync(FE("pages/dashboard/sections/OwnerEditor.tsx"), "utf8")
    .replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  // 🔴 ВИМОГА ТА САМА, МЕХАНІЗМ ІНШИЙ (переписано 26.08.2026), І ЦЕ НЕ ПОСЛАБЛЕННЯ.
  //
  // Було: «обидві кнопки запису `disabled={busy || !noteOk}`». Заборона трималась
  // на ВИМКНЕНІЙ КНОПЦІ — а вимкнена кнопка ковтає клік БЕЗ СЛІДУ: людина
  // натискає, нічого не стається, і немає підказки, куди дивитись. Власник
  // сказав «не можу надати ліміт»; насправді зберегти блокувала порожня причина,
  // і невидимою була сама відмова. Тому `disabled` прибрано, а відмову зроблено
  // видимою — текстом і фокусом у полі причини.
  //
  // Гейт спирався на МЕХАНІЗМ, а не на вимогу, і тому почервонів на правці, що
  // вимогу зберегла. Той самий клас, що «прибираєш інваріанту — знайди всіх,
  // хто на неї спирався». Тепер перевіряємо саму ЗАБОРОНУ, і це СИЛЬНІШЕ:
  // `disabled` живе у верстці й обходиться, а `run()` — єдиний шлях запису.
  assert.match(ed, /if \(!noteIsValid\(note\)\)[\s\S]{0,400}?return;/,
    "🔴 запис БЕЗ примітки більше нічим не зупиняється — ані кнопкою, ані обробником");
  assert.match(ed, /noteRef\.current\?\.focus\(\)/,
    "🔴 відмова не ставить курсор у поле причини — сказали «чого бракує», не показавши ДЕ");
  // 🪞 І заборона стоїть ПЕРЕД записом, а не після: інакше вона нічого не зупиняє.
  const runIdx = ed.indexOf("const run = async");
  const guardIdx = ed.indexOf("if (!noteIsValid(note))");
  const writeIdx = ed.indexOf("setBusy(true)");
  assert.ok(runIdx >= 0 && guardIdx > runIdx && guardIdx < writeIdx,
    "🔴 перевірка примітки стоїть не на початку `run` — порядок операторів тут і є заборона");
...
test("#163 готівці пропонується пояснення, а не кнопка, що впаде 404", () => {
  ...
  //
  // 🔴 ПЕРЕВІРЯЄМО ОБРОБНИК, А НЕ ПІДПИС (25.08.2026). Було `/✏️ змінити/` — і це
  // привʼязувало гейт до ТЕКСТУ кнопки: коли прохід верстки прибрав слово
  // «змінити» (щоб контрол не був другим рядком і не з'їдав 24% висоти
  // таблиці), гейт почервонів, хоча його предмет — пояснення для готівки —
  // не зачепили. Обробник — це те, що кнопка РОБИТЬ, і воно не залежить від
  // того, як її підписали.
  assert.match(block, /onClick=\{\(\) => setOwnerFor\(/,
    "🔴 самого контрола в блоці немає — готівці показали пояснення, а решті не лишили дії");
...
test("#164 діалог склейки показує, що саме зіллється і в який бік", () => {
  const d = readFileSync(FE("pages/dashboard/sections/MergeDialog.tsx"), "utf8")
    .replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  assert.match(d, /Зникнуть як окремі рядки/, "🔴 не сказано, яка сторона зникне");
  assert.match(d, /Лишиться основним/, "🔴 не сказано, яка сторона лишиться");
  assert.match(d, /Після обʼєднання/, "🔴 немає підсумку «що вийде» — підтверджують намір, а не результат");
  // 🔗 27.08.2026: діалог зливає N за раз, тож підсумок рахує `mergeSummary`
  // (одна функція на всі сторони), а не `alias.amount + canonical.amount`.
  // Ручна арифметика в JSX і була б тим другим виразом, який розходиться.
  assert.match(d, /mergeSummary\(/, "🔴 підсумок рахується не спільною функцією");
  assert.match(d, /\{formatAmount\(sum\.amount\)\}/, "🔴 підсумкова сума не показується");
  // 🔴 НАПРЯМОК ЛИШАЄТЬСЯ ЯВНИМ. Кнопки «⇄» більше немає — при N сторонах вона
  // безглузда, — але асиметрія мусить бути ВИДНОЮ дією: щойно ключ став
  // основним, він зникає з приєднуваних. Без цього кнопка мовчки лишалась би
  // сірою, а причину людина шукала б у списку з 70 рядків.
  assert.match(d, /setAliasKeys\(\(prev\) => prev\.filter\(\(k\) => k !== v\)\)/,
    "🔴 обраний основним лишається серед приєднуваних — напрямок перестав бути явним");
...
test("#165 склейка сама з собою і без причини блокується ДО запиту", async () => {
  const src = readFileSync(FE("pages/dashboard/receivablesView.ts"), "utf8");
  assert.match(src, /aliases\.some\(\(a\) => a\.clientKey === canonical\.clientKey\)/,
