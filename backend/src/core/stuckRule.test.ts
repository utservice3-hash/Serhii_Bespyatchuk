 ...
 * сигналу «рух етапу». Число обиралось проти ЖИВОГО списку (14/21/30) до мержу, тож
 * воно мусить лишатись параметром, а не осісти в SQL.
 *
 * 🟢 Замір зроблено 17.08.2026, обрано **21** (живий список — 140 угод). Змінилось
 * ЧИСЛО, а не механізм: перевірки нижче стережуть саме механізм — поріг підставляється
 * рівно один раз, обидва роути беруть дефолт із ядра, підпис на екрані не називає
 * власного числа. Наступна зміна порога має бути правкою ОДНОГО рядка в ядрі.
 */
test("#72g ПОРІГ ЄДИНИЙ, ПАРАМЕТРОМ, поділу 7/21 більше немає", () => {
  ...
  assert.equal(STUCK_MIN_DAYS, 21,
    "🔴 дефолт розійшовся з рішенням власника 17.08.2026: поріг 21 (живий список — 140 угод)");
