 ...
 * Переходи менеджерів між командами тут ні до чого: склад команди не змінювався.
 */
test("еталон РПК-Яцика серпень 2025: ядро 594 296 ₴ і реконструкція листа 680 655", needsDb(), async () => {
  const M = await loadMoney();
  ...
  assert.ok(Math.abs(y!.revenue - 594296) / 594296 <= 0.01,
    `дохід Яцика ${y!.revenue} відхиляється від еталона ядра 594 296 більш ніж на 1%`);
