...
/**
 * 🔒 ВЛАСНИК ЗАДАЧІ — ВУЖЧЕ ЗА «БАЧИТИ», І ЦЕ РІШЕННЯ ВЛАСНИКА 14.09.2026.
 *
 * Дослівно: «файли мають бути доступними тільки для власників цієї задачі».
 * Тобто вкладення — не те саме, що задача: назву задачі бачить наглядач, байти
 * файла — лише ті двоє, кого вона стосується.
 *
 * Власник = автор АБО виконавець (менеджер із CRM чи акаунт). Поіменно НЕ
 * власники, хоч задачу й бачать: наскрізні ролі (`admin`, `ceo`, `opdir`,
 * `kvp`, `financier`), роль `company` (HR, бухгалтерія) і тімлід команди
 * виконавця, поки він сам не автор і не виконавець.
 *
 * 🔴 ІНВАРІАНТА, ЯКУ ЦЕ МУСИТЬ ЗБЕРЕГТИ: `isTaskOwner ⊆ canSeeTask`. Власник, що
 * не бачить своєї задачі, — стан, у якому вкладення доступне, а задача ні; саме
 * дзеркальну діру (`canTouch ⊆ canSee`) ми лікували 14.09 для менеджера-автора.
 * Тримає гейт `#400p`.
 *
 * ⚠️ І ДРУГА ПОЛОВИНА, БЕЗ ЯКОЇ ЗВУЖЕННЯ ЛАМАЄ БІЛЬШЕ, НІЖ ЛІКУЄ: межа ставиться
 * і на ЧИТАННЯ, і на ЗАПИС. Якщо звузити лише читання, тімлід і наскрізний
 * збережуть право ПОКЛАСТИ файл і втратять право його ПРОЧИТАТИ — «хто кладе,
 * той бачить» перестане бути правдою, і людина не зможе перевірити те, що щойно
 * завантажила.
 */
export function isTaskOwner(v: TaskViewer, t: TaskOwnerRow): boolean {
  return t.createdBy === v.userId
    || (t.assigneeUserId != null && t.assigneeUserId === v.userId)
    || (t.assigneeId != null && t.assigneeId === v.managerId);
}

/**
 * SQL-дзеркало `isTaskOwner` — щоб лічильник вкладень у списку не показував
 * число, яке потім віддає 403.
 *
 * 🔴 Без нього межа стояла б лише на роутах, а «📎 2» світилося б наглядачеві:
 * він тиснув би й отримував відмову — рівно той клас, що «помилка сервера не
 * видима на екрані». Число теж мусить знати межу.
 */
export function ownerCondSql(v: TaskViewer, push: (value: unknown) => number): string {
  const me = push(v.userId);
  const mgr = push(v.managerId);
  return `(t.created_by = $${me} OR t.assignee_user_id = $${me} OR t.assignee_id = $${mgr})`;
}
