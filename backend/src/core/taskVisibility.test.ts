...
/**
 * #398 — ХТО МОЖЕ ЗМІНИТИ, ТОЙ МУСИТЬ БАЧИТИ (`canTouch ⊆ canSee`).
 *
 ...
 */
test("#398 МЕЖА ЗАДАЧІ: хто може ЗМІНИТИ, той БАЧИТЬ — по всіх ролях і всіх видах задач", () => {
  const broken: string[] = [];
...
/**
 * #398b — 🪞 ОСОБИСТА ЗАДАЧА ПРИВАТНА НАВІТЬ ВІД НАСКРІЗНОГО, І ЦЕ ДВОБІЧНО.
 *
 ...
 */
test("#398b 🪞 ОСОБИСТА ЗАДАЧА: автор бачить — решта, включно з наскрізним, НЕ бачить", () => {
...
/**
 * #398c — ВИКОНАВЕЦЬ-АКАУНТ НЕ РОБИТЬ ЗАДАЧУ ОСОБИСТОЮ.
 *
 ...
 */
test("#398c ВИКОНАВЕЦЬ-АКАУНТ: задача не особиста, видна виконавцю й наглядачу, чужому менеджеру — ні", () => {
...
/**
 * #398e — ВЬЮ `ai_tasks` І КОД ОПИСУЮТЬ ОДНЕ Й ТЕ САМЕ «НЕ ОСОБИСТА».
 *
 ...
 */
test("#398e ВЬЮ ai_tasks == визначенню «не особиста» з ядра", () => {
...
/**
 * #398f — СУПУТНИКИ ЗАДАЧ ЗАКРИТІ ВІД МОДЕЛІ НА ДВОХ РУБЕЖАХ.
 *
 ...
 */
test("#398f СУПУТНИКИ ЗАДАЧ: закриті і в схемі (REVOKE), і в переліку застосунку", () => {
...
/**
 * #398g — КОЖЕН ЧИТАЧ `task_files` НЕСЕ `deleted_at IS NULL`.
 *
 ...
 */
test("#398g ЧИТАЧІ task_files НЕСУТЬ deleted_at IS NULL — мʼяке видалення не протікає", () => {
...
/**
 * #398d — 🪞 СХЕМА З НУЛЯ: SQL-УМОВА ВИДИМОСТІ == JS-ПРАВИЛУ, НА СПРАВЖНІХ РЯДКАХ.
 *
 ...
 */
test("#398d 🪞 СХЕМА З НУЛЯ: SQL-скоуп == JS-правилу; CHECK одного виконавця; група не тягне задачі", async (t) => {
