...
/**
 * #399 — ХТО МОЖЕ ЗМІНИТИ, ТОЙ МУСИТЬ БАЧИТИ (`canTouch ⊆ canSee`).
 *
 ...
 */
test("#399 МЕЖА ЗАДАЧІ: хто може ЗМІНИТИ, той БАЧИТЬ — по всіх ролях і всіх видах задач", () => {
  const broken: string[] = [];
...
/**
 * #399b — 🪞 ОСОБИСТА ЗАДАЧА ПРИВАТНА НАВІТЬ ВІД НАСКРІЗНОГО, І ЦЕ ДВОБІЧНО.
 *
 ...
 */
test("#399b 🪞 ОСОБИСТА ЗАДАЧА: автор бачить — решта, включно з наскрізним, НЕ бачить", () => {
...
/**
 * #399c — ВИКОНАВЕЦЬ-АКАУНТ НЕ РОБИТЬ ЗАДАЧУ ОСОБИСТОЮ.
 *
 ...
 */
test("#399c ВИКОНАВЕЦЬ-АКАУНТ: задача не особиста, видна виконавцю й наглядачу, чужому менеджеру — ні", () => {
...
/**
 * #399e — ВЬЮ `ai_tasks` І КОД ОПИСУЮТЬ ОДНЕ Й ТЕ САМЕ «НЕ ОСОБИСТА».
 *
 ...
 */
test("#399e ВЬЮ ai_tasks == визначенню «не особиста» з ядра", () => {
...
/**
 * #399f — СУПУТНИКИ ЗАДАЧ ЗАКРИТІ ВІД МОДЕЛІ НА ДВОХ РУБЕЖАХ.
 *
 ...
 */
test("#399f СУПУТНИКИ ЗАДАЧ: закриті і в схемі (REVOKE), і в переліку застосунку", () => {
...
/**
 * #399g — КОЖЕН ЧИТАЧ `task_files` НЕСЕ `deleted_at IS NULL`.
 *
 ...
 */
test("#399g ЧИТАЧІ task_files НЕСУТЬ deleted_at IS NULL — мʼяке видалення не протікає", () => {
...
/**
 * #399d — 🪞 СХЕМА З НУЛЯ: SQL-УМОВА ВИДИМОСТІ == JS-ПРАВИЛУ, НА СПРАВЖНІХ РЯДКАХ.
 *
 ...
 */
test("#399d 🪞 СХЕМА З НУЛЯ: SQL-скоуп == JS-правилу; CHECK одного виконавця; група не тягне задачі", async (t) => {
