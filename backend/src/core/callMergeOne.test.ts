...
test("#26m ДЗЕРКАЛО НА НІЧИЇЙ: однакові позначки в даних Є, і обидві форми дають на них одне",
  ...
  /**
   * 🔴 «РІВНО ПО ОДНОМУ НА НІЧИЮ» — ТВЕРДЖЕННЯ, ЯКЕ Я СПЕРШУ НАПИСАВ І ЯКЕ ХИБНЕ.
   * Живі дані одразу показали: 53 нічиї, а вцілілих 50. Причина не в дефекті —
   * нічия може мати ще й НЕ-нічийного попередника в межах вікна (дзвінок о 10:00:00,
   * потім два з ідентичним 10:00:30), і тоді з неї законно не лишається жодного:
   * склейка про ЛАНЦЮГ, а не про пару. Правильна межа — «не БІЛЬШЕ одного».
   *
   * І вона так само ловить зникнення розриву нічиєї: без нього обидва плеча «чистої»
   * нічиї вціліють, і вцілілих стане БІЛЬШЕ за кількість нічиїх.
   */
  assert.ok(n("ne_lyshyv") <= n("nichyih"),
    `🔴 із ${n("zapysiv_u_nichyih")} записів у ${n("nichyih")} нічиїх уціліло ${n("ne_lyshyv")} — `
    + "більше, ніж по одному на нічию. Саме так виглядає зниклий розрив нічиєї за uniqueid, "
    + "і саме цей подвійний рахунок власник побачив на екрані");
  assert.ok(n("ne_lyshyv") > 0 && n("ne_lyshyv") < n("zapysiv_u_nichyih"),
    `🔴 на нічиях уціліло ${n("ne_lyshyv")} із ${n("zapysiv_u_nichyih")} — або склейка з'їла всі `
    + "(тоді це схлопування в нуль із пастки `<=`), або не склеїла жодного");
});

/**
 * 🧭 #26p — ТЕКСТ, ЩО ПОЯСНЮЄ ОЗНАЧЕННЯ, БЕРЕ ЙОГО З ЯДРА.
 *
 ...
 * на числі, яке спростовує наш власний екран, і помітити це нема з чого.
 *
 * Гейт не про красу тексту: він про те, що пояснення й обчислення не можуть розійтись,
 * бо пояснення БУДУЄТЬСЯ з того самого рядка, що документує обчислення.
 */
test("#26p ІНСТРУКЦІЯ ОРАКУЛА БЕРЕ ЯКІР ІЗ ЯДРА — і не називає власного", async () => {
  const fs = await import("node:fs/promises");
  const url = await import("node:url");
  const src = async (rel: string) =>
    fs.readFile(url.fileURLToPath(new URL(`../../src/${rel}`, import.meta.url)), "utf8");
  const { DISPATCHED_RULE_UA, DISPATCH_ANCHOR } = await import("./dispatched.js");

  // ① Правило ядра справді НАЗИВАЄ якір — інакше порожнє речення задовольнило б усе нижче.
  assert.match(DISPATCHED_RULE_UA, new RegExp(DISPATCH_ANCHOR),
    `🔴 правило ядра не називає поля-якоря «${DISPATCH_ANCHOR}» — підставляти в інструкцію нема чого`);

  // ② Інструкція БЕРЕ його, а не переказує.
  const respond = await src("ai/respond.ts");
  assert.match(respond, /\$\{DISPATCHED_RULE_UA\}/,
    "🔴 інструкція оракула більше не підставляє правило з ядра — отже десь зʼявився переказ, "
    + "а переказ розходиться мовчки");

  // ③ І НЕ називає власного якоря для відправлених. Предикат — по РЯДКУ інструкції, де
  //    стоїть підстановка, а не по всьому файлу: пояснити розкол у коментарі має бути можна
  //    (на цьому вже червонів `#26e`).
  const рядок = respond.split("\n").find((l) => l.includes("${DISPATCHED_RULE_UA}")) ?? "";
  assert.ok(рядок.length > 0, "🔴 не знайдено рядка з підстановкою — гейт втратив предмет");
  assert.doesNotMatch(рядок, /відправлен[а-яіїєґ]*\s+(авто|машин)[^.]*?(142|ПРОДАЖІ)/i,
    `🔴 у рядку інструкції знову зʼявився ВЛАСНИЙ якір для відправлених:\n   ${рядок.trim()}\n`
    + "   Саме він давав власнику 955 там, де екран показує 250");
  assert.doesNotMatch(рядок, /НЕ вигадувати/i,
    "🔴 повернулась заборона «іншого анкера НЕ вигадувати» — вона забороняла моделі САМЕ той "
    + "якір, який ядро вважає правильним, і суперечила даним її ж інструмента dispatched_by_manager");

  // ④ Еталон Q4 не питає про «відправлених», рахуючи успішних. Питання й мірило —
  //    про одне; інакше розбіжність читалась би як помилка оракула.
  const oracle = await src("ai/oracle.ts");
  const q4 = oracle.slice(oracle.indexOf('id: "Q4"'), oracle.indexOf('id: "Q4"') + 400);
  assert.ok(q4.length > 0, "🔴 випадку Q4 в еталоні немає — гейт втратив предмет");
  if (/success_by_bucket/.test(q4))
    assert.doesNotMatch(q4.split("\n").find((l) => l.includes("q:")) ?? "", /відправлен/i,
      "🔴 Q4 питає про ВІДПРАВЛЕНИХ авто, а звіряє з success_by_bucket (перехід у 142) — "
      + "це різні множини, і оракул програв би тест за правильну відповідь");
});
