...
/**
 * 🔀 #160–#163 — ДОБІР ЗА РЕЄСТРОМ ЛІДОГЕН-БОТА (24.08.2026, рішення власника).
 *
...
/**
 * #160 — ЗНАМЕННИК РЕКОМЕНДАЦІЇ БІЛЬШЕ НЕ З РЕЄСТРУ.
 *
 ...
 */
test("#160 conversionLeadgenByManager рахує знаменник за КАНАЛОМ, не за реєстром", () => {
...
/**
 * #160b — 🪞 ЗНАМЕННИК КОНВЕРСІЇ == ФАКТ ЛІДГЕНУ ЗІ ЗВІТУ, ПО КОЖНОМУ МЕНЕДЖЕРУ.
 *
 ...
 */
test("#160b знаменник конверсії лідгену == факт лідгену Звіту, по кожному менеджеру",
  { ...needsApi() }, async () => {
...
/**
 * #160c — У МЕТРИКАХ НЕ ЛИШИЛОСЬ ЖОДНОГО ЧИТАЧА РЕЄСТРУ.
 *
 * Ширше за #160: воно про ФАЙЛ, а не про одну функцію. Наступний, хто додасть
 * лідген-метрику, найпростішим шляхом візьме `leadgen_touch` (вона в схемі, вона
 * називається як треба) — і заведе третє означення. Гейт це ловить одразу.
 *
 ...
 * живить `reclassifyAdChannel`, тобто той самий `lead_channel`, на який ми перейшли.
 *
 * 🧨 САБОТАЖ (виконано): дописати `FROM leadgen_touch` у будь-яку функцію
 * `core/metrics.ts` → червоніє; прибрати `upsertLeadgenTouch` із `syncKommo.ts` →
 * червоніє дзеркало.
 */
test("#160c core/metrics.ts не читає leadgen_touch — але писар таблиці живий", () => {
  const metricsSrc = stripComments(src("core/metrics.ts"));
  assert.doesNotMatch(metricsSrc, /leadgen_touch/,
    "🔴 у метриках знову зʼявився читач реєстру — це заводить друге означення лідгену");

  // 🧾 ЄДИНИЙ ЗАЛИШЕНИЙ ЧИТАЧ `leadgen_registry` — НАЗВАНИЙ ЧИСЛОМ, А НЕ ЗАМОВЧАНИЙ.
  // Це когортна гілка `conversionByCohort(entry='transferred')`: вона, як і
  // `lg_transfers`, міряє ПЕРЕДАЧІ, тож канал їй не підходить. Але вона малює графік
  // за 12 МІСЯЦІВ із таблиці, що тримає ~5 тижнів, — тобто має ТОЙ САМИЙ дефект
  // обрізаної історії, і перевести її на `leadgen_touch` в один рядок НЕ МОЖНА:
  // там `MIN(transferred_at)` (timestamptz) порівнюється з `changed_at`, а в
  // `leadgen_touch` лежить DATE — зсув до доби в чисельнику. Це окремий прохід зі
  // своїм прийманням; тут зафіксовано, що читач РІВНО один і новий не зʼявився.
  const regReads = (metricsSrc.match(/FROM leadgen_registry/g) ?? []).length;
  assert.equal(regReads, 1,
    `🔴 читачів \`leadgen_registry\` у метриках ${regReads}, а має бути рівно 1 (когортні передачі). `
    + "Побільшало — заводиться нове реєстрове означення; поменшало — когорту перевели мовчки, "
    + "без приймання зсуву чисельника");
...
/**
 * #161 — «НЕДОСЯЖНА ЦІЛЬ» МІРЯЄТЬСЯ ТІЄЮ САМОЮ МНОЖИНОЮ, ЩО Й КОНВЕРСІЯ.
 *
 ...
 */
test("#161 максимум лідгену за місяць — канальний (джерело)", () => {
...
test("#161b максимум лідгену == найбільший місячний знаменник конверсії",
  { ...needsApi() }, async () => {
    const m = await import("./metrics.js");
...
/**
 * #162 — РЯД «ПРОРАХУНКИ ЛІДГЕНІВ» БІЛЬШЕ НЕ ОБРІЗАЄТЬСЯ, І ПРИ ЦЬОМУ МІРЯЄ ТЕ САМЕ.
 *
 ...
 */
test("#162 lg_transfers читає append-only leadgen_touch, а не обрізаний реєстр", () => {
...
test("#162b ряд передач непорожній і збігається з реєстровим за перекритий період",
  { ...needsApi() }, async () => {
...
/**
 * #163 — МЕРТВІ РЕЄСТРОВІ ФУНКЦІЇ ЗНЯТО, А НЕ ЛИШЕНО «ПРО ЗАПАС».
 *
 ...
 * червоніє.
 */
test("#163 leadgenByManager / leadgenByManagerBucket не існують і ніким не кличуться", () => {
  const metricsSrc = stripComments(src("core/metrics.ts"));
...
/**
 * #163b — ОПИС AI-ІНСТРУМЕНТА НЕ ОБІЦЯЄ ТОГО, ЧОГО ФУНКЦІЯ БІЛЬШЕ НЕ РОБИТЬ.
 *
 ...
 */
test("#163b опис conversion_leadgen_by_manager описує КАНАЛ, а не реєстр", () => {
