...
/**
 * 🎥 ЗАПИСИ СПІВБЕСІД tl;dv (23.09.2026, прохід 7) — гейти `#730`–`#733`.
 * Номери з запасом над `#716` (у main уже #704–#716 інших проходів) — борг 17: перед мержем перемірити перетин.
 */
...
/**
 * #730 — ЧИЯ ЗУСТРІЧ: певний збіг лише за поштою учасника; збіг за часом — «підказка», а не привʼязка;
 ...
 */
test("#730 tl;dv: певний збіг лише за поштою; час — підказка; двоє в вікні — «кілька»", () => {
  const rows = [
...
/**
 * #731 — РОЗБІР ВІДПОВІДІ tl;dv в ОДНОМУ місці: id, назва, час, тривалість у хвилинах, посилання, пошти
 ...
 */
test("#731 tl;dv: розбір відповіді — секунди в хвилини, пошти учасників, посилання з id", () => {
  const body = { results: [
...
/**
 * #732 — ЖИВИЙ SQL: певний збіг привʼязується сам (посилання в рядок, подія в історію кандидата), непевний
 ...
 */
test("#732 ЖИВИЙ SQL: певний збіг привʼязується сам, непевний чекає; ручне посилання не затирається", async (t) => {
...
/**
 * #733 — БЕЗ КЛЮЧА — ЧЕСНИЙ ПРОПУСК, А НЕ ПОРОЖНІЙ СПИСОК; таблиця зустрічей відібрана в ai_readonly і є у
 * FORBIDDEN_TABLES. 🧨 Червоніє, якщо джоба без ключа «успішно» нічого не робить або таблиця відкрита моделі.
 */
test("#733 tl;dv: без ключа — пропуск із причиною; tldv_meetings закрита для AI", async () => {
