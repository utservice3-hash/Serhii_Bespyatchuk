...
/**
 * 🎓 ЕКРАН «НАВЧАННЯ» — ОДИН ДЛЯ ЧИТАЧА Й РЕДАКТОРА (23.09.2026) — гейти `#706`–`#708`.
...
/**
 * #706 — ПЕРШИМ ВІДКРИВАЄТЬСЯ КУРС, А НЕ СХОВИЩЕ: вкладка «Курси» стоїть перед «Бібліотекою» і є
 ...
 */
test("#706 НАВЧАННЯ: початкова вкладка — «Курси», бібліотека друга й доступна всім", () => {
  const s = fe(SECTION);
...
/**
 * #707 — РЕДАГУВАННЯ: лише за правом СЕРВЕРА (`canEdit`) і лише в окремому режимі, а не по кліку в тексті;
 ...
 */
test("#707 НАВЧАННЯ: редагування — за правом сервера, окремим режимом і наявними роутами", () => {
  const s = fe(COURSES);
...
/**
 * #708 — СТАН КРОКУ Й ЗАМКИ — З СЕРВЕРА: екран малює `state`/`blockedBy` із відповіді, а не рахує сам
 ...
 */
test("#708 НАВЧАННЯ: стан кроку й замки беруться з сервера, а не рахуються в екрані", () => {
  const s = fe(COURSES);
