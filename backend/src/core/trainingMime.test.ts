...
/**
 * 📄 ТИП ФАЙЛА НАВЧАННЯ (23.09.2026) — гейти `#709`, `#709b`.
...
/**
 * #709 — ВИВЕДЕННЯ ТИПУ ПО ОБИДВА БОКИ МЕЖІ. Фікстура з одного значення доводила б лише те,
 ...
 */
test("#709 ТИП ФАЙЛА: відоме розширення → тип, невідоме → null, збережений виграє в виведеного", () => {
...
/**
 * #709b — ЖОДЕН РОУТ НАВЧАННЯ НЕ ВІДДАЄ СИРИЙ `mime`. Правило має жити в одній копії: щойно
 ...
 */
test("#709b РОУТИ НАВЧАННЯ: тип файла — лише через ядро, у жодному місці не сирою колонкою", () => {
  ...
  assert.match(src, /mime = b\.mime \? String\(b\.mime\) : mimeFromName\(display\)/,
    "🔴 новий файл без типу від браузера знову ляже з порожнім mime — тобто дефект відтвориться на нових даних");
