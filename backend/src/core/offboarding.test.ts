...
/**
 * 🚪 ЗВІЛЬНЕННЯ У ДВА КРОКИ + 📎 ДОКУМЕНТИ ЛЮДИНИ В HR (21.09.2026) — гейти `#610`–`#614`.
 * Номери з запасом над `#609` — борг 17: перед мержем перемірити перетин.
 */
...
/**
 * #610 — ДВА КРОКИ: «Звільнити…» → реєстр «завершує», менеджер «завершує», вхід ПРАЦЮЄ; «Завершити звільнення»
 ...
 */
test("#610 ЖИВИЙ SQL: звільнення у два кроки — «завершує» лишає вхід, «завершити» закриває", async (t) => {
...
/**
 * #611 — «ПОВЕРНУТИ» ДО БАЙТА: після обох кроків реєстр, стан менеджера (разом із тим, що стояв ДО) і акаунт
 ...
 */
test("#611 ЖИВИЙ SQL: «Повернути» — реєстр, стан менеджера й акаунт до байта; сейф і документи не чіпаються", async (t) => {
  const s = await scratch(t); if (!s) return;
  try {
...
/**
 * #612 — ЗАПОБІЖНИКИ: без дати чи причини — 400; себе не звільнити; під час звільнення форма не міняє статус
 ...
 */
test("#612 ЖИВИЙ SQL: під час звільнення форма й повторний імпорт не міняють статус; себе не звільнити", async (t) => {
  const s = await scratch(t); if (!s) return;
...
/**
 * #613 — ДОКУМЕНТИ ЛЮДИНИ: без акаунта документ належить людині (`employee_id`), з акаунтом — адресат-акаунт;
 ...
 */
test("#613 ЖИВИЙ SQL: документи людини — власник, розділ, чужий 404, прибрати й повернути", async (t) => {
  const s = await scratch(t); if (!s) return;
 ...
 */
test("#614 ЗВІЛЬНЕННЯ: employee_offboarding відібрана в ai_readonly і є в FORBIDDEN_TABLES", () => {
