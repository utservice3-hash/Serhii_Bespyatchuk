...
/**
 * 💰 #405 — ПЛИТКА «ЗАРОБИЛИ» НАЗИВАЄ ПОЛЕ CRM, А НЕ ВДАЄ РОЗРАХУНОК.
 *
 * 📐 ПРИВІД, НАЗВАНИЙ ВЛАСНИЦЕЮ 14.09.2026: «я на цьому вже помилилась у розрахунку,
 * тімліди помиляться теж». Помилка відтворювана з екрана: людина бачить суму, поруч
 * відсоток «від суми рахунків» — і добудовує «отже, суму ПОРАХУВАЛИ з суми рахунків».
 * Насправді `earned` = `deals.price` (`core/receivablesFacts.ts`), тобто готове поле
 * «Бюджет» угоди, яке в цьому продукті вже є маржею; з повної суми рахунків рахується
 * ЛИШЕ відсоток поруч. Той самий клас, що «сер.чек ÷ авто»: підпис правдоподібний,
 * величина за ним інша.
 *
 * 🔴 ЧОМУ ТЕКСТОМ, А НЕ ЧЕРЕЗ `marginHint`. Підказка спільна з клітинкою рядка клієнта
 * (`ReceivablesSection.tsx`), і правка там змінила б ДВА місця; до того ж підказка живе
 * в `title=`, тобто її не видно, поки не наведеш. Помилку робили, ДИВЛЯЧИСЬ на плитку.
 *
 * 🧨 САБОТАЖ: замінити в плитці «Береться з поля «Бюджет» угоди в CRM» на
 * «Маржа, порахована з повної суми угоди» → червоніє.
 */
test("#405 плитка «Заробили» називає ПОЛЕ CRM, з якого взято число", () => {
  const tiles = readFileSync(FE("pages/dashboard/sections/ReceivablesTiles.tsx"), "utf8");
  assert.match(strip(tiles), /Бюджет/,
    "🔴 плитка більше не називає поле CRM — число знову читатиметься як порахований "
    + "показник, і наступний читач повторить помилку в розрахунку");
});

/**
 * 🪞 #405b — І НЕ СТВЕРДЖУЄ ПРО ГРОШІ ТОГО, ЧОГО НЕ ЗНАЄ.
 *
 * 🔴 ДВОБІЧНИЙ СВІДОМО (правило 8: перевірка на відсутність мусить мати парне «а де воно
 * ТЕПЕР»). Сам лише запобіжник «немає слова про надходження» зеленів би й тоді, коли
 * скоуп не названий ЗОВСІМ.
 *
 * 📐 ЧОМУ САМЕ ТАКЕ СЛОВО. У ТЗ просили написати «оплата за які ще не надійшла». Це
 * НЕДОВЕДЕНЕ: `deals.price` про рух грошей не каже нічого, а в скоупі є й частково
 * оплачені рахунки — тобто на вимірюваній підмножині фраза просто хибна. Натомість
 * «лишився непогашений борг» правдиве ЗА ПОБУДОВОЮ: рядок потрапляє в
 * `receivable_invoices` або з ненульового залишку 1С, або як готівкова угода на
 * `approved`/`invoiced` поза `PAID_STATUSES` (`jobs/syncReceivables.ts`).
 *
 * 🧨 САБОТАЖ: замінити «по яких лишився непогашений борг» на «оплата за які ще не
 * надійшла» → червоніє ОБОМА половинами одразу.
 */
test("#405b 🪞 плитка називає скоуп боргом і НЕ обіцяє рух грошей", () => {
  const tiles = strip(readFileSync(FE("pages/dashboard/sections/ReceivablesTiles.tsx"), "utf8"));
  assert.match(tiles, /непогашений борг/,
    "🔴 плитка перестала називати скоуп — без нього «Заробили» читається як гроші, "
    + "які вже в касі");
  assert.ok(!/надійшл|отриман/i.test(tiles),
    "🔴 плитка стверджує про НАДХОДЖЕННЯ грошей. Поле «Бюджет» про рух грошей не каже "
    + "нічого, а в скоупі є частково оплачені рахунки — тобто твердження хибне, а не "
    + "лише недоведене");
});

/**
 * 🧮 #405c — ПЕРЕЛІЧУВАЧ: УСІ ТРИ МІСЦЯ З ЦИМ ЧИСЛОМ НАЗИВАЮТЬ ОДНЕ Й ТЕ САМЕ ПОЛЕ.
 *
 * 🔴 ПЕРЕЛІЧУВАЧ МАЄ ВЛАСНУ СЛІПУ ЗОНУ — ФАЙЛ (правило 12). Одне число живе на трьох
 * екранних місцях: плитка підсумку, колонка рядка клієнта, підказка в розкритті. Гейт
 * на одне з них зеленіє, поки два інші розходяться — а розійшовшись, вони дають тій
 * самій величині два різні означення на ОДНОМУ екрані.
 *
 * ⚠️ Критерій від ПРЕДМЕТА, а не від слівця: «усі файли, що показують це число», а не
 * «файли, де є слово Бюджет».
 *
 * 🧨 САБОТАЖ: прибрати «Бюджет» із підказки в `receivablesView.ts` → червоніє, і текст
 * падіння називає САМЕ той файл.
 */
test("#405c усі читачі числа «Заробили» називають поле «Бюджет»", () => {
  const READERS: [string, string][] = [
    ["плитка підсумку", "pages/dashboard/sections/ReceivablesTiles.tsx"],
    ["рядок клієнта", "pages/dashboard/sections/ReceivablesSection.tsx"],
    ["правила подання", "pages/dashboard/receivablesView.ts"],
  ];
  for (const [what, rel] of READERS) {
    assert.match(strip(readFileSync(FE(rel), "utf8")), /Бюджет/,
      `🔴 ${what} (${rel}) більше не називає поле CRM. Тоді одне число має на екрані `
      + "два різні означення — рівно те, від чого стоїть цей перелічувач");
  }
});
...
test("#199k нерозбірна або відсутня дата НЕ валить екран", async () => {
  ...
  // ⚠️ ЧИСЛО КОЛОНОК ТУТ НЕ ПРИБИВАЄТЬСЯ, І ЦЕ НЕ ПОСЛАБЛЕННЯ (27.08.2026).
  // Гейт стереже МЕЖУ, а не сітку. Прибитий літерал `cols={11}` червонів на
  // цілком законному додаванні колонки «Ліміт суми» — тобто наказував не
  // міняти таблицю, хоч про таблицю нічого не стверджує. Рівність числа
  // колонок шапці доводить `#199h`, і доводить сильніше: він виводить її з
  // `<th>`, а не порівнює з написаним тут. Той самий клас, що зріз «+4500
  // символів» — перевірка, прибита до значення, яке має право змінюватись.
  // ⚠️ І ВИРАЗ КЛЮЧА ТУТ ТЕЖ НЕ ПРИБИВАЄТЬСЯ — з тієї самої причини, що й cols
  // (01.09.2026). Було прибито `key={`${c.clientKey}-${i}`}`, і гейт почервонів на
  // правці, яка ПРИБИРАЛА дефект: індекс у ключі змушував React розмонтовувати рядок
  // разом із відкритим редактором домовленості при кожному пересортуванні, і набраний
  // текст зникав. Гейт стереже МЕЖУ, а не те, з чого зроблено ключ; сам ключ доводить
  // `#251`, і доводить сильніше — викликом функції, а не звіркою тексту.
  assert.match(sec, /<RowBoundary key=\{[^}]+\} label=\{c\.clientName\} cols=\{\d+\}>/,
...
test("#229 скоуп дебіторки — ОДИН вираз, і сусідні двері беруть його, а не свій", () => {
  // ⚠️ НОМЕР 229, А НЕ 227, І ЦЕ НЕ ПРИДИРКА. `#227`-`#227g` уже зайняті замком
  // прод-чекауту (`tools/checkoutLock.test.ts`). Два різні тести під одним іменем
  // роблять рядок падіння НЕОДНОЗНАЧНИМ: «не ok #227c» перестає називати, що саме
  // зламалось. Ми на цьому вже втратили прохід із `#30*`, які досі існують
  // одночасно у двох файлах; повторювати не будемо.
...
test("#229b звуження — ПРЕДИКАТ, а не «менеджер бачить менше»", async () => {
...
test("#229c архів списань фільтрується ключами СВОЇХ клієнтів, і міст — через рахунки", () => {
...
/* ═════════════ ПРОХІД B — СТРУКТУРА ЗА МАКЕТОМ v6.1 (27.08.2026) ═════════════
   🔴 МЕЖА ВСІХ ЧОТИРЬОХ, НАЗВАНА ПЕРШИМ РЯДКОМ: вони стережуть ДЖЕРЕЛО, з якого
   виходить геометрія, а піксель бачить лише екран. Числа приймання (рядок
   48..48, девʼять боржників у вікні) знімаються браузером і в гейт не входять —
   інакше він міряв би не те, чим користується людина. Це та сама чесна межа, що
   в `#199ad`. */


test("#199bk у рядку клієнта НЕМАЄ полів вводу — редагування живе в поповері", () => {
  // 🔴 ЦЕ ПЕРЕВІРКА ПРИЧИНИ, А НЕ НАСЛІДКУ. Висоту тримали три контроли в
  // клітинці домовленості: `span 16 + input 27 + CommentField 42 + button 32`.
  // Гейт на «рядок ≤52px» вимагав би браузера; гейт на ПРИЧИНУ падає там-таки,
  // де її повернуть, і називає її словом.
  const sec = strip(readFileSync(FE("pages/dashboard/sections/ReceivablesSection.tsx"), "utf8"));
  const row = sec.slice(sec.indexOf('className="recv-row"'), sec.indexOf("{renderInvoices(c.clientKey"));
  assert.ok(row.length > 500, "🔴 рядок клієнта не знайдено — гейт міряє порожнечу");

  assert.ok(!/<input\b/.test(row), "🔴 у рядок клієнта повернулось <input> — саме воно тримало 117px");
  assert.ok(!/<textarea\b/.test(row), "🔴 у рядок клієнта повернулась <textarea>");
  assert.ok(!/<CommentField\b/.test(row),
    "🔴 у рядок клієнта повернувся CommentField — його підлога 53px і робила рядок утричі вищим");

  // 🪞 ДЗЕРКАЛО, БЕЗ ЯКОГО ЦЕ ЗЕЛЕНІЛО Б НА МЕРТВОМУ ЕКРАНІ: поля не просто
  // зникли — вони переїхали. У рядку РАХУНКА вони лишаються (там свій ритм 38px
  // і своя дія), а домовленість клієнта відкривається кнопкою.
  const inv = sec.slice(sec.indexOf("{inv.map((x, i) => {"), sec.indexOf("\n        })}"));
  assert.ok(/<CommentField\b/.test(inv),
    "🔴 CommentField зник і з рядка РАХУНКА — прохід верстки забрав робочу дію, а не переніс її");
  assert.ok(/<input type="date"/.test(inv),
    "🔴 дедлайн оплати рахунка перестав редагуватись — це інша дія, і вона нікуди не переїжджала");
});

test("#199bl домовленість відкриває ВСЯ ділянка, і поповер уміє те саме, що вміла клітинка", () => {
  // 🔴 ТРИГЕР — ДІЛЯНКА, А НЕ ОЛІВЕЦЬ (макет v6.1). Заміряно на макеті: після
  // підлоги кнопок 32×32 окремий олівець зʼїдав 38px і лишав тексту коментаря
  // **12px** — один символ і трикрапка. Тобто колонка була, а прочитати в ній
  ...
  // Поповер уміє ТЕ САМЕ: дата, коментар, збереження тим самим викликом.
  const ed = strip(readFileSync(FE("pages/dashboard/sections/AgreementEditor.tsx"), "utf8"));
  assert.match(ed, /<input[^>]*type="date"/, "🔴 у поповері немає обіцяної дати — дія втрачена при переїзді");
  assert.match(ed, /<textarea\b/, "🔴 у поповері немає поля домовленості");
  assert.match(ed, /\bsaveReceivableNote\(\{[^}]*clientKey[\s\S]{0,120}comment[\s\S]{0,120}dueDate/,
    "🔴 поповер не зберігає тим самим викликом і тим самим тілом — журнал розійшовся б із полем");
  // 17.09.2026: у патч іде ЗЛИТЕ значення (`effective`) — порожнє поле лишає старий текст (#459).
  assert.match(ed, /onPatch\(\{ comment: (?:next|effective), dueDate: nextDate \}\)/,
    "🔴 зник оптимістичний патч — значення поверталось би лише на рефреші, і людина вирішила б, що не збереглось");

  // ⌨️ Esc закриває І ПОВЕРТАЄ ФОКУС. Без другої половини клавіатурний шлях
  // обривається: поповер зник, фокус на body, наступний Tab починає спочатку.
  assert.match(ed, /e\.key === "Escape"/, "🔴 Esc більше не закриває поповер домовленості");
  // 🔴 І ФОКУС МУСИТЬ ЧЕКАТИ, ПОКИ ПОПОВЕР СТАНЕ ВИДИМИМ. Затискач тримає
  // `visibility: hidden` до заміру, а `focus()` на прихованому елементі браузер
  // МОВЧКИ ІГНОРУЄ. Перша редакція фокусувала на монтуванні: виклик у джерелі
...
test("#199bl2 рядок і поповер беруть ОДИН вираз домовленості — тижнева межа не роздвоїлась", async () => {
  // 🔴 ЦЕ НАЙТИХІША З МОЖЛИВИХ ПОЛОМОК ЦЬОГО ПРОХОДУ. Якби згорнутий рядок
  // читав `c.comment`, а редактор — звужений `activeNote`, людина бачила б
  // торішню обіцянку й правила б цьоготижневу (або навпаки). Обидва числа
  ...
  // Чиста функція перелічує ВСІ чотири стани, і порожній має ВІДПОВІДЬ.
  // 🔴 ЗНАЧЕННЯ, А НЕ ТЕКСТ. Гейт кличе САМУ функцію фронта — інакше він
  // доводив би рівність двох рядків, написаних поруч (урок `#214c`).
  const V = (await import(FE_SPEC("pages/dashboard/receivablesView.ts"))) as {
    agreementLine: (d: string | null, n: string)
      => { dateText: string; text: string; empty: boolean; tip: string };
    AGREEMENT_EMPTY_LABEL: string;
  };
  const line = V.agreementLine;
  const both = line("2026-08-28", "оплатять у пʼятницю");
  assert.equal(both.empty, false);
  assert.ok(both.dateText && both.text, "🔴 повний стан утратив половину");
  assert.ok(both.tip.includes("оплатять у пʼятницю"),
    "🔴 підказка не несе повного тексту — обрізання почало ховати зміст");

  const dateOnly = line("2026-08-28", "");
  assert.equal(dateOnly.empty, false, "🔴 дата без коментаря вважається порожнечею");
  const textOnly = line(null, "передзвонять");
  assert.equal(textOnly.empty, false, "🔴 коментар без дати вважається порожнечею");
  assert.equal(textOnly.dateText, "", "🔴 з нізвідки взялась дата");

  const none = line(null, "");
  assert.equal(none.empty, true, "🔴 порожній стан не розпізнано");
  assert.ok(none.tip.includes("поточний тиждень") || none.tip.includes("поточного тижня"),
    "🔴 порожній стан не пояснює, ЧОМУ порожньо — читається як «домовленостей немає взагалі»");
  assert.equal(V.AGREEMENT_EMPTY_LABEL, "записів немає");

  // 🪞 Нерозбірна дата не робить рядок порожнім і не кидає — той самий сторож,
  // що в `#199k`: один поганий запис не сміє забрати з собою решту таблиці.
...
test("#199cc КОЖЕН споживач затискача приєднує його `ref` — інакше поповер невидимий назавжди", () => {
  // 🔴 ЦЕЙ ГЕЙТ ПИШЕТЬСЯ ПІСЛЯ ЖИВОГО ДЕФЕКТУ НА ПРОДІ, І ЦЕ ВАЖЛИВО НАЗВАТИ.
  // `OwnerEditor` брав `clamp.style`, але `clamp.ref` не приєднував. Наслідок не
  // «трохи зʼїхало», а «редактора немає»: без `ref` вихід у `useLayoutEffect`
  // стається першим рядком, координати не рахуються НІКОЛИ, і стиль назавжди
  // лишається `visibility: hidden; left: 0; top: 0`.
  //
  // Доведено ДІЄЮ на БАЙТАХ ПРОДА (sha звірено з докрутом), а не читанням:
  // клік по олівцю «Змінити відповідального» → `visibility: "hidden"`.
  //
  // 🔴 І ЧОМУ ЦЬОГО НЕ БАЧИЛИ ГЕЙТИ: вони перевіряли, що стеля висоти й
  // прокрутка ЗАДАНІ. Вони й були задані — приїхали з того самого `clamp.style`.
  ...
  for (const f of users) {
    ...
    if (!/usePopoverClamp\(/.test(c)) continue;
    checked++;
    assert.match(c, /ref=\{narrow \? undefined : clamp\.ref\}/,
      `🔴 ${f}: бере clamp.style, але НЕ приєднує clamp.ref — поповер лишиться visibility:hidden назавжди`);
    // І клас, що скасовує заборону переносу: поповер живе всередині <td> рядка
    // клієнта, а там прохід B поставив `nowrap` заради висоти 48px.
    assert.match(c, /className="recv-pop"/,
      `🔴 ${f}: без .recv-pop текст поповера успадкує nowrap клітинки й обріжеться на краю`);
  }
  assert.equal(checked, users.length,
    `🔴 перевірено ${checked} із ${users.length} поповерів — гейт міряв порожнечу`);

  // 🪞 ДЗЕРКАЛО: сам затискач досі ставить `hidden` до заміру. Якби він цього не
  // робив, вимога про `ref` втратила б підставу, і гейт стеріг би звичку.
  const hook = strip(readFileSync(FE("pages/dashboard/usePopoverClamp.ts"), "utf8"));
  assert.match(hook, /visibility: pos \? "visible" : "hidden"/,
    "🔴 затискач більше не ховає поповер до заміру — перевір, чи вимога про `ref` іще про щось");
  assert.match(hook, /const el = ref\.current;\s*if \(!el\) return;/,
    "🔴 затискач більше не виходить без `ref` — саме цей вихід і робив поповер вічно невидимим");

  // І правило скасування переносу існує в CSS, а не лише клас у розмітці.
  const css = readFileSync(FE("index.css"), "utf8");
...
test("#199cg вираз скоупу дебіторки — ОДИН на всі чотири місця", () => {
  ...
  // 🪞 ДЗЕРКАЛО: РЕЗУЛЬТАТ СКОУПУ МУСИТЬ ВИКОРИСТОВУВАТИСЬ, а не лише рахуватись.
  //
  // 🔴 ПЕРША РЕДАКЦІЯ ЦЬОГО ДЗЕРКАЛА БУЛА БЕЗЗУБА, і спіймав це саботаж. Вона
  // вимагала в тілі `status(403)` або `status(sc.status)` — а `status(sc.status)`
  // стоїть у перевірці `if (!sc.ok)`, тобто у ВІДМОВІ ПРО ІНШЕ. Саботаж «узяти
  // скоуп і не застосувати» (`void mine;`) лишав гейт ЗЕЛЕНИМ: вираз спільний,
  // межі немає, дефект повернувся. Тепер перевіряється споживання `mine`.
  //
  // ⚠️ `GET /receivables` сюди не входить свідомо: він САМ і є список, його
  // скоуп споживається побудовою відповіді, а не гейтом по одному клієнту.
  const gated = places.filter(([l]) => l !== "GET /receivables");
  for (const [label, anchor] of gated) {
    const from = src.indexOf(anchor);
    const to = src.indexOf("dashboardRouter.", from + anchor.length);
    const body = src.slice(from, to > from ? to : undefined);
    assert.match(body, /\bmine\.some\(|\bmine\.map\(|\bvisibleKeys\b/,
      `🔴 ${label}: скоуп ПОРАХОВАНО І НЕ ВИКОРИСТАНО — вираз спільний, а межі немає. `
      + "Це гірше за другий вираз: той хоч відмовляв.");
  }
});

/**
 * 🔁 #255/#255b — ЗАМІНА ТРЬОМ ГЕЙТАМ, ЗНЯТИМ 01.09.2026 РАЗОМ ІЗ ЇХНІМ ПРЕДМЕТОМ.
 *
 * Знято поіменно (`gateCount` назве це зникненням, і так і має бути):
 *   · `#199d`  — «висоту рядка тримає ОДИН рядок»: макет власника СВІДОМО вводить
 *                другі рядки (юрособа+перевізник під менеджером, вік під сумою);
 *   · `#199bj` — «Ліміт суми — ВЛАСНА колонка»: макет зводить два ліміти в одну
 *                клітинку на два рядки;
 *   · `#199cb` — правильне твердження про доступність, але привʼязане до ПІДПИСУ
 *                колонки («Сума боргу»), а підпис змінився на «Борг».
 *
 * 🔴 ЗНЯТТЯ ГЕЙТА НЕ МОЖЕ БУТИ ЧИСТИМ ВІДНІМАННЯМ. Перевірка на відсутність
 * винагороджує зникнення, якщо не має парного «а де воно ТЕПЕР» — тож замість
 * трьох знятих стоять два, що стережуть те саме по суті: дія лишилась доступною,
 * дані лишились на екрані.
 *
 * ⚠️ МЕЖА: DOM-харнеса в проєкті немає, тож це твердження про ДЖЕРЕЛО. Інша
 * поверхня для JSX-розмітки відсутня в принципі; піксельну частину доводить
 * замір рамок у браузері, а не гейт.
 */
test("#255 обидва сортування лишились доступними й ОГОЛОШЕНИМИ — без привʼязки до підпису", () => {
  const sec = strip(readFileSync(FE("pages/dashboard/sections/ReceivablesSection.tsx"), "utf8"));

  // Кнопки шукаємо за ДІЄЮ (`nextSort(sort, key)`), а не за текстом заголовка:
  // саме привʼязка до тексту й зробила попередній гейт крихким.
  for (const key of ["amount", "days"] as const) {
    const btn = new RegExp(`nextSort\\(sort, "${key}"\\)`);
    assert.match(sec, btn, `🔴 сортування за «${key}» зникло разом із колонкою — це втрата ДІЇ, а не стовпця`);
    const mark = new RegExp(`sortMark\\(sort, "${key}"\\)`);
    assert.match(sec, mark, `🔴 «${key}»: напрям сортування не показано`);
    assert.match(sec, new RegExp(`ariaSort\\(sort, "${key}"\\)`),
      `🔴 «${key}»: стан сортування не оголошується — порядок видно, але не чути`);
  }
  // Заголовки лишаються КНОПКАМИ: `div` довелося б вручну наділяти роллю,
  // tabIndex і обробником Enter/Space, і в цьому відтворенні щось незмінно гине.
  assert.match(sec, /<button type="button" className="recv-sort"/,
    "🔴 заголовок сортування перестав бути кнопкою");
});

test("#255b ЖОДЕН ПОКАЗНИК НЕ ЗНИК разом зі своєю колонкою", () => {
  // 🔴 Це парне твердження до зняття трьох гейтів вище. Тринадцять колонок
  // стиснуто у вісім — і саме тут доводиться, що стиснуто КОЛОНКИ, а не дані.
  const src = readFileSync(FE("pages/dashboard/sections/ReceivablesSection.tsx"), "utf8");
  const sec = strip(src);
  const row = sec.slice(sec.indexOf('className="recv-row"'), sec.indexOf("{renderInvoices(c.clientKey"));
  assert.ok(row.length > 500, "🔴 рядок клієнта не знайдено — гейт міряє порожнечу");
  ...
  for (const [what, re] of [
    ["наша юрособа", /\bent\b/],
    ["перевізник", /\bcar\b/],
    ["ліміт днів", /limitLabel\(c\.limitDays\)/],
    ["ліміт суми", /amountLimitLabel\(c\.limitAmount\)/],
  ] as const) {
    assert.match(row, re, `🔴 «${what}» зник із рядка разом зі своєю колонкою — це втрата ДАНИХ`);
  }

  // 🔴 ВІК БОРГУ ПЕРЕВІРЯЄТЬСЯ У ТОМУ БЛОЦІ, ЩО ЙОГО ПОКАЗУЄ, А НЕ ПО ВСЬОМУ РЯДКУ.
  // Перша редакція шукала `c.overdueDays` у рядку цілком — і лишалась ЗЕЛЕНОЮ,
  // коли саме число прибрали з екрана: поле й далі згадувалось у сусідньому
  // `isAncientDebt(c.overdueDays)`. Заміряно саботажем; це та сама «перевірка на
  // присутність без межі», що вже ловила нас на `INSERT INTO leadgen_touch_OFF`.
  const dl = row.indexOf('className="recv-daysline"');
  assert.ok(dl > 0, "🔴 блок віку боргу зник цілком — рядок більше не каже, скільки чекає борг");
  const daysBlock = row.slice(dl, row.indexOf("</div>", dl));
  assert.match(daysBlock, /c\.overdueDays/,
    "🔴 блок віку є, а самого числа в ньому немає — залишилась порожня рамка");
  assert.match(daysBlock, /дн\./,
    "🔴 зникла одиниця «дн.» — голе число без одиниці читається як сума або відсоток");

  // 🪞 І дзеркало: колонок справді стало вісім, тобто стиснення відбулось.
  // Без цієї половини гейт був би зелений і на старій тринадцятиколонковій
  // таблиці, тобто не стеріг би нічого.
  const head = sec.slice(sec.indexOf("<thead>"), sec.indexOf("</thead>"));
  const cols = (head.match(/<th\b/g) ?? []).length;
