...
test("#199bj «Ліміт суми» — ВЛАСНА колонка, а не другий рядок у клітинці боргу", () => {
  // 🔴 ЧОМУ ЦЕ СТРУКТУРА, А НЕ СМАК. Стан ліміту суми стояв блоковим підписом
  // ПІД сумою боргу, і саме він робив клітинку 150px там, де даних на 18.
  // Заміряно перед проходом: рядок клієнта 117..156 при цільових 48.
  const sec = strip(readFileSync(FE("pages/dashboard/sections/ReceivablesSection.tsx"), "utf8"));
  const head = sec.slice(sec.indexOf("<thead>"), sec.indexOf("</thead>"));

  // (а) колонка існує в шапці — і поруч із «Ліміт днів», а не абиде.
  // ⚠️ БЕЗ `\b` — І ЦЕ НЕ НЕДБАЛІСТЬ. `\b` у JS рахує межу слова за ASCII, тож
  // після кирилічної «и» межі НЕМАЄ ВЗАГАЛІ, і перевірка мовчки не знаходить
  // нічого. Спіймано власним прогоном: гейт червонів на правильній шапці.
  assert.ok(/>\s*Ліміт суми\s/.test(head), "🔴 колонки «Ліміт суми» немає в шапці");
  assert.ok(/>\s*Ліміт днів\s/.test(head),
    "🔴 сусідня колонка знову зветься просто «Ліміт» — два незалежні ліміти під однією назвою читаються як один");
  assert.ok(head.indexOf("Ліміт днів") < head.indexOf("Ліміт суми"),
    "🔴 порядок колонок розійшовся з макетом: спершу дні, потім сума");

  const rowStart = sec.indexOf('className="recv-row"');
  const rowEnd = sec.indexOf("{renderInvoices(c.clientKey");
  assert.ok(rowStart > 0 && rowEnd > rowStart, "🔴 рядок клієнта не знайдено — гейт міряє порожнечу");
  const cells = sec.slice(rowStart, rowEnd).split(/<td\b/);

  // (б) 🔴 ГОЛОВНЕ: у клітинці БОРГУ стану ліміту суми більше НЕМАЄ. Без цієї
  //     половини гейт зеленів би на дублі — колонка з'явилась, а другий рядок
  //     лишився, і висота не змінилась би ані на піксель.
  const debtCell = cells.find((c) => c.includes("formatAmount(c.amount)"));
  assert.ok(debtCell, "🔴 клітинку суми боргу не знайдено");
  assert.ok(!/amountLimitLabel|amountLimitState|isOverAmount/.test(debtCell!),
    "🔴 стан ліміту суми ПОВЕРНУВСЯ в клітинку боргу — це і є той другий рядок, що тримав висоту 150px");

  // (в) і він таки є у СВОЇЙ клітинці — інакше (б) зеленів би на видаленні.
  const amtCell = cells.find((c) => c.includes("Змінити ліміт суми"));
  assert.ok(amtCell, "🔴 клітинки ліміту суми немає — колонку оголосили в шапці й не намалювали");
  assert.ok(/amountLimitLabel\(c\.limitAmount\)/.test(amtCell!),
    "🔴 у власній клітинці ліміту суми немає значення");
  // Три стани, не два: `null` не сміє читатись як нуль.
  assert.ok(/amountLimitState\(c\.limitAmount\) === "agreed"/.test(amtCell!),
    "🔴 зникла гілка «не задано» — пара «в межах / переліміт» збрехала б там, де ліміту не ставили");
});
