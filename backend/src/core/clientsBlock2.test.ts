 ...
 *  B готівка, ще не оплачена: БЕЗ етапу 4, «Авто працює» 10.09 → рахується 10.09 (буквальне ТЗ дало б 0)
...
test("#717 ФАКТ З РАХУНКУ: готівка з наступного етапу, програні — ні, перший вхід один раз, 142 без подій — за закриттям", async (t) => {
  ...
  try {
    ...
    // B ще НЕ успішна: інакше дата закриття підхопила б її і без етапу рахунку, і гейт не відрізнив би
    // «з рахунку і далі» від буквального «лише етап 4» (спіймано саботажем S1 24.09.2026).
    await deal(2, "kb", 69716312, 2000, null); await ev(2, 69716300, "2026-09-10T09:00:00Z");
    ...
    assert.deepEqual([...succ.keys()].sort(), ["kd", "kf"], "контроль: ① рахує інакше — інакше порівнювати нема з чим");
