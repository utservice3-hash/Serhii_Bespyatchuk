...
import { aliasConflictText } from "./mergeConflict.js";
...
/**
 * #248 — ОБʼЄДНАННЯ Є ПʼЯТИМ МІСЦЕМ ОДНОГО ВИРАЗУ СКОУПУ, І ПРАВО ТАМ ПЕРШЕ.
 *
 * `#199cg` тримає чотири місця й лишається чотирма навмисно: перейменувати його
 * заради «пʼяти» означало б, що в порівнянні прогонів (`deploy:check` рахує
 * приріст по ІМЕНАХ) гейт покажеться ЗНИКЛИМ там, де нічого не ламали. Заміряно
 * на собі 27.08.2026 — саме так це й виглядало.
 *
 * Тому пʼяте місце стоїть окремо. І воно не копія: у цього роута є те, чого в
 * решти чотирьох немає — гейт ПРАВА перед скоупом. Порядок між ними не
 * косметика: валідація тіла або похід у БД перед правом дали б 400/500 замість
 * 403 і зламали б гарантію матриці «403 == спрацював гейт» (на цьому вже раз
 * відкочували прод, 04.08.2026).
 *
 * ⚠️ ЧЕСНА МЕЖА: усі ролі з `merge_receivables` (КВП/СЕО/ОД/адмін) мають ПОВНИЙ
 * скоуп, тож СЬОГОДНІ ця межа нікого не зупиняє. Вона не заради сьогодні:
 * щойно право дістане тімлід, скоуп звузиться сам, а не чекатиме, поки хтось
 * згадає дописати фільтр.
 */
test("#248 у обʼєднанні право стоїть ПЕРЕД скоупом і перед будь-яким походом у БД", () => {
  ...
  // 🔗 ТОЙ САМИЙ ВИРАЗ, ЩО В РЕШТИ ЧОТИРЬОХ — і жодного власного фільтра поруч.
  // Дефект 27.08.2026 виник не з помилки в скоупі, а з того, що виразів було
  // ДВА: обидва працювали, разом давали «рядок 2 323 000 ₴ проти розкриття
  // 691 000 ₴».
  assert.match(body, /metrics\.receivablesByClient\(/,
    "🔴 обʼєднання рахує «мої клієнти» власним виразом — другий вираз поруч із першим");
  assert.doesNotMatch(body, /ri\.manager_id\s*=\s*\$/,
    "🔴 у обʼєднанні зʼявився власний фільтр по менеджеру рахунка");
  assert.doesNotMatch(body, /\bm\.team_id\s*=\s*\$/,
    "🔴 у обʼєднанні зʼявився власний фільтр по команді");
...
/**
 * #249 — РЕЄСТР ПСЕВДОНІМІВ НЕ КОШТУЄ ОКРЕМОГО ПОХОДУ В БД.
 *
 * 🔴 ЦЕЙ ГЕЙТ НАПИСАНО ПІСЛЯ ВЛАСНОГО РЕГРЕСУ, І ЦЕ ВАЖЛИВІШЕ ЗА НЬОГО САМОГО.
 * Викат `d6f7edf` поїхав у прод із `canonicalOf` ОКРЕМИМ запитом — `/receivables`
 * став робити 5 походів проти стелі 4, і `#158` почервонів у `test:prod` уже НА
 * ПРОДІ. Спокуса була рівно та, від якої `#158` і застерігає у власному тексті:
 * «підняти стелю щоб пройшло». Стелю не рухали — запит згорнули в наявний.
 *
 * ⚠️ `#158` ловить це числом і без мене. Цей гейт ловить ПРИЧИНУ: він каже, що
 * реєстр мусить приїжджати разом із `synced_at`, тобто пояснює наступному, ЧОМУ
 * два неспоріднені факти лежать в одному запиті. Без такого пояснення перший,
 * хто «наведе лад», розділить їх назад і поверне регрес.
 */
test("#249 реєстр псевдонімів їде тим самим запитом, що synced_at — і лише merge-ролі", () => {
  const src = readFileSync(SRC("routes/dashboard.ts"), "utf8");
  ...
  // 🔴 ОДИН ЗАПИТ, А НЕ ДВА: реєстр — підзапит усередині того, що бере synced_at.
  assert.match(body, /MAX\(r\.synced_at\)[\s\S]{0,400}?client_key_alias/,
    "🔴 реєстр псевдонімів поїхав ОКРЕМИМ запитом — це +30 мс RTT на кожне відкриття "
    + "списку і 5-й похід проти стелі #158. Саме цей регрес червонів на проді 27.08.2026.");
  assert.doesNotMatch(body, /await pool\.query[\s\S]{0,80}?SELECT canonical_key, COUNT/,
    "🔴 повернувся окремий запит по реєстру");

  // 🔒 І лише тому, хто має право зливати. Дзеркало нижче не дає «сховати всім».
  assert.match(body, /roleHasPerm\(auth\.roleKey, "merge_receivables"\) \? \{ canonicalOf \}/,
    "🔴 реєстр їде всім ролям — поле, яке одного дня опиниться на екрані, для якого його не робили");
  // 🪞 ДЗЕРКАЛО: поле взагалі існує. Без цього «нікому не віддавати» було б
  // зеленим, а діалог обʼєднання втратив би позначку ланцюжка мовчки.
...
/* ────────────────── ДВЕРІ «КЛІЄНТИ»: ТРАНЗАКЦІЯ Й ПРИЧИНА ────────────────── */

function clientsMergeRouteBody(): string {
  const src = readFileSync(SRC("routes/dashboard.ts"), "utf8");
  const from = src.indexOf('dashboardRouter.post("/client-merge", async');
  assert.ok(from > 0, "🔴 роут обʼєднання клієнтів не знайдено — гейт міряє порожнечу");
  // Межа ЗМІСТОВА (наступний роут), а не «N рядків»: зріз за довжиною поїхав би
  // від чужого коментаря — правило 9 у CLAUDE.md.
  const to = src.indexOf("dashboardRouter.", from + 40);
  return src.slice(from, to > from ? to : undefined);
}

/**
 * #422c — ДВЕРІ «КЛІЄНТИ» ТЕЖ ПИШУТЬ У РЕЄСТР І ПЕРЕРАХОВУЮТЬ В ОДНІЙ ТРАНЗАКЦІЇ.
 *
 * 🔴 `#244` вилікував рівно це для дверей ДЕБІТОРКИ ще 01.09.2026, а двері
 * «Клієнти» лишались нетранзакційними до 15.09.2026: `INSERT` і перерахунок були
 * двома окремими запитами, тож падіння перерахунку лишало псевдонім у реєстрі при
 * старих даних — дія ставала НЕЗАВЕРШЕНОЮ і НЕПОВТОРЮВАНОЮ одночасно, і наступна
 * спроба діставала те саме 409. Тобто до 409 вело ДВА незалежні шляхи, і фікс лише
 * в реєстрі лишив би другий відкритим.
 *
 * Червоніє, якщо винести вставку або перерахунок за межі транзакції.
 */
test("#422c ДВЕРІ «КЛІЄНТИ»: реєстр і перерахунок в ОДНІЙ транзакції", () => {
  const body = clientsMergeRouteBody();
  const begin = body.indexOf('query("BEGIN")');
  const alias = body.indexOf("INSERT INTO client_key_alias");
  const recompute = body.indexOf("RECOMPUTE_SQL");
  const commit = body.indexOf('query("COMMIT")');
  assert.ok(begin > 0 && alias > 0 && commit > 0, "🔴 транзакції або вставки в реєстр більше немає");
  assert.ok(alias > begin && alias < commit,
    "🔴 ЗАПИС У РЕЄСТР ПОЗА ТРАНЗАКЦІЄЮ: падіння перерахунку лишить псевдонім у реєстрі, "
    + "і повторна спроба дістане 409 — дефект, який #244 уже лікував для дверей дебіторки");
  assert.ok(recompute > begin && recompute < commit,
    "🔴 ПЕРЕРАХУНОК ПОЗА ТРАНЗАКЦІЄЮ: реєстр і дані розʼїдуться при падінні");
  assert.match(body, /ROLLBACK/, "🔴 зник відкіт — часткове злиття лишалось би в базі");
});

/**
 * #422d — ВІДМОВА НАЗИВАЄ ОДНУ ПРИЧИНУ, А НЕ ТРИ.
 *
 * До 15.09.2026 будь-який `duplicate key` давав текст «Такий псевдонім уже є в
 * реєстрі», під яким жили ТРИ різні відмови, і для двох він брехав: коли клієнти
 * вже обʼєднані саме так; коли псевдонім активний під ІНШИМ канонічним (треба
 * розʼєднати там, і саме цього текст не казав); коли рядок скасований — тоді
 * текст був неправдою повністю. Правило 3 з `CLAUDE.md`: стан, що стверджує
 * причину, не може бути смітником для кількох відмов.
 *
 * Червоніє, якщо звести гілки назад до одного тексту або загубити імена ключів.
 */
test("#422d ВІДМОВА НАЗИВАЄ ОДНУ ПРИЧИНУ: три стани — три різні тексти", () => {
  const same = aliasConflictText({ aliasKey: "смар", canonicalKey: "смартекс" }, "смартекс");
  const other = aliasConflictText({ aliasKey: "смар", canonicalKey: "іншафірма" }, "смартекс");
  const none = aliasConflictText(null, "смартекс");
  assert.notEqual(same, other, "🔴 «вже обʼєднані» і «веде до іншого» дають один текст — смітник повернувся");
  assert.notEqual(other, none, "🔴 «веде до іншого» і «конфлікт без активного рядка» злились");
  assert.notEqual(same, none, "🔴 два різні стани під одним текстом");
  // Відповідь мусить НАЗИВАТИ, що саме розʼєднувати: двері дебіторки шлють кілька
  // псевдонімів за раз, і без імені людина не знає, котрий саме заважає.
  assert.match(other, /смар/, "🔴 текст не називає псевдонім, який конфліктує");
  assert.match(other, /іншафірма/, "🔴 текст не називає, куди псевдонім веде зараз");
  assert.doesNotMatch(same, /розʼєднайте/,
    "🔴 «вже обʼєднані» радить розʼєднувати — це порада зламати те, що й так правильне");
});
