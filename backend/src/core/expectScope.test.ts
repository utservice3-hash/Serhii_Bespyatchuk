...
/**
 * 🔭 #272–#272c — ТРИ ЧИТАЧІ ЗОНИ ОЧІКУВАНЬ МУСЯТЬ БАЧИТИ ОДНЕ Й ТЕ САМЕ.
 *
 ...
 * поіменно, а живий стан прода стереже окремий будильник `#272c`.
 */
...
test("#272 РОЗБІЖНІСТЬ ІМЕНУЄТЬСЯ ПОКЛАСОВО, і кожен клас має СВОЮ причину", () => {
  /**
...
test("#272b 🪞 ДЗЕРКАЛО: три читачі розходяться САМЕ на цих угодах, і сходяться без них", () => {
  /**
   * 🔴 БЕЗ ЦІЄЇ ПОЛОВИНИ ГЕЙТ БЕЗЗУБИЙ. `divergenceKlass` можна зробити «завжди null» —
   * і `#272` вище почервоніє, але це доводить лише, що функція щось повертає. Тут
...
/**
 * #272c — БУДИЛЬНИК НА ЖИВОМУ ПРОДІ: плитка «Очікуємо» на КВП == Σ рядків під нею.
 *
 ...
 * лишає `#272c` зеленим на живих даних, бо їх сьогодні й немає, — і саме тому
 * властивість тримає фікстура `#272`/`#272b`, а не цей будильник.
 */
test("#272c ЖИВА ЗОНА: три читачі очікувань сходяться, і розбіжність названа поіменно", needsBackendEnv(), async (t) => {
...
/**
 * #272d — ПЛИТКА, ЯКУ ПЕРІОД НЕ ЗВУЖУЄ, МУСИТЬ БУТИ ПІДПИСАНА ЯК ТАКА.
 *
 ...
 */
test("#272d плитка «Очікуємо» на КВП підписана скоупом, і він НЕ «за період»", () => {
...
/**
 * #273 — ФІЛЬТР ЕКРАНА ДОХОДИТЬ ДО ЯДРА, І ПОРОЖНІЙ ФІЛЬТР ВІДДАЄ КОМПАНІЮ.
 *
 ...
 */
test("#273 ФІЛЬТР ДОХОДИТЬ: звуження ≠ компанія, а порожній фільтр == компанія", () => {
...
test("#273b 🪞 ВІДСТАВАННЯ Σ КОМАНД ВІД КОМПАНІЇ НАЗВАНЕ ЯВНО, а не списане на скоуп", () => {
  /**
...
/**
 * #273c — ЖИВА ПЕРЕВІРКА, ЩО МОДЕЛЬ СКОУПУ ВІДПОВІДАЄ СПРАВЖНЬОМУ SQL.
 *
 * `#273` доводить властивість на фікстурі; сам по собі він лишився б зеленим, якби
 ...
 *    команду → червоне.
 */
test("#273c ЖИВЕ ЯДРО: звуження по команді == рядок тієї команди, а без фільтра — усі", needsBackendEnv(), async (t) => {
  const metrics = await import("./metrics.js");
