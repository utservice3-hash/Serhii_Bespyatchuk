 ...
 */
/**
 * 🔴 ДЖЕРЕЛО ГРОШОВОЇ КАСИ — ЕКСПОРТОВАНЕ, бо в нього ДВА споживачі.
 *
 * Число рахує `money`, а СКЛАД цього числа показує `core/dayItems`. Поки склад
 * будувався власним запитом, вони розходились: `dayItems` брав угоду за подією
 * входу в БУДЬ-ЯКИЙ статус каси, а тут анкер у кожної гілки СВІЙ — успішні по
 * `closed_at_kommo`, оплачені по ОСТАННЬОМУ входу в етап 9. Угода, що 05.08
 * побувала в 142, а 10.08 стала «оплата отримана», через це з'являлась у
 * розкритті 05.08, хоч каса рахує її 10.08.
 *
 * 📐 Заміряно 20.08.2026 на проді: розійшлось 13 із 68 днів (19%), розкриття
 * показувало +20 угод і +96 840 ₴ проти чисел, які пояснювало. Угода-доказ —
 * `62551669`.
 *
 * Тепер вираз ОДИН на обох. Дописати гілку в одну копію і забути другу вже
 * неможливо — той самий прийом, що `createdKlassCase` і `stuckSignals`.
 */
export function moneySourceSql(kind: Kind, p: unknown[]): string {
  return sourceSql(kind, p);
}
export type MoneyKind = Kind;
 ...
 *   revenue/deals   — ті самі, що `receivedByMgr` (тримає `#600`);
...
async function snapshotBy(stages: number[], s: MoneyScope, extraSelect: string, groupBy: string): Promise<Record<string, string>[]> {
  ...
  const conds = [
    ...
    //
    // 🔴 МЕЖА — ПРАВИЛО ВЛАСНИКА, А НЕ МОЯ ОБЕРЕЖНІСТЬ (26.08.2026).
    //
    // Питання було поставлене дослівно: чи зменшується ЗАДНІМ ЧИСЛОМ факт
    // менеджера за виграну угоду, якщо борг по ній списали. Відповідь власника
    // дослівно: **«Ні, факт лишається.»**
    //
    // Отже: предикат накриває ЛИШЕ знімок `CHAIN_INFLIGHT` — ОЧІКУВАНУ половину
    // чека. Пул `success` (142) не накривається НІ ЗАРАЗ, НІ ПОТІМ без окремого
    // рішення власника. **Списання прощає БОРГ, а не скасовує виграну угоду.**
    // За цим стоять закриті місяці, KPI й уже виплачені премії — вони заднім
    // числом не рухаються.
    //
    // ⚠️ ЧОМУ ЦЕ ЗАПИСАНО ТУТ, А НЕ В ДОКУМЕНТАЦІЇ. Рішення про списання
    // сформульоване словами «вона зникає з усіх етапів і екранів у дашборді».
    // Прочитавши їх без цього абзацу, наступний розширить предикат на `success`
    // і вважатиме, що ДОРОБЛЯЄ незакінчене, — а насправді зрушить факт Звіту,
    // «Виконано плану» і премії за закриті місяці. Тримає `#199ap`.
