...
/**
 * 🗂 ВКЛАДКИ РОЛІ — ОДИН ВИРАЗ НА ВЕСЬ ПРОДУКТ.
 *
 * 📐 Заміряно 09.09.2026: цей вираз існував РІВНО ОДИН раз — вбудовано й неекспортовано,
 * у `routes/auth.ts` при логіні. Другий читач (фільтр адресних новин) мав би написати
 * його заново — і це дослівно та помилка, що вже коштувала проєкту проходу: «правило
 * класифікації існувало тричі, і копії збігались одна з одною, а не з правилом».
 *
 * 🔴 FAIL-CLOSED, ЯК І СУСІДИ. Порожній кеш або невідома роль → **порожній список**, а не
 * «всі вкладки». Наслідок названий вголос: читач у такому стані побачить лише новини БЕЗ
 * адресата. Це збіднення, а не витік — і воно збігається з `roleHasTab`, який у тій самій
 * ситуації віддає `false`.
 *
 * ⚠️ НЕ ПЛУТАТИ З `screens` У ТОКЕНІ. Там лежить знімок, узятий У МИТЬ ЛОГІНУ, з TTL 12
 * годин і без ендпоінта оновлення; у самому `auth.ts` він підписаний «ЛИШЕ для косметики
 * nav у FE». Фільтр, збудований із токена, відставав би до півдоби після зміни ролі.
 * Ця функція читає ЖИВИЙ кеш, тож зміна тумблера діє без перелогіну.
 */
export function tabsOfRole(key: string): string[] {
  const def = cache.get(key);
  return def ? Object.keys(def.screenAccess).filter((k) => def.screenAccess[k] === true) : [];
}
