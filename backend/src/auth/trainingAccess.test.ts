...
test("#443 КАНДИДАТ: рівно дві вкладки — навчання і документи (щоб підписати офер до підвищення), роль оголошена найвужчим обсягом", () => {
  ...
  // бачить Навчання і Документи, підписує там свій офер, потім його підвищують до менеджера.
  ...
  // Сід у схемі мусить давати РІВНО дві вкладки. Порівнюємо з текстом схеми, а не з
  ...
  assert.match(seed[0], /'\{"training":true,"documents":true\}'::jsonb/,
    "🔴 кандидат отримав не рівно дві вкладки (навчання + документи) — рішення власника 15.09.2026");
  assert.match(seed[0], /'\{\}'::jsonb/, "🔴 кандидат отримав права — їх у нього бути не повинно");
  // Міграція для ВЖЕ заведеної ролі: сід із ON CONFLICT не оновить наявний рядок, тож вкладку
  // документів мусить додати окремий UPDATE — інакше на проді кандидат лишиться з однією.
  ...
  assert.deepEqual(tabsForPath("/api/documents/tree"), ["documents"],
