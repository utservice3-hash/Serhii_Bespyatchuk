...
/**
 * 🎓 #399–#401 — КРОК 1 ТЗ «НАВЧАННЯ»: РОЛЬ «КАНДИДАТ» І ПРАВО `manage_training`.
 *
 ...
 *
 * ⚠️ Це не робить живу пробу зайвою — і причина названа в `#401`.
 */
...
test("#399 КАНДИДАТ: єдиний екран — навчання, і роль оголошена найвужчим обсягом", () => {
...
test("#400 ПРАВО `manage_training` ІСНУЄ, ЛЕЖИТЬ У КАТАЛОЗІ Й ВИДАНЕ РІВНО ЧОТИРЬОМ", () => {
...
test("#401 🪞 ЗЛІПОК ЗНАЄ ПРО ЗВУЖЕННЯ: фінансист у deny КОЖНОГО роуту запису навчання", () => {
  const write = ACCESS_MATRIX.filter((r) => r.path.startsWith("/api/training") && r.method !== "GET");
