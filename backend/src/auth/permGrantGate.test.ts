...
/**
 * 🔁 #380i — ЗАМІНА `#231g`, і причина заміни в самому імені.
 *
 * `#231g` стверджував «ТРИ двері», і 09.09.2026 їх стало ЧОТИРИ: додано `/analytics`
 * («Коротка аналітика»). Гейт спрацював правильно — саме за цим він і стояв. Але
 * твердження змінилось, а за правилом 13 із CLAUDE.md назву гейта не уточнюють:
 * змінилось твердження — новий номер, старий у `RETIRED_GATES`.
 */
test("#380i порожньо ≠ відмова: чотири двері 1×1 відповідають словами", () => {
  ...
  assert.equal(calls, 4,
    `🔴 предикат кличеться ${calls} раз(и) замість 4. Двері: /meetings/:type/:managerId,\n` +
    "   /stats/scores, /enps, /analytics. `/conduct-types` НАВМИСНО лишається 200 — це\n" +
    "   довідка, з якої фронт дізнається, що людині доступно.");
});
