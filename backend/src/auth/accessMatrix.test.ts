...
/**
 * 📣 #368/#368b — ХТО БАЧИТЬ «РЕКЛАМУ» (рішення власника 08.09.2026, дослівно:
 * «всі в кого є адмін, квп, всі керівники»).
 *
 * 🔴 ЧОМУ ЦЕ ОКРЕМИЙ ГЕЙТ, ХОЧ Є `#280`. Той стереже, що роут ВЗАГАЛІ є в матриці —
 * тобто що ми про нього не забули. Він мовчить про те, КОМУ саме відкрито, а рішення
 * власника тут не механічна копія `/lead-quality`: `financier` у формулювання
 * «адмін, квп, керівники» не входить, і без гейта ця свідома різниця через місяць
 * читалась би як недогляд і «полагодилась» би назад.
 *
 * 🔴 І ДРУГА ПОЛОВИНА (#368b) ОБОВʼЯЗКОВА: сам лише `deny` зеленіє й тоді, коли роут
 * закритий геть усім — тобто коли екран мертвий. Тому парне твердження про `allow`.
 *
 * 🧨 САБОТАЖ: додати `financier` в `allow` → червоніє #368; прибрати `kvp` з `allow`
 * → червоніє #368b.
 */
test("#368 «Реклама» ЗАКРИТА для менеджера, HR і фінансиста", () => {
  ...
  for (const role of ["manager", "hr", "financier"]) {
    assert.ok(row!.deny.includes(role),
      `🔴 роль «${role}» більше не в deny — рішення власника було «адмін, квп, керівники», ` +
      "а фінансист і менеджер туди не входять");
...
test("#368b 🪞 …І ВІДКРИТА адміну, КВП і всім керівникам", () => {
  ...
  for (const role of ["admin", "kvp", "ceo", "opdir", "team_lead"]) {
    assert.ok(row.allow.includes(role),
      `🔴 роль «${role}» втратила доступ до «Реклами» — без цієї половини гейт #368 ` +
