...
test("#5.13 🔴 HR НЕ успадкував нічого з того, що отримав фінансист", needsDb(), async () => {
  ...
  // 🟢 `goals` додано 10.08.2026 — доступ HR до екрана «Цілі» дав ВЛАСНИК свідомо.
  // Еталон оновлено ПІСЛЯ його підтвердження, а не «щоб приймання позеленіло»:
  // цей гейт на те й стоїть, щоб тиха зміна політики ставала видимою. Наступна
  // зміна цього списку так само має початися з питання, а не з правки тесту.
  // 🟢 `hiring` додано 17.09.2026 — розділ «Найм». Питання поставлено ДО коду: рекрутер Іван
  // працює з роллю HR (рішення Романа 17.09, план проходу 1 затверджено з цим переліком ролей).
  // Приймання викату 5ae829d почервоніло саме тут — гейт спрацював як задумано.
  assert.deepEqual(screens,
    ["documents", "duty", "feedback", "goals", "hiring", "messenger", "news", "oneonone", "tasks", "training"],
