...
export const CORE_BYPASS_EXEMPTIONS: CoreExemption[] = [
  { file: "routes/dashboard.ts",
    why: "БОРГ, що ХУДНЕ. 02.08.2026 з нього прибрано ГРОШОВИЙ анкер по `created_at` у "
       + "п'яти місцях (/overview «Історія за 3 місяці» + «Виконано плану», /personal "
       + "місяць/поденно/12міс, /loyalty динаміка, /managers тижні, plans-grid, "
       + "report-plan) — усі тепер кличуть `core/money.ts`. Лишається 93 збіги детектора "
       + "у 22 роутах, і БІЛЬШІСТЬ із них — НЕ гроші періоду: `funnel_stage='paid'` там "
       + "означає «клієнт колись платив» (lifetime-предикат для сегментів, реактивації, "
       + "постійних). Детектор їх не розрізняє, тому файл лишається у реєстрі. Зняти "
       + "виняток можна лише роут за роутом, з окремою звіркою кожної цифри." },
