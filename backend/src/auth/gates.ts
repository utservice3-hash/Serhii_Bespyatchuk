...
export const ROUTE_BOUNDARY_EXEMPTIONS: RouteExemption[] = [
  { method: "GET", path: "/api/people/photo/:employeeId", permanent: true,
    why: "Фото співробітника — на слайдах зустрічі, у номінаціях і (далі) на дошці пошани для всієї команди. Лише "
       + "залогіненим (requireAuth), крім ролі «Кандидат» — їй 403 в самому обробнику; файл береться за імʼям із "
       + "бази, не з запиту. Список і запис — під правом сейфу." },
  ...
  { method: "GET", path: "/api/auth/tracker-users", permanent: true,
    why: "Список людей для трекера. Стукає його сервер, не браузер: обліковка — ключ "
       + "X-Dashboard-Sso-Key, вкладки немає й бути не може. Віддає лише особу й підказки; "
       + "доступу в трекері не надає — там акаунти вмикають руками." },
...
export const ROW_SPREAD_EXEMPTIONS: SpreadExemption[] = [
  ...
  { file: "routes/nominations.ts", frag: "({ ...row, canEditRow",
