...
test("МАНІФЕСТ: динамічні тести безпеки розгортаються по всіх таблицях", () => {
  ...
  /**
   * 🔴 І ГОЛОВНЕ — ЦИКЛ МУСИТЬ ІТЕРУВАТИ САМЕ `FORBIDDEN_TABLES`, А НЕ СВІЙ ЛІТЕРАЛ.
   *
   * Без цього рядка перевірка вище була ПІВСПРАВОЮ, і це заміряно на прийманні
   * викату `ed7f6be` 14.09.2026: у циклі стояв зашитий масив із 8 назв — ТРЕТЯ
   * копія переліку. Додавши 5 таблиць у `FORBIDDEN_TABLES` і в маніфест, ми
   * отримали НЕДОБІР рівно на 5 (оголошено 13 динамічних, народилось 8), а цей
   * гейт лишився зеленим: він звіряв ДВА списки з ТРЬОХ, і вони збігались один з
   * одним, а не з джерелом. Рівно та форма, що дала «чипи новий/постійний».
   *
   * 🧨 Червоніє, якщо повернути в цикл літерал замість джерела.
   */
  assert.match(src, /for\s*\(\s*const\s+table\s+of\s+MANIFEST_SECURITY_TABLES\s*\)/,
    "🔴 ЦИКЛ ІТЕРУЄ НЕ ДЖЕРЕЛО. Динамічні тести безпеки мусять народжуватись із "
    + "`MANIFEST_SECURITY_TABLES` (а той, перевіркою вище, дорівнює `FORBIDDEN_TABLES` "
    + "ядра). Будь-який власний перелік у циклі — третя копія, і розходження з нею дає "
    + "НЕДОБІР на прийманні, а не червоний тест.\n"
    + "⚠️ І саме `MANIFEST_SECURITY_TABLES`, не `FORBIDDEN_TABLES` напряму: другий тягне "
    + "`config.js`, який кидає без `DATABASE_URL` НА ІМПОРТІ й убиває весь файл.");
});
