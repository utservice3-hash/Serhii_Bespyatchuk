...
export function ReportSection({
  ...
  const kpis = s
    ? [
        ...
        { label: "Озвучено ціну в перший дотик", value: s.adPriceVoiced.toLocaleString("uk-UA"), sub: s.adLeads > 0 ? `${Math.round((s.adPriceVoiced / s.adLeads) * 100)}% реклами` : "", color: "#6366f1", hint: "Рекламні угоди, у яких менеджер озвучив ціну (вхід у «Пропозицію зроблено») у ТОЙ САМИЙ день, що й перший контакт (перший дзвінок/повідомлення). Тобто ціну дано на першому дотику. Знаменник — уся реклама періоду." },
