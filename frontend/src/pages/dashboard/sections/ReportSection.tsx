...
export function ReportSection({
  ...
  const kpis = s
    ? [
        ...
        { label: "Оплата отримана (знімок)", value: formatAmount(s.paymentRevenue), sub: `${s.paymentDeals} угод`, color: "#16a34a", hint: "Угоди, що ЗАРАЗ на етапі «Оплата отримана» (знімок поточного стану, без фільтра дати)." },
        { label: "⏳ Очікувані оплати", value: formatAmount(s.expected), sub: "грошова зона", color: "#b45309", hint: "Повна грошова зона (5 стадій core, expectedPaymentsByPlanned) — знімок «зараз», як на Огляді/Звіті 2.0. Декомпозується на менеджерів (Σ = відділ). Ще НЕ в «Отриманих коштах»." },
        { label: "Середній чек", value: formatAmountFull(s.avgCheck), sub: "", hint: "Успішно реалізовано ÷ Поставлені машини — один якір (перехід в успіх), METRICS_GLOSSARY §5 (уточнено 13.07)." },
        { label: "Прийнято реклами", value: s.adLeads.toLocaleString("uk-UA"), sub: "", hint: "Рекламні угоди повного циклу (Источник клиента ∈ adSources АБО дотик у Кваліфікації, крім лідогену/реактивації), створені в періоді." },
        { label: "Озвучено ціну в перший дотик", value: s.adPriceVoiced.toLocaleString("uk-UA"), sub: s.adFirstTouchAnalyzed > 0 ? `${Math.round((s.adPriceVoiced / s.adFirstTouchAnalyzed) * 100)}% з ${s.adFirstTouchAnalyzed} дотиків` : "немає даних", color: "#6366f1", hint: "Джерело — автоматизація AI-транскрибації першого дзвінка менеджера з лідом (пуш «ціну озвучено»). Кількість перших дотиків, у яких ціну озвучено, за період. Підпис — % озвучень від усіх проаналізованих перших дотиків." },
        ...
        { label: "Поставлені машини", value: s.dispatched.toLocaleString("uk-UA"), sub: formatAmount(s.dispatchedSum), hint: "Угоди, що ПЕРЕЙШЛИ в «Успішно реалізовано» в періоді — якір ПРОДАЖІ (Правило №1 словника). Той самий якір, що й дохід." },
