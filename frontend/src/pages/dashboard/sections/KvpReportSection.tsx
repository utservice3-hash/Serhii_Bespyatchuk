...
/** Порядок і кольори чотирьох відер зони очікування. Імена — з ядра (`core/expectSplit.ts`);
 *  тут лише те, чого ядро не знає: у якому порядку показати й яким кольором. */
const EXPECT_BUCKET_ORDER = [
  ["overdue", "прострочено", "#dc2626"],
  ["thisMonth", "цього місяця", "#0369a1"],
  ["later", "пізніше", "#6b7280"],
  ["noDate", "без дати", "#6b7280"],
] as const;
...
const HINT: Record<string, string> = {
  ...
  awaiting: "Очікуємо = УСЯ зона визнання доходу (виставлено рахунок→оплата), знімок «станом на зараз». "
    + "🔴 ПЕРІОД НА НЕЇ НЕ ДІЄ: це не «очікуємо за вибраний період», а вся зона цілком. Тому число не "
    + "змінюється від перемикача періоду, і в жодну виручку періоду воно НЕ входить. "
    + "РОЗБИТО ЗА ДАТОЮ ОЧІКУВАНОГО ПЛАТЕЖУ на чотири частини: прострочено (дата вже минула) · цього "
    + "місяця · пізніше · без дати. Межа «прострочено» — СЬОГОДНІ по-київськи, а не назва місяця, тому "
    + "числа змінюються щодня самі собою. Сума чотирьох дорівнює підсумку точно. "
          ...
          <div className="chart-card" style={{ marginBottom: 16, borderTop: "3px solid #c5141c" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              ...
              {([["Відправлено", "за період", v.lifecycle.sent, BLUE, HINT.sent],
                 ["Очікуємо", "уся зона · за датою очікуваного платежу", v.lifecycle.awaiting, AMBER, HINT.awaiting],
                ...
                <div key={lbl} style={{ padding: 10, background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 10 }}>
                  ...
                  {/* 🔴 РОЗБИВКА ЗОНИ — ЧОТИРИ ВІДРА, І ПОРОЖНЄ ТЕЖ ПОКАЗУЄТЬСЯ.
                      «Без дати» сьогодні нуль, але сховати його означало б показати
                      три числа замість чотирьох і зламати очевидність партиції:
                      читач мусить бачити, що сума частин дорівнює підсумку. */}
                  {"split" in agg && (
                    <div style={{ marginTop: 5, display: "grid", gap: 2 }}>
                      {EXPECT_BUCKET_ORDER.map(([bk, ttl, c]) => (
                        <div key={bk} style={{ display: "flex", justifyContent: "space-between", fontSize: 10.5 }}>
                          <span style={{ color: c }}>{ttl}</span>
                          <span style={{ color: MUTED }}>
                            {agg.split[bk].deals} · {fmtMoney(agg.split[bk].sum)}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table className="data-table" style={{ width: "100%", tableLayout: "fixed", minWidth: 640 + rep.weekBlocks.length * WK_W }}>
                ...
                <thead><tr>
                  ...
                  <th style={{ textAlign: "right" }}>План</th><th style={{ textAlign: "right" }}>Факт</th><th>Вик. %</th><th style={{ textAlign: "right" }}>Очікуємо <InfoHint text="За плановою датою оплати (коли має надійти); зміна дати переносить між місяцями. Верх — цей календарний місяць, «наст» — наступний, «мин» — з МИНУЛИХ місяців (у прогноз не входить; це не «прострочено» з плитки вгорі — там межа «сьогодні»). Знімок «зараз». Наведи на клітинку — + ср.чеки команди." /></th><th style={{ textAlign: "right" }}>Конв. <InfoHint text="Лайфтайм-конверсія (весь час, чесна воронка): РНК = рекламні угоди, що досягли «авто працює» ÷ прийнята реклама; РПК = лідген-угоди, що досягли «авто працює» ÷ лідген-заявки. Команда = Σчисельників÷Σзнаменників. Тонкий знаменник (0) → «—»." /></th>
