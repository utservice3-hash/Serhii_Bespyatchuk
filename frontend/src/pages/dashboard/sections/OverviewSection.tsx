...
const KPI_HINTS: Record<string, string> = {
  ...
  sum:
    "⚠️ ОРІЄНТИР, не оцінка роботи. Отримані кошти = «Успішно реалізовано» (142) + «Оплата отримана» — тобто ВКЛЮЧНО з оплаченими, але ще НЕ закритими угодами. "
    + "Показує, скільки коштів фактично в компанії. Результат менеджера рахується інакше — лише «успішно реалізовано» (див. «Виконано плану» і Звіт), "
    + "бо частково оплачені угоди переносяться в наступний місяць. Обидва числа правильні; вони НЕ мають збігатися.",
  convAd:
  ...
  avg:
    "Середній чек = «Успішно реалізовано» ÷ успішні угоди — один якір, перехід у 142 за датою закриття. Історія за 3 місяці рахується так само.",
  newc:
        ...
        <>
          ...
          <div className="kpi-grid">
            <ProgressGauge
              ...
              factHint="Факт = «успішно реалізовано» (статус 142, за датою закриття). Частково оплачені угоди сюди НЕ входять — вони переносяться в наступний місяць. Ширшу картину («гроші в компанії») показує картка «Отримані кошти»."
