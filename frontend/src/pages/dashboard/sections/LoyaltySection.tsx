  ...
  return (
    <>
      ...
      {/* 🪦 ПРИБРАНО 04.08.2026 (рішення власника): картки по кожному менеджеру
          ...
          (`POST /dashboard/loyalty-override`, hidden=true).
          ✅ ВХІД ПОВЕРНУТО 04.08.2026 (рішення власника): кнопка живе в КАРТЦІ
          КЛІЄНТА — там, де видно гістограму й угоди, тобто підставу для рішення.
          Право рахує сервер (`canHide`) тим самим гейтом, що стоїть на роуті.
          🔴 І ТАМ ЖЕ ЗʼЯСУВАЛОСЬ, що дія НІКОЛИ не працювала: фільтр
          `LEFT JOIN … AND NOT lo.hidden` разом із `WHERE COALESCE(lo.hidden,false)=false`
          — порожня операція (для прихованого клієнта join не дає рядка → NULL →
          умова істинна). Виправлено в обох екранах, тримає тест #25e.
          Скасувати вже наявні правки можна тут же, у блоці «🔧 Ручні правки постійних». */}
    </>
  );
}
