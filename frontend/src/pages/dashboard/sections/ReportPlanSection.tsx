    ...
    <div className="rpt-card" style={{ background: isSelf ? "var(--rpt-tint)" : undefined, borderLeft: `3px solid ${SCOL[s]}`, marginBottom: 12, overflow: "hidden" }}>
      <div onClick={onToggle} style={{ display: "grid", gridTemplateColumns: "196px 1fr 1fr 392px 26px", gap: 18, alignItems: "center", padding: "17px 20px", cursor: "pointer" }}>
        ...
        <TrajBlock title="Місяць" fact={m.fact} plan={m.plan} pct={pct} status={s} elapsed={elapsed}
          footer={<>
            <div>треба <b style={{ color: INK }}>{fmt(m.needPerDay)} ₴/д</b> ({remWd} дн.) · прогноз <b style={{ color: INK }} title="факт + зона визнання + добір нового бізнесу (як у КВП)">{k(m.projected)}</b>{m.plan > 0 && m.monthInProgress ? ` (${Math.round((m.projected / m.plan) * 100)}%)` : ""}</div>
            <div style={{ marginTop: 3, fontSize: 12.5, color: INK }} title="Очікування за ПЛАНОВОЮ датою оплати. Цей / наступний календарний місяць.">
              очікуємо <b style={{ color: GREEN }}>{k(m.expectThisMonth)}</b> цей · <b style={{ color: INK }}>{k(m.expectNextMonth)}</b> наст. міс
            </div>
          </>} showTempo />
        {/* місячні стати + spark */}
        ...
        {sectionOpen && (data == null ? <div style={{ padding: "0 22px 20px", color: MUTED }}>Завантаження…</div> : (
          <>
            ...
            <div className="rpt-strip">
              без руху ≥ 7 дн. (рахунок/гроші) · ≥ 21 дн. («взято в роботу») · всі відкриті стадії, включно з «Авто працює» · дата = остання активність
            </div>
