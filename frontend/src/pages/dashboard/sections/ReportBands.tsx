...
export const HINTS = {
  ...
  teamPct: "Відсоток команди = факт ② ÷ план команди. 🔴 План команди рахується як Σ планів "
    + "її МЕНЕДЖЕРІВ, тому гроші менеджера, якому плану не виставили, піднімають відсоток, "
    + "не піднявши знаменник. Заміряно 06.08.2026: у РПК-Яцика це +8.3 п.п. (24.9% замість 16.5%), "
    + "по компанії +2.6 п.п. Складова показана окремим рядком у колонці «Факт ②» — але лише там, "
    + "де вона не нульова. ⚠️ Після звільнення людини складова НЕ зникає: її гроші переїжджають у "
    + "рядок «звільнені», який теж входить у суму команди (перевірено симуляцією: % не змінився).",
...
/**
 * ⚠️ ЩЕ НЕ ВИКОРИСТОВУЄТЬСЯ — написано наперед під перебудову рядка менеджера в
 * таблицю (черга, `CLAUDE.md`). Позначено ЯВНО, бо мертвий код, який виглядає
 * живим, ми вже одного разу ловили: поле `expected` у `/teams` оголошувалось,
 * ініціалізувалось нулем і ніколи не заповнювалось — і при читанні коду мало
 * вигляд робочої метрики. Тут те саме, тільки зі знаком «поки що».
 *
...
/**
 * ⚠️ ЩЕ НЕ ВИКОРИСТОВУЄТЬСЯ (як і `PaceCell` вище) — і в теперішньому вигляді
 * ДЛЯ ТАБЛИЦІ НЕ ГОДИТЬСЯ: це `<div>`, а шапка групи колонок у таблиці мусить
 * бути `<th colSpan={N}>` всередині `<thead>`, інакше вона не тримається над
 * своїми колонками при горизонтальному скролі. Тобто наступного проходу це не
 * «переюзати», а переписати під табличну семантику — оцінку в черзі дано з
 * урахуванням цього.
 *
