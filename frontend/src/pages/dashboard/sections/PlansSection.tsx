...
function LeadRec({ row, loading, period, onPeriod }: {
  ...
  return (
    <div>
      ...
      {/**
        * 🚧 ПОЗНАЧКА «РОЗРАХУНОК УТОЧНЮЄТЬСЯ» (рішення власника 24.08.2026).
        *
        * 🔴 ПРИВІД, НАЗВАНИЙ ЧЕСНО. 24.08 знаменник конверсії переведено з реєстру
        * лідоген-бота на КАНАЛ (`564bee0`) — це полагодило обрізану історію, але
        * разом із джерелом змінило СЕНС: було «скільки лідів менеджеру ПЕРЕДАЛИ»,
        * стало «скільки угод каналу він СТВОРИВ», включно з тими, що він завів сам
        * постійному клієнту одразу на «Авто працює». Для РПК це дає викиди:
        * Антипенко 4.3%→71.6%, «треба лідів» 138→9; Дмитрук 0%→74.1%.
        * Плюс відкрита асиметрія: реклама рахує знаменником лише ПЛАТНІ НОВІ ліди,
        * лідген — УСІ угоди каналу.
        *
        * 🔴 ЧОМУ ПІДПИС, А НЕ ХОВАННЯ ЧИСЛА. Порожнє місце читається як «даних
        * немає», а тут дані є — ненадійне саме ЗНАЧЕННЯ. Ховати його означало б
        * приховати й те, що воно існує, а разом із ним і привід полагодити.
        *
        * ⚠️ Це ТІЛЬКИ підпис: жодне число тут не змінене й не приховане.
        */}
      <div style={{
        ...
        <span aria-hidden style={{ fontSize: 13, lineHeight: 1.45 }}>🚧</span>
        <div style={{ fontSize: 11.5, lineHeight: 1.45, color: "var(--text)" }}>
          <b style={{ color: "#d97706" }}>Розрахунок уточнюється — поки не орієнтуватись.</b>{" "}
          <span style={{ color: MUTED }}>
            Знаменник конверсії міряє <b>створені угоди каналу</b>, а не <b>отримані ліди</b>,
            тож у командах, де багато роботи з постійними клієнтами, «треба лідів» суттєво занижене.
            Числа нижче показані як є, нічого не приховано.
          </span>
        </div>
      </div>
      ...
      {loading ? <span style={{ fontSize: 12, color: MUTED }}>Завантаження…</span> : !row ? (
        ...
        <>
          <div style={{ display: "flex", gap: 22, flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            {/**
              * 🔀 ПІДПИС НАЗИВАЄ ОСНОВУ (рішення власника 21.08.2026).
              *
              * 🔴 ПІДПИС ПЕРЕПИСАНО 24.08.2026, І ЦЕ ВИПРАВЛЕННЯ НАШОЇ Ж БРЕХНІ.
              * Тут стояло «за передачами» / «переданих заявок» — правда рівно доти,
              * доки знаменник брався з реєстру лідоген-бота. Того самого дня коміт
              * `564bee0` перевів його на КАНАЛ (`lead_channel='leadgen'`, угоди
              * СТВОРЕНІ в періоді), а підпис лишився старий — тобто екран називав
              * джерелом те, чого функція вже не читає. Той самий клас, що
              * «синхронізовано із Задачником» над статичним `<span>`: підпис
              * правдоподібний, а величина за ним інша.
              *
              * 📐 Знаменник тепер ДОСЛІВНО той самий, що факт лідгену у Звіті
              * (`createdSplitByManager().leadgenCount`) — тримає гейт `#160b`. Тому
              * розриву «3.8% проти 24.8%», заради якого підпис і заводили, більше
              * НЕМАЄ: два числа на сусідніх екранах зійшлись.
              *
              * ⚠️ Змінено ТІЛЬКИ підпис: `row.conversionPct` як приходив із
              * `conversionAdsByManager`/`conversionLeadgenByManager`, так і приходить.
              */}
            {cell(row.channel === "leadgen" ? "Конверсія (за угодами лідгену)" : "Конверсія (за прийнятою рекламою)",
              <>
              ...
              row.conversionSource === "team" ? "особистих заявок замало"
                ...
                : `${row.conversionWon}/${row.conversionEntered} ${row.channel === "leadgen" ? "угод лідгену" : "прийнятих реклами"}`)}
            {cell("Ср. чек",
              <>
