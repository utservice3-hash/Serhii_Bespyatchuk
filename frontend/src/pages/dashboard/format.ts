...
/**
 * 🔴 «—» ЗАМІСТЬ ПАДІННЯ, АЛЕ НЕ ЗАМІСТЬ ГЕЙТА. Якщо поле не приїхало з API,
 * `value` = `undefined`, і `value.toFixed(0)` роняв **увесь екран** (TypeError у
 * рендері), а не одну клітинку. Так було в «Рейтингу менеджерів»: `manager.expected`
 * приходить з `/dashboard/managers`, і зникни воно — сторінка біліла б цілком.
 *
 * ⚠️ ЦЕ НЕ ФОЛБЕК, ЩО ХОВАЄ ПРОБЛЕМУ. Тихий `?? 0` намалював би «0 ₴» — тобто
 * ЗБРЕХАВ би, і відсутнє поле читалось би як «очікувань немає». «—» каже рівно те,
 * що сталось: величини НЕМАЄ (невідоме має читатись як невідоме). А щоб зникнення
 * поля не жило непоміченим на екрані, воно ловиться ОКРЕМО — гейтом `#47`, який
 * вимагає `expected` у кожному рядку відповіді. Захист від падіння і сигналізація —
 * різні механізми; підміняти другий першим і означало б зробити баг тихим.
 */
