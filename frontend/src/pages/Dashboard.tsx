...
export function Dashboard() {
  ...
  // ПРОМТ 0.9: один якір — успішна виручка періоду ÷ машини періоду (без снапшотів).
  const avgDeal = overview && (overview.dispatchedCount ?? 0) > 0
    ? overview.successRevenue / overview.dispatchedCount : 0;
  ...
  const prevAvg = prevOverview && (prevOverview.dispatchedCount ?? 0) > 0
    ? prevOverview.successRevenue / prevOverview.dispatchedCount : 0;
  ...
  return (
    ...
    >
      ...
      {section === "leadgen" && (
         ...
         * Період у екрана СВІЙ (навігатор, як у «Звіті»: день/тиждень/місяць/період), тож
         * спільний `dateRange` сюди НЕ передається — екран не залежить від чужого періоду.
         */
        <LeadgenSection />
      )}
      ...
      {section === "oneonone" && (auth?.screens?.includes("oneonone") ?? auth?.role !== "manager") && <OneOnOneSection />}
