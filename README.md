...
- Маппінг команд та воронок Kommo (`src/db/seedKommoMapping.sql`):
  - **РНК / РПК / Тендери (КВП)** — спільна воронка `Перевозки (Продажі повний цикл) (New)`
    (id `8921932`), команди розрізняються за відповідальним менеджером
  - **Лідогенератори** — воронка `Продзвін (New)` (id `8921936`)
  - Команди: Михальчевська, Безпамятний → РНК; Шаврова, Яцик, Дмитрук → РПК;
    Ковтонюк, Сердюк → Лідогенератори
...
- Підтвердити менеджерів і відповідального для команди КВП (наразі лише
  команда створена, без прив'язаних менеджерів)
- Дешборди по командах і окремих менеджерах окремо (зараз єдиний загальний графік)
