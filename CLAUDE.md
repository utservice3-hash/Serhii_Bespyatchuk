...
Зведення керівника відділу продажу. Порівняння **місяць** (цей vs минулий) і **тиждень** (цей vs минулий), план/факт/%. Фронт реюзає `/dashboard/overview` × 4 періоди (цифри збігаються з рештою) + `/dashboard/lead-quality`. Рядки: дохід (отримані/успішно/оплата/сер.чек/нові/постійні), угоди+клієнти, реклама (ліди/оплачено/конверсія/**цільові**/**не цільові**), лідоген, по командах. **Цільові ліди** = пайплайн `8921932` створені в періоді; **не цільові** = Кваліфікація `8921928` статус `143` створені в періоді. Фінансові рядки поза CRM (CASH FLOW, рекл.бюджет, тендери) — поки не показуються (за потреби ручне введення).
