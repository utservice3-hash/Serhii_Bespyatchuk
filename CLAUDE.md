...
- **Редактор планів** — розділ «Плани» (`PlansSection`, admin/team_lead). `GET /dashboard/plans-grid?month=&teamId=` → менеджери по командах з місячним планом + календар; збереження через `POST /plans`. Автодекомпозиція: тиждень = план × днів_блоку/днів_місяця; день = план ÷ робочі_дні. Підсумки по командах і відділу. Тімлід — своя команда, адмін — усі.
