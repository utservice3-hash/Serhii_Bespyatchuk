...
@app.route("/post-coo-report", methods=["GET"])
def post_coo_report():
    """ОДНОРАЗОВИЙ службовий ендпоінт: публікує звіт по рекламі (буд/вих
    економіка за 60 днів) у групу COO Board. Прибрати після використання."""
    thread = request.args.get("thread", "1")
    report = (
        "📊 <b>Реклама: економіка буднів vs вихідних (60 днів)</b>\n"
        "<i>вартість ліда 120 грн; «виручка» = маржа (бюджет у CRM)</i>\n\n"
        "<pre>"
        "               Ліди  Авто  Конв    Маржа      Витрати   Чистими     ROMI\n"
        "Будні(Пн-Пт)   1662   240  14.4%   549 708   199 440  +350 268   2.76x\n"
        "Вихідні(Сб-Нд)  577    40   6.9%    84 362    69 240   +15 122   1.22x\n"
        "Разом          2239   280  12.5%   634 070   268 680  +365 390   2.36x"
        "</pre>\n\n"
        "<b>Висновки:</b>\n"
        "• Будні дають <b>+350 тис чистими</b> за 2 міс; вихідні — лише <b>+15 тис</b> (майже в нуль).\n"
        "• Кожна гривня реклами: будні <b>×2.76</b>, вихідні <b>×1.22</b>.\n"
        "• Маржа з одного ліда: 330 грн (буд) vs 146 грн (вих); чистими після реклами — 211 vs 26 грн.\n"
        "• Вихідні = 26% бюджету, але лише 13% маржі; авто з вихідних коштує вдвічі дорожче (1731 vs 831 грн).\n"
        "• <b>Рекомендація:</b> перерозподілити рекламний бюджет із вихідних у будні — ROI у ~2.3 раза вищий."
    )
    resp = notifier.send_raw(report, "-1004407179676", thread)
    return jsonify({"sent_ok": resp.get("ok"), "thread": thread, "tg_response": resp})
